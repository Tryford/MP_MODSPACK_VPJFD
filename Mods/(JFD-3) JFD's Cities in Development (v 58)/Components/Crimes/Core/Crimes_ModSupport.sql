...
SET Text = '[COLOR_POSITIVE_TEXT]City Planning[ENDCOLOR][NEWLINE] Cities earn +10 [ICON_FOOD] Food and [ICON_CULTURE] Culture when they construct Buildings. Bonuses scale with Era. Reduces [ICON_JFD_CRIME_BLACK] Crime from [ICON_JFD_CRIME_ORANGE] Theft by 15% in all cities.'
WHERE Tag = 'TXT_KEY_POLICY_COLLECTIVE_RULE_HELP' 
AND EXISTS (SELECT * FROM JFD_GlobalUserSettings WHERE Type = 'JFD_CID_COMMUNITY_BALANCE_PATCH' AND Value = 1)
AND EXISTS (SELECT * FROM JFD_GlobalUserSettings WHERE Type = 'JFD_CID_CRIMES_POLICY_CHANGES_REPUBLIC' AND Value = 1);

UPDATE Language_en_US
SET Text = '[COLOR_POSITIVE_TEXT]Free Thought[ENDCOLOR][NEWLINE]+3 [ICON_RESEARCH] Science and +2 [ICON_CULTURE] Culture from Villages, and -100% [ICON_HAPPINESS_3] Religious Unrest in all cities. Reduces [ICON_JFD_CRIME_BLACK] Crime from [ICON_JFD_CRIME_BLUE] Fraud by 15% in all cities.'
