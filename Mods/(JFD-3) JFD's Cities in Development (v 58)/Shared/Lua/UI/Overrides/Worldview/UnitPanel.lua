...
function UpdateUnitPromotions(unit)
    ...
    for unitPromotion in GameInfo.UnitPromotions("ShowInUnitPanel = 1") do
...
function UpdateUnitStats(unit)
	...
	if userSettingBuildCharges then
		local numCharges, numChargesMax = JFD_GetNumBuildCharges(activePlayerID, unit, unit:GetUnitType())
	...
	-- Distance
	if (userSettingColoniesCore and userSettingColoniesDistanceRadius <= 3) then
		if activePlayer:HasPolicy(policyDecolonizationID) then return end
		if (unit:IsFoundAbroad() or unit:IsFound() or unit:IsUnitClass(GameInfoTypes.UNITCLASS_GREAT_GENERAL)) then
			local distance, distanceThreshold = JFD_GetUnitDistanceStats(unit)
...
function TipHandler( control )
	...
	if (bBuild) then
		...
		if userSettingBuildCharges then
			local iBuildCharges = buildInfo.ChargeCost
			for row in GameInfo.Build_JFD_ExtraChargeCost("BuildType = '" .. buildInfo.Type .. "'") do
				if ((not row.RequiresPolicy) or (row.RequiresPolicy and pActivePlayer:HasPolicy(GameInfoTypes[row.RequiresPolicy]))) then
					iBuildCharges = iBuildCharges + row.ExtraChargeCost
				end
			end	
		...
		for yield in GameInfo.Yields("IsFake = 1") do
			...
			if yieldID then
				local amount = JFD_CalculateFakeYield(pActivePlayer:GetID(), yieldID, pPlot, improvementID) 
				if amount > 0 then
