...
function UpdateDisplay()
	...
	if userSettingCrimeCore then
		Controls.SortCrime.SetHide(false)
		Controls.SortStrength.SetHide(true)
	else
		Controls.SortCrime.SetHide(true)
		Controls.SortStrength.SetHide(false)	
	end
    for pCity in pPlayer:Cities() do
		...
		-- JFD Crime
		if userSettingCrimeCore then
			sortEntry.Crime = math.floor( JFD_GetCrimeTotal(Game.GetActivePlayer(), pCity) );
			instance.Crime:SetText( sortEntry.Crime );
			instance.Crime:SetHide(false)
			instance.Crime:SetToolTipString(JFD_GetCrimeBreakdownTT(Game.GetActivePlayer(), pCity))
			instance.Defense:SetHide(true)
		else
			sortEntry.Strength = math.floor( pCity:GetStrengthValue() / 100 );
			instance.Defense:SetText( sortEntry.Strength );
			instance.Crime:SetHide(true)
			instance.Defense:SetHide(false)
        end
...
-- END   
		...
		elseif(pCity:IsPuppet() and not JFD_IsCityColony(pCity)) then
			instance.IconCapital:SetText("[ICON_PUPPET]");
			instance.IconCapital:SetHide(false);
		elseif(pCity:IsPuppet() and JFD_IsCityColony(pCity)) then
			instance.IconCapital:SetText("[ICON_JFD_COLONY]");
			instance.IconCapital:SetHide(false);
...
Controls.SortCrime:RegisterCallback( Mouse.eLClick, OnSort );
...
Controls.SortCrime:SetVoid1( eCrime );
