print("This is the modded CityBannerManager from 'UI - Improved City Banner' and JFD's Cities in Development")
...
local userSettingHappinessCityBanner = JFD_GetUserSetting("JFD_CID_HAPPINESS_CORE_CITY_BANNER_TURNS")
local userSettingHealthCore 		 = JFD_GetUserSetting("JFD_CID_HEALTH_CORE") == 1
local userSettingHealthCityBanner 	 = JFD_GetUserSetting("JFD_CID_HEALTH_CORE_CITY_BANNER_TURNS")
...
local isUsingSuzerainty				 = JFD_IsUsingSuzerainty()
------------------------------
-- JFD
------------------------------
...
function RefreshCityBanner(cityBanner, iActiveTeam, iActivePlayer)
	...
	if city ~= nil then
		...
		if userSettingProvincesCore and (not JFD_IsCityColony(city)) then
			--JFD Province Status
			local iconString = nil
			if isActivePlayerCity then
				local provinceID = JFD_GetProvinceID(cityBanner.playerID, city)
				local provinceTitle = JFD_GetProvinceTitle(cityBanner.playerID, city)
		...
		if (isActivePlayerCity) then
			if (city:IsPuppet() and not player:MayNotAnnex() and not JFD_IsCityColony(city)) then
...
-- COMMUNITY PATCH
		if (isActivePlayerCity and JFD_IsUsingCBP()) then
...
-- END
		...
		-- Puppet Status
		if (city:IsPuppet() and not JFD_IsCityColony(city)) then
		...
		--JFD Crime Status
 		if userSettingOrganizedCrimeCore then
			local organizedCrimeID = JFD_GetOrganizedCrimeID(cityBanner.playerID, city)
		...
		if userSettingHappinessCore and (isActivePlayerCity or bHasSpy) then
			local excessHappiness, excessUnhappiness = JFD_GetExcessHappiness(cityBanner.playerID, city, true)
			if excessUnhappiness > excessHappiness then
				controls.HappinessIcon:SetHide(false)
				if isActivePlayerCity then
					local resistanceCounter, resistanceThreshold, resistanceTurns = JFD_GetResistanceCounter(cityBanner.playerID, city, true, true)
					if resistanceTurns <= userSettingHappinessCityBanner then	
		...
		if userSettingLoyaltyCore then
			local loyaltyID = JFD_GetCityLoyaltyID(cityBanner.playerID, city)
			if (loyaltyID < 3 and loyaltyID > -1) then
				...
				if isActivePlayerCity then
					if (loyaltyID == GameInfoTypes["LOYALTY_JFD_PATRIOTIC"] or loyaltyID == GameInfoTypes["LOYALTY_JFD_ALLEGIANT"]) then
						local WLTKDCounter, WLTKDThreshold, WLTKDRate, WLTKDTurns = JFD_GetLoyaltyCounter(cityBanner.playerID, city, loyaltyID, false, false, false, false, true, true, true)
						controls.LoyaltyIcon:LocalizeAndSetToolTip("TXT_KEY_JFD_CITYBANNER_LOYALTY_TEXT_TT_WLTKD", loyalty.Description, WLTKDTurns)
					else
						local revoltCounter, revoltThreshold, revoltRate, revoltTurns = JFD_GetLoyaltyCounter(cityBanner.playerID, city, loyaltyID, false, true, true, true)
		...
		if userSettingHealthCore then
			if JFD_IsCityPlagued(city) then
				...
				local numPlagueTurns = JFD_GetNumPlagueTurns(city)
				...
				if isActivePlayerCity then
					local excessHealth, excessDisease = JFD_GetExcessHealth(cityBanner.playerID, city, true)
					if excessDisease > excessHealth then
						local plagueCounter, plagueThreshold, plagueTurns = JFD_GetPlagueCounter(cityBanner.playerID, city, true, true)
						if plagueTurns <= userSettingHealthCityBanner then	
							controls.PlagueIcon:SetHide(false)
		...
		-- Update strength
		local cityStrengthStr = JFD_GetCityDefense(city)
		...
		if(controls.BuildGrowth) then
			...
			if (city:IsProduction() and not city:IsProductionProcess()) then
				if (city:GetCurrentProductionDifferenceTimes100(false, false) > 0) then
					buildGrowth = JFD_GetProductionTurnsLeft(city:GetOwner(), city) -- JFD Happiness
				end
			elseif (city:IsProductionProcess() and city:GetProductionProcess() == GameInfoTypes.PROCESS_STOCKPILE) then
		...
		if( controls.MinorIndicator and controls.StatusIcon ) then
			...
			if( player:IsMinorCiv() ) then
				...
				if isUsingSuzerainty then
					local iconAtlas, iconIndex = GetCityStateIcon(player)
...
function OnBannerClick( x, y )
	...
	if plot then
		...
		if playerID == Game.GetActivePlayer() then
			-- Puppets are special
			if (plot:GetPlotCity():IsPuppet() and not player:MayNotAnnex() and not JFD_IsCityColony(plot:GetPlotCity())) then
				local popupInfo = {
...
function OnProdClick( cityID, prodName )
	...
	if city and not city:IsPuppet() and not JFD_IsCityColony(city) then
		local popupInfo = {
