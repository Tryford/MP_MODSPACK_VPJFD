...
local userSettingColonies = JFD_GetUserSetting("JFD_CID_COLONIES_CORE") == 1
local userSettingCrimes = JFD_GetUserSetting("JFD_CID_CRIMES_CORE") == 1
local userSettingHealth = JFD_GetUserSetting("JFD_CID_HEALTH_CORE") == 1
local userSettingSovereignty = JFD_GetUserSetting("JFD_RTP_SOVEREIGNTY_CORE") == 1
local userSettingRankingsDropdown = JFD_GetUserSetting("JFD_CID_MISC_RANKINGS_DROPDOWN") == 1
...
function GetScoreFromMode(iPlayer, iMode)
	...
	-- Health Mode
	elseif (iMode == g_ListMode_Health) and userSettingHealth then
		return pPlayer:CalculateTotalYield(GameInfoTypes["YIELD_JFD_HEALTH"])

	-- Crimes Mode
	elseif (iMode == g_ListMode_Crimes) and userSettingCrimes then
		return pPlayer:CalculateTotalYield(GameInfoTypes["YIELD_JFD_CRIME"])

	-- Colonies Mode
	elseif (iMode == g_ListMode_Colonies) and userSettingColonies  then
		return JFD_GetNumColonies(iPlayer)

	-- Autocracy Mode
	elseif (iMode == g_ListMode_Autocracy) and userSettingSovereignty and pPlayer:HasGovernment() then
		return JFD_GetReformIndex(iPlayer, true, false, false)

	-- Freedom Mode
	elseif (iMode == g_ListMode_Freedom) and userSettingSovereignty and pPlayer:HasGovernment() then
		return JFD_GetReformIndex(iPlayer, false, true, false)

	-- Order Mode
	elseif (iMode == g_ListMode_Order) and userSettingSovereignty and pPlayer:HasGovernment() then
		return JFD_GetReformIndex(iPlayer, false, false, true)
