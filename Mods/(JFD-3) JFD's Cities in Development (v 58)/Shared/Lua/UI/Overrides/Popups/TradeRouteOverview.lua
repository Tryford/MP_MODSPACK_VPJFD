	...
	if (pUnit) then
	  if (UIManager:GetControl()) then
	    if (pUnit:IsTrade() and not pUnit:IsRecalledTrader()) then
		  ...
		  instance.TurnsLeft1:LocalizeAndSetText("TXT_KEY_TRO_RECALLED")
		  instance.TurnsLeft2:LocalizeAndSetText("TXT_KEY_TRO_RECALLED")
		end
	  end
...
function DisplayData()
    ...
    for i,v in ipairs(g_Data) do
		...
		if userSettingSlaveryCore then
			...
			local toSlaves = "";
			if (v.FromID ~= v.ToID) then
				local fromCitySlaves = JFD_GetNumShacklesFromTradeRoute(nil, v.FromID, v.FromCity)
				if(fromCitySlaves > 0) then
					fromSlaves = fromCitySlaves
					instance.FromSlaves:LocalizeAndSetToolTip("TXT_KEY_TRO_COL_SLAVES_FROM_TT", fromSlaves);	
				end
				local toCitySlaves = JFD_GetNumShacklesFromTradeRoute(nil, v.ToID, v.ToCity)
				if(toCitySlaves > 0) then
					toSlaves = toCitySlaves
					instance.ToSlaves:LocalizeAndSetToolTip("TXT_KEY_TRO_COL_SLAVES_TO_TT", toSlaves);	
				end
				instance.FromSlaves:SetText("[COLOR_JFD_SLAVES]" .. fromSlaves .. "[ENDCOLOR]");
				instance.ToSlaves:SetText("[COLOR_JFD_SLAVES]" .. toSlaves .. "[ENDCOLOR]");
			end
		end
		...
		if userSettingHealthCore then
			local fromDisease = "";
			local toDisease = "";
			local fromCityHealth, fromCityDisease = JFD_GetHealthTotal(v.FromID, v.FromCity, true, true)
			fromCityDisease = JFD_Round((fromCityDisease*JFD_GetDiseaseFromTradeRoutes(v.FromID, v.FromCity))/100)
			if(fromCityDisease > 0) then
				fromDisease = fromCityDisease
				instance.FromDisease:LocalizeAndSetToolTip("TXT_KEY_TRO_COL_DISEASE_FROM_TT", fromDisease);	
			end
			local toCityHealth, toCityDisease = JFD_GetHealthTotal(v.ToID, v.ToCity, true, true)
			toCityDisease = JFD_Round((toCityDisease*JFD_GetDiseaseFromTradeRoutes(v.ToID, v.ToCity))/100)
			if(toCityDisease > 0) then
				toDisease = toCityDisease
				instance.ToDisease:LocalizeAndSetToolTip("TXT_KEY_TRO_COL_DISEASE_TO_TT", toDisease);	
			end
			instance.FromDisease:SetText("[COLOR_JFD_DISEASE]" .. fromDisease .. "[ENDCOLOR]");
			instance.ToDisease:SetText("[COLOR_JFD_DISEASE]" .. toDisease .. "[ENDCOLOR]");
		end
		-------------------------------------------
		-- JFD 
		-------------------------------------------
		...
		instance.TurnsLeft1:SetText("[COLOR_POSITIVE_TEXT]" .. strTurnsRemaining .. "[ENDCOLOR]");
		instance.TurnsLeft2:SetText("[COLOR_POSITIVE_TEXT]" .. strTurnsRemaining .. "[ENDCOLOR]");
