...
function RefreshData()
	...
	local iOriginOwner = pOriginCity:GetOwner();
...
-- END
		...
		if userSettingSlaveryCore then
			local fromCitySlaves = JFD_GetNumShacklesFromTradeRoute(nil, iOriginOwner, pOriginCity)
			if fromCitySlaves ~= 0 then
				theirBonuses = theirBonuses .. "  " .. Locale.Lookup("TXT_KEY_CHOOSE_INTERNATIONAL_TRADE_ROUTE_ITEM_JFD_SLAVES", fromCitySlaves);
			end	
			local toCitySlaves = JFD_GetNumShacklesFromTradeRoute(nil, iTargetOwner, pTargetCity)
			if toCitySlaves ~= 0 then
				myBonuses = myBonuses .. "  " .. Locale.Lookup("TXT_KEY_CHOOSE_INTERNATIONAL_TRADE_ROUTE_ITEM_JFD_SLAVES", toCitySlaves);
			end	
		end
		
		--JFD Disease
		if userSettingHealthCore then
			local fromCityHealth, fromCityDisease = JFD_GetHealthTotal(iOriginOwner, pOriginCity, true, true)
			fromCityDisease = JFD_Round((fromCityDisease*JFD_GetDiseaseFromTradeRoutes(iOriginOwner, pOriginCity))/100)
			if fromCityDisease ~= 0 then
				theirBonuses = theirBonuses .. "  " .. Locale.Lookup("TXT_KEY_CHOOSE_INTERNATIONAL_TRADE_ROUTE_ITEM_JFD_DISEASE", fromCityDisease);
			end	
			local toCityHealth, toCityDisease = JFD_GetHealthTotal(iTargetOwner, pTargetCity, true, true)
			toCityDisease = JFD_Round((toCityDisease*JFD_GetDiseaseFromTradeRoutes(iOriginOwner, pOriginCity))/100)
			if toCityDisease ~= 0 then
				myBonuses = myBonuses .. "  " .. Locale.Lookup("TXT_KEY_CHOOSE_INTERNATIONAL_TRADE_ROUTE_ITEM_JFD_DISEASE", toCityDisease);
			end	
		end
		-------------------------------------------
		-- JFD 
		...
		if (pUnit:GetUnitClassType() == GameInfoTypes["UNITCLASS_CARGO_SHIP"] and pPlayer:GetCivilizationType() == GameInfoTypes["CIVILIZATION_JFD_EGYPT_HATSHEPSUT"]) then
			if (pTargetCity:IsCapital()) then
				theirBonuses = theirBonuses .. "  " .. "[ICON_ARROW_LEFT] " .. Locale.Lookup("TXT_KEY_JFD_EGYPT_HATSHEPSUT_WLTKD")
			end
		end
...
-- CBP
		...
		if isUsingCBP and (pTargetCity ~= nil and pPlayer ~= nil and tradeRoute.Category == 1) then
