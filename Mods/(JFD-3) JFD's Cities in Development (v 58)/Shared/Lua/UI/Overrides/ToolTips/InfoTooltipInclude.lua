...
-- END
	...
	-- CBP
	if userSettingInvestmentsCoreUnits then
		if (pCity ~= nil) then
			if pCity:GetUnitInvestment(iUnitID) <= 0 then
				local strInvestment = "TXT_KEY_PRODUCTION_INVESTMENT_UNIT"
				local iAmount = JFD_GetInvestmentValue(Game.GetActivePlayer(), false, false, true)
				iAmount = (iAmount * -1);
...
-- END
	...
	-- Culture Mod
	local iCulture = Game.GetBuildingYieldModifier(iBuildingID, YieldTypes.YIELD_CULTURE);
	iCulture = iCulture + GameInfo.Buildings[iBuildingID].CultureRateModifier
	iCulture = iCulture + pActivePlayer:GetPolicyBuildingClassYieldModifier(buildingClassID, YieldTypes.YIELD_CULTURE);
	if (pCity ~= nil) then
		--iCulture = iCulture + pCity:GetEventBuildingClassYieldModifier(buildingClassID, YieldTypes.YIELD_CULTURE);
	end
	if (iCulture ~= nil and iCulture ~= 0) then
		if iCulture >= 0 then
			table.insert(lines, Locale.ConvertTextKey("TXT_KEY_PRODUCTION_BUILDING_CULTURE_MOD", iCulture));
		else
			table.insert(lines, Locale.ConvertTextKey("TXT_KEY_PRODUCTION_BUILDING_CULTURE_MOD_NEGATIVE", iCulture));
		end
	end
	
	-- Culture
	local iCulture = Game.GetBuildingYieldChange(iBuildingID, YieldTypes.YIELD_CULTURE) + pActivePlayer:GetPolicyBuildingClassYieldChange(buildingClassID, YieldTypes.YIELD_CULTURE);
	if (pCity ~= nil) then
...
-- END
	...
	-- Faith Mod
	local iFaith = Game.GetBuildingYieldModifier(iBuildingID, YieldTypes.YIELD_FAITH);
	iFaith = iFaith + pActivePlayer:GetPolicyBuildingClassYieldModifier(buildingClassID, YieldTypes.YIELD_FAITH);
	if (pCity ~= nil) then
		--iFaith = iFaith + pCity:GetEventBuildingClassYieldModifier(buildingClassID, YieldTypes.YIELD_FAITH);
	end
	if (iFaith ~= nil and iFaith ~= 0) then
		if iFaith >= 0 then
			table.insert(lines, Locale.ConvertTextKey("TXT_KEY_PRODUCTION_BUILDING_FAITH_MOD", iFaith));
		else
			table.insert(lines, Locale.ConvertTextKey("TXT_KEY_PRODUCTION_BUILDING_FAITH_MOD_NEGATIVE", iFaith));
		end
	end
	
	-- Faith
	local iFaith = Game.GetBuildingYieldChange(iBuildingID, YieldTypes.YIELD_FAITH) + pActivePlayer:GetPolicyBuildingClassYieldChange(buildingClassID, YieldTypes.YIELD_FAITH);
	if (pCity ~= nil) then
...
-- END
	...
	if userSettingHealthCore then
		local healthID = GameInfo.Yields["YIELD_JFD_HEALTH"].ID
		-- Health Mod
		local iHealth = Game.GetBuildingYieldModifier(iBuildingID, healthID);
		iHealth = iHealth + pActivePlayer:GetPolicyBuildingClassYieldModifier(buildingClassID, healthID);
		if (pCity ~= nil) then
			iHealth = iHealth + pCity:GetReligionBuildingYieldRateModifier(buildingClassID, healthID);
		end
		if (iHealth ~= nil and iHealth ~= 0) then
			if iHealth >= 0 then
				table.insert(lines, Locale.ConvertTextKey("TXT_KEY_PRODUCTION_BUILDING_HEALTH_MOD", "+" .. iHealth));
			else
				table.insert(lines, Locale.ConvertTextKey("TXT_KEY_PRODUCTION_BUILDING_HEALTH_MOD_NEGATIVE", iHealth));
			end
		end

		iHealth = Game.GetBuildingYieldChange(iBuildingID, healthID);
		iHealth = iHealth + pActivePlayer:GetPlayerBuildingClassYieldChange(buildingClassID, healthID)
		iHealth = iHealth + pActivePlayer:GetPolicyBuildingClassYieldChange(buildingClassID, healthID)
		if (pCity ~= nil) then
			iHealth = iHealth + pCity:GetReligionBuildingClassYieldChange(buildingClassID, healthID)
			iHealth = iHealth + pCity:GetEventBuildingClassYield(buildingClassID, healthID);
		end
		if (iHealth ~= 0) then
			if iHealth > 0 then
	...
	-- JFD Disease
	if userSettingHealthCore then
		local diseaseID = GameInfo.Yields["YIELD_JFD_DISEASE"].ID
		-- Disease Mod
		local iDisease = Game.GetBuildingYieldModifier(iBuildingID, diseaseID);
		iDisease = iDisease + pActivePlayer:GetPolicyBuildingClassYieldModifier(buildingClassID, diseaseID);
		...
		iDisease = Game.GetBuildingYieldChange(iBuildingID, diseaseID);
		iDisease = iDisease + pActivePlayer:GetPlayerBuildingClassYieldChange(buildingClassID, diseaseID)
		iDisease = iDisease + pActivePlayer:GetPolicyBuildingClassYieldChange(buildingClassID, diseaseID)
		if (pCity ~= nil) then
			iDisease = iDisease + pCity:GetReligionBuildingClassYieldChange(buildingClassID, diseaseID)
			iDisease = iDisease + pCity:GetReligionBuildingYieldRateModifier(buildingClassID, diseaseID);
			iDisease = iDisease + pCity:GetEventBuildingClassYield(buildingClassID, diseaseID);
		end
		if (iDisease ~= 0) then
			if iDisease > 0 then
	...
	-- Tourism Change
	local iTourism = Game.GetBuildingYieldChange(iBuildingID, YieldTypes.YIELD_TOURISM) + pActivePlayer:GetPolicyBuildingClassYieldChange(buildingClassID, YieldTypes.YIELD_TOURISM);
	if (pCity ~= nil) then
		iTourism = iTourism + pCity:GetReligionBuildingClassYieldChange(buildingClassID, YieldTypes.YIELD_TOURISM) + pActivePlayer:GetPlayerBuildingClassYieldChange(buildingClassID, YieldTypes.YIELD_TOURISM);
		iTourism = iTourism + pCity:GetLeagueBuildingClassYieldChange(buildingClassID, YieldTypes.YIELD_TOURISM);
		iTourism = iTourism + pCity:GetEventBuildingClassYield(buildingClassID, YieldTypes.YIELD_TOURISM);
	end
	...
	-- Golden Age Points
	local iGoldenAgePoints = Game.GetBuildingYieldChange(iBuildingID, YieldTypes.YIELD_GOLDEN_AGE_POINTS) + pActivePlayer:GetPolicyBuildingClassYieldChange(buildingClassID, YieldTypes.YIELD_GOLDEN_AGE_POINTS);
	if (pCity ~= nil) then
		iGoldenAgePoints = iGoldenAgePoints + pCity:GetReligionBuildingClassYieldChange(buildingClassID, YieldTypes.YIELD_GOLDEN_AGE_POINTS) + pActivePlayer:GetPlayerBuildingClassYieldChange(buildingClassID, YieldTypes.YIELD_GOLDEN_AGE_POINTS);
		iGoldenAgePoints = iGoldenAgePoints + pCity:GetLeagueBuildingClassYieldChange(buildingClassID, YieldTypes.YIELD_GOLDEN_AGE_POINTS);
		iGoldenAgePoints = iGoldenAgePoints + pCity:GetEventBuildingClassYield(buildingClassID, YieldTypes.YIELD_GOLDEN_AGE_POINTS);
	end
	...
	-- Food Mod
	local iFood = Game.GetBuildingYieldModifier(iBuildingID, YieldTypes.YIELD_FOOD);
	iFood = iFood + pActivePlayer:GetPolicyBuildingClassYieldModifier(buildingClassID, YieldTypes.YIELD_FOOD);
	if (pCity ~= nil) then
	...
	if (iFood ~= nil and iFood ~= 0) then
		if iFood >= 0 then
			table.insert(lines, Locale.ConvertTextKey("TXT_KEY_PRODUCTION_BUILDING_FOOD_MOD", iFood));
		else
			table.insert(lines, Locale.ConvertTextKey("TXT_KEY_PRODUCTION_BUILDING_FOOD_MOD_NEGATIVE", iFood));
		end
	end
	
	-- Food
	iFood = Game.GetBuildingYieldChange(iBuildingID, YieldTypes.YIELD_FOOD) + pActivePlayer:GetPolicyBuildingClassYieldChange(buildingClassID, YieldTypes.YIELD_FOOD);
	if (pCity ~= nil) then
...
-- END
	...
	if (iGold ~= nil and iGold ~= 0) then
		if iGold >= 0 then
			...
			table.insert(lines, Locale.ConvertTextKey("TXT_KEY_PRODUCTION_BUILDING_GOLD_MOD_NEGATIVE", iGold));
		end
	end
	
	-- Gold Change
	iGold = Game.GetBuildingYieldChange(iBuildingID, YieldTypes.YIELD_GOLD) + pActivePlayer:GetPolicyBuildingClassYieldChange(buildingClassID, YieldTypes.YIELD_GOLD);
...
-- END	
	if (iScience ~= nil and iScience ~= 0) then
		if iScience >= 0 then
			...
			table.insert(lines, Locale.ConvertTextKey("TXT_KEY_PRODUCTION_BUILDING_SCIENCE_MOD_NEGATIVE", iScience));
		end
	end
	...
	if (iScienceChange ~= nil and iScienceChange ~= 0) then
		if iScienceChange >= 0 then
			...
			table.insert(lines, Locale.ConvertTextKey("TXT_KEY_PRODUCTION_BUILDING_SCIENCE_NEGATIVE", iScienceChange));
		end
	end
	...
	-- Production Change
	local iProd = Game.GetBuildingYieldChange(iBuildingID, YieldTypes.YIELD_PRODUCTION) + pActivePlayer:GetPolicyBuildingClassYieldChange(buildingClassID, YieldTypes.YIELD_PRODUCTION);
-- CBP
	if (pCity ~= nil) then
...
-- END	
	...
	if userSettingLoyaltyCore then
		...
		if (iLoyalty ~= nil and iHealth ~= 0) then
			if iLoyalty ~= 0 then
	...
	if userSettingSovereigntyCore then
		...
		if ((not isBuildingFree) and iSovereignty ~= nil and iSovereignty ~= 0) then		
			if iSovereignty > 0 then
				table.insert(lines, Locale.ConvertTextKey("TXT_KEY_PRODUCTION_BUILDING_SOVEREIGNTY", iSovereignty));
			else
				table.insert(lines, Locale.ConvertTextKey("TXT_KEY_PRODUCTION_BUILDING_SOVEREIGNTY_NEGATIVE", iSovereignty));
			end
		end
	...
	local iNumGAPoints = Game.GetBuildingYieldChange(iBuildingID, GameInfo.Yields["YIELD_GREAT_ADMIRAL_POINTS"].ID)
	if (iNumGAPoints > 0) then
		table.insert(lines, Locale.ConvertTextKey("TXT_KEY_PRODUCTION_BUILDING_GREAT_ADMIRAL_POINTS", iNumGAPoints)); 
	end
	local iNumGGPoints = Game.GetBuildingYieldChange(iBuildingID, GameInfo.Yields["YIELD_GREAT_GENERAL_POINTS"].ID)
	if (iNumGGPoints > 0) then
		table.insert(lines, Locale.ConvertTextKey("TXT_KEY_PRODUCTION_BUILDING_GREAT_GENERAL_POINTS", iNumGGPoints)); 
	end
-- CBP
	if (pCity ~= nil) then
...
-- CBP FIX
	...
	if userSettingDevelopmentCore then
		...
		local iDevelopment = JFD_GetBuildingDevelopment(pBuildingInfo.Type)
	...
	for pResource in GameInfo.Resources("Type = 'RESOURCE_JFD_POWER'") do
		...
		for row in GameInfo.Building_ResourceQuantity("BuildingType = '" .. pBuildingInfo.Type .. "' AND ResourceType = '" .. pResource.Type .. "'") do
			local localizedText = ""
			localizedText = localizedText .. pResource.IconString .. " " .. Locale.ConvertTextKey(pResource.Description) .. ": +" .. row.Quantity
			table.insert(lines, localizedText);
		end
 	end
	...
	if (strWrittenHelpText ~= nil and strWrittenHelpText ~= "") then
		-- Separator
		strHelpText = strHelpText .. "[NEWLINE]----------------[NEWLINE]" .. strWrittenHelpText 
	end

-- CBP
	if ((isUsingCBP or userSettingInvestmentsCore) and (not bIsMagistrate)) then
		if (pCity ~= nil) then
			if(pCity:GetNumRealBuilding(iBuildingID) <= 0) then
				if(pCity:GetNumFreeBuilding(iBuildingID) <= 0 and (pCity:GetBuildingInvestment(iBuildingID) <= 0)) then
					...
					local iAmount = JFD_GetInvestmentValue(Game.GetActivePlayer(), false, isWonder)
					iAmount = (iAmount * -1);
					if isWonder then
...
function GetCultureTooltip(pCity)
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_POLICIES)) then
		...
		local iCultureFromBuildings = pCity:GetJONSCulturePerTurnFromBuildings()+JFD_GetRealCrimeLosses(pCity, GameInfoTypes["CRIME_JFD_CULTURE"])
...
-- END		
		...
		local iAmount, strYieldFromCrime = JFD_GetRealCrimeLosses(pCity,  GameInfoTypes["CRIME_JFD_CULTURE"]);
		if iAmount > 0 then
			strCultureToolTip = strCultureToolTip .. "[NEWLINE][NEWLINE]";
...
function GetFaithTooltip(pCity)
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_RELIGION)) then
		...
		local iFaithFromBuildings = pCity:GetFaithPerTurnFromBuildings()+JFD_GetRealCrimeLosses(pCity, GameInfoTypes["CRIME_JFD_FAITH"])
...
-- END
		...
		local iYieldFromCrime, strYieldFromCrime = JFD_GetRealCrimeLosses(pCity, GameInfoTypes["CRIME_JFD_FAITH"]);
		if iYieldFromCrime > 0 then
			table.insert(faithTips, "[ICON_BULLET]" .. Locale.ConvertTextKey(strYieldFromCrime, iYieldFromCrime));
		end
		
		-- Modifiers
		local iFaith = pCity:GetYieldRateModifier(YieldTypes.YIELD_FAITH)
		if iFaith ~= 0 then
...
function GetTourismTooltip(pCity)
	...
	-- JFD
	local tourismFromCrime, strYieldFromCrime = JFD_GetRealCrimeLosses( pCity, GameInfoTypes["CRIME_JFD_TOURISM"]);
...
function GetYieldTooltip(pCity, iYieldType, iBase, iTotal, strIconString, strModifiersString)
	...
	-- JFD Yield from Crime
	local crimeID = nil
	local yieldType = GameInfo.Yields[iYieldType].Type
	for row in GameInfo.JFD_Crimes("YieldType = '" .. yieldType .. "'") do
		crimeID = row.ID
		break
	end
	local iLossesFromCrime = JFD_GetRealCrimeLosses(pCity, crimeID)
	iYieldFromBuildings = iYieldFromBuildings + iLossesFromCrime
	--JFD Yield from Pillaged Tiles Begins
	local traitType = JFD_GetTraitType(activePlayerID)
...
-- END
	...
	local iYieldFromCrime, strYieldFromCrime = JFD_GetRealCrimeLosses(pCity, crimeID)
	if iYieldFromCrime and iYieldFromCrime > 0 then
