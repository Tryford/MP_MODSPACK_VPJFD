...
local function AddPreWrittenHelpTextAndConcat( tips, row ) -- assumes tips is custom EUI table object
	...
	if row and row.Help then
		-- Additional Help
		if GameInfo.Buildings[row.Type] then
			for building in GameInfo.Building_JFD_HelpTexts() do
				if (building.BuildingType == row.Type and building.IsWrittenFirst == 1 and building.HelpText) then
					tip = tip .. Locale.ConvertTextKey( building.HelpText );
		...
		if GameInfo.Buildings[row.Type] then
			for building in GameInfo.Building_JFD_HelpTexts() do
				if (building.BuildingType == row.Type and building.IsWrittenFirst == 0 and building.HelpText) then
					tip = tip .. Locale.ConvertTextKey( building.HelpText );
					tip = tip .. "[NEWLINE][NEWLINE]";
				end
			end
...
-- END CBP
	...
	if userSettingCrimeCore then
		local thisBuilding = { BuildingType = building.Type }
		for row in GameInfo.Building_JFD_CrimeMods(thisBuilding) do
			tips:insert( L("TXT_KEY_PRODUCTION_BUILDING_SPECIFIC_CRIME_MOD_EUI", row.CrimeMod, GameInfo.JFD_Crimes[row.CrimeType].IconString, GameInfo.JFD_Crimes[row.CrimeType].Description) .. tip )
			tip = ""
		end
	end
...
-- CBP
	if city then
		if (isUsingCBP or userSettingInvestmentsCore) then
			if(city:GetBuildingInvestment(buildingID) <= 0) then
...
local function GetYieldTooltip( city, yieldID, baseYield, totalYield, yieldIconString, strModifiersString )
	...
	local yieldRateFromBuildings = city:GetBaseYieldRateFromBuildings( yieldID )
	...
	local lossesFromCrime = JFD_GetRealCrimeLosses(city, crimeID)
	yieldRateFromBuildings = yieldRateFromBuildings + lossesFromCrime
	tips:insertLocalizedBulletIfNonZero( "TXT_KEY_YIELD_FROM_BUILDINGS", yieldRateFromBuildings, yieldIconString )
...
-- END
	...
	local iAmount, strYieldFromCrime = JFD_GetRealCrimeLosses(city, crimeID)
	tips:insertLocalizedBulletIfNonZero( strYieldFromCrime, iAmount)
...
local function GetCultureTooltip( city )
	...
	if civ5_mode then
		...
		cultureFromBuildings = city:GetJONSCulturePerTurnFromBuildings() + JFD_GetRealCrimeLosses(city, GameInfoTypes["CRIME_JFD_CULTURE"])
...
-- END
	...
	local iAmount, strYieldFromCrime = JFD_GetRealCrimeLosses(city, GameInfoTypes["CRIME_JFD_CULTURE"])
	tips:insertLocalizedBulletIfNonZero( strYieldFromCrime, iAmount)
...
local function GetFaithTooltip( city )
	...
	if g_isReligionEnabled then
		...
		local faithFromBuildings = city:GetFaithPerTurnFromBuildings() + JFD_GetRealCrimeLosses(city, GameInfoTypes["CRIME_JFD_FAITH"])
		tips:insertLocalizedBulletIfNonZero( "TXT_KEY_FAITH_FROM_BUILDINGS",  faithFromBuildings)
...
-- END
		...
		local iAmount, strYieldFromCrime = JFD_GetRealCrimeLosses(city,  GameInfoTypes["CRIME_JFD_FAITH"])
		tips:insertLocalizedBulletIfNonZero( strYieldFromCrime, iAmount)
...
local function GetTourismTooltip(city)
	local strTourismToolTip = city:GetTourismTooltip();
	for row in GameInfo.Building_YieldChanges() do
		if (row.YieldType == 'YIELD_TOURISM' and row.Yield > 0) then
	...
	-- JFD
	local tourismFromCrime, strYieldFromCrime = JFD_GetRealCrimeLosses(city, GameInfoTypes["CRIME_JFD_TOURISM"])
