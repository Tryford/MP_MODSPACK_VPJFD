...
local userSettingCityDistricts			 = JFD_GetUserSetting("JFD_CID_MISC_CITY_DISTRICTS") == 1
...
local userSettingInvestmentsCoreUI		 = JFD_GetUserSetting("JFD_CID_INVESTMENTS_CORE_UI") == 1
local userSettingInvestmentsCoreUnits 	 = JFD_GetUserSetting("JFD_CID_INVESTMENTS_CORE_UNITS") == 1
local userSettingOrganizedCrimeCore 	 = JFD_GetUserSetting("JFD_CID_CRIMES_CORE_ORGANIZED") == 1
...
local dignitaryTexture = "citizenDignitary.dds";
local magistrateTexture = "citizenMagistrate.dds";
local doctorTexture = "citizenDoctor.dds";
--added by Gazebo
...
-- Provinces
-------------------------------------------------
function JFD_OnConfirmProvinceName()
	Controls.ProvinceNameConfirm:SetHide(true)
	Events.SerialEventEnterCityScreen()
end
Controls.EditOKButton:RegisterCallback(Mouse.eLClick, JFD_OnConfirmProvinceName)
...
function JFD_ValidateEditBox(sValue, control, bFire)
	...
	if (sValue and bFire and text:len() > 1) then
		...
		JFD_SetProvinceName(city, sValue)
	end
end
Controls.EditHeading:RegisterCallback(JFD_ValidateEditBox)

function JFD_OnEditNameClick()
	Controls.ProvinceNameConfirm:SetHide(false)
end
Controls.ProvinceString:RegisterCallback(Mouse.eLClick, JFD_OnEditNameClick)
			...
			for i = 0, iNumGreatWorks - 1, 1 do
				...
				if (iGreatWorkIndex >= 0) then
					...
					if userSettingCrimeCore then
						if (userSettingCrimesUffizi and (not pCity:HasBuilding(buildingUffiziID))) then 
							strGWTooltip = strGWTooltip .. ", " .. Locale.ConvertTextKey("+{1_Num} [ICON_JFD_CRIME_BLACK]", defineCrimePerGreatWorks)
						end
					end
					for row in GameInfo.Building_GreatWorkChangesUI("GreatWorkSlotType = '" .. buildingGreatWorkSlotType .. "'") do
						if pCity:HasBuilding(GameInfoTypes[row.BuildingType]) then
...
-- End
			...
			if userSettingDevelopmentCore then
				local developmentPerSpecialist = JFD_GetDevelopmentPerSpecialist(iPlayer, pCity, pSpecialistInfo.ID) 
			...
			--Crime
			if userSettingCrimeCore then
				local crimePerSpecialist = JFD_GetCrimePerSpecialist(iPlayer, pCity)
				if crimePerSpecialist ~= 0 then
					ToolTipString = ToolTipString .. " +" .. crimePerSpecialist .. "[ICON_JFD_CRIME_BLACK]"
				end
			end
			...
			elseif building.SpecialistType == "SPECIALIST_JFD_MONK" then
			...
			elseif building.SpecialistType == "SPECIALIST_JFD_DOCTOR" then
			...
			elseif building.SpecialistType == "SPECIALIST_JFD_DIGNITARY" then
				controlTable.BuildingFilledSpecialistSlot1:SetTexture(dignitaryTexture);
				controlTable.BuildingFilledSpecialistSlot2:SetTexture(dignitaryTexture);
				controlTable.BuildingFilledSpecialistSlot3:SetTexture(dignitaryTexture);
			elseif building.SpecialistType == "SPECIALIST_JFD_MAGISTRATE" then
...
-- JFD District List
-------------------------------------------------
function JFD_AddBuildingToDistrict(pCity, districtID, thisButton, thisButtonLabel, sortedList)
...
function OnCityViewUpdate()
	...
	if (pCity ~= nil) then
		...
		-- if userSettingSlaveryCore then -- JFD Slavery
			-- purchaseString = purchaseString .. "/[ICON_JFD_SHACKLES]"
		-- end
		-- if userSettingProvincesCoreLevies then -- JFD Provinces
		...
		-- Puppet Status
		if (pCity:IsPuppet()) and (not JFD_IsCityColony(pCity)) then
		...
		-- Colony Status
		if JFD_IsCityColony(pCity) then
			if (not userSettingProvincesCore) then
			...
			local canAssimilate, popReq, goldCost = JFD_CanAssimilateColony(iPlayer, pCity)
			local assimilateStringTT = nil
			if (not canAssimilate) then
				assimilateStringTT = Locale.ConvertTextKey( "TXT_KEY_CITYVIEW_JFD_ASSIMILATE_BUTTON_TT_DISABLED", popReq, goldCost )
				Controls.AssimilateColonyButton:SetDisabled(true)
			else
				assimilateStringTT = Locale.ConvertTextKey( "TXT_KEY_CITYVIEW_JFD_ASSIMILATE_BUTTON_TT", popReq, goldCost )
				Controls.AssimilateColonyButton:SetDisabled(false)
			end
...
-- END
		...
		local defense = JFD_GetCityDefense(pCity)
	    Controls.Defense:SetText(defense);
		Controls.Defense:SetToolTipString(JFD_GetCityDefenseTooltip(pCity));
		...
		elseif JFD_IsCityColony(pCity) then
			Controls.ProductionButton:SetDisabled( true );
		else
		...
		-------------------------------------------
		-- JFD Investment
		JFD_UpdateInvestmentInfo(pPlayer, pCity)		
		...
		if userSettingDevelopmentCore then
			...
			-- if developmentHeadingOpen then	
				local developmentRate, developmentTotal, developmentThreshold, developmentTT = JFD_GetDevelopment(iPlayer, pCity, true)
				local developmentIcon = "[ICON_JFD_DEVELOPMENT_SURPLUS]"
				local localizedLabel = ""
				if developmentRate > 0 then
					localizedLabel = localizedLabel .. Locale.ConvertTextKey( "TXT_KEY_JFD_CITYVIEW_DEVELOPMENT_TEXT", "[COLOR_POSITIVE_TEXT]+" .. developmentRate .. "[ENDCOLOR]");
				elseif developmentRate < 0 then
					developmentIcon = "[ICON_JFD_DEVELOPMENT_DEFICIT]"
					localizedLabel = localizedLabel .. Locale.ConvertTextKey( "TXT_KEY_JFD_CITYVIEW_DEVELOPMENT_TEXT", "[COLOR_NEGATIVE_TEXT]" .. developmentRate .. "[ENDCOLOR]");
				else
					localizedLabel = localizedLabel .. Locale.ConvertTextKey( "TXT_KEY_JFD_CITYVIEW_DEVELOPMENT_TEXT", developmentRate);
				end
			...
			-- else
				-- local developmentRate, developmentTotal, developmentThreshold, developmentTT = JFD_GetDevelopment(iPlayer, pCity, true)
				-- local localizedLabel = "[ICON_PLUS] "
				-- if developmentRate > 0 then
					-- localizedLabel = localizedLabel .. Locale.ConvertTextKey( "TXT_KEY_JFD_CITYVIEW_DEVELOPMENT_TEXT", "[COLOR_POSITIVE_TEXT]+" .. developmentRate .. "[ENDCOLOR]" );
				-- elseif developmentRate < 0 then
					-- localizedLabel = localizedLabel .. Locale.ConvertTextKey( "TXT_KEY_JFD_CITYVIEW_DEVELOPMENT_TEXT", "[COLOR_NEGATIVE_TEXT]" .. developmentRate .. "[ENDCOLOR]" );
		...
		if numSlackersInThisCity > 0 then
			...
			if slackerHeadingOpen then
				...
				-- Yield
				for pYieldInfo in GameInfo.Yields("Type <> 'YIELD_JFD_HAPPINESS'") do
					local iYieldID = pYieldInfo.ID;
					if(iYieldID ~= YieldTypes.YIELD_CULTURE) then
						local iYieldAmount = pCity:GetSpecialistYield(iSpecialistID, iYieldID);					
						--Specialist Yield included in pCity:GetSpecialistYield();
				...
				--Development
				if userSettingDevelopmentCore then
					local developmentPerSpecialist = JFD_GetDevelopmentPerSpecialist(pCity:GetOwner(), pCity, pSpecialistInfo.ID) 
					if developmentPerSpecialist ~= 0 then
						ToolTipString = ToolTipString .. " +" .. developmentPerSpecialist .. "[ICON_JFD_DEVELOPMENT]" 
					end
				end
				--Crime
				if userSettingCrimeCore then
					local crimePerSpecialist = JFD_GetCrimePerSpecialist(pCity:GetOwner(), pCity)
					if crimePerSpecialist ~= 0 then
						ToolTipString = ToolTipString .. " +" .. crimePerSpecialist .. "[ICON_JFD_CRIME_BLACK]"
					end
				end
		...
		if userSettingCityDistricts then
		...
		if userSettingCityDistricts then
			...
			for row in GameInfo.JFD_Districts() do
				...
				if (numDistrictBuildingsInThisCity[row.ID] and numDistrictBuildingsInThisCity[row.ID] > 0) then
					...
					JFD_AddBuildingToDistrict(pCity, row.ID, thisButton, thisButtonLabel, sortedList)
				end
			end
		else
		...
		if userSettingProvincesCore then
			local provinceTitle = JFD_GetProvinceTitle(iPlayer, pCity)
			local provinceTT = JFD_GetProvinceInfoTT(iPlayer, pCity)
			...
			local reqImpr, reqPop, cityImpr, cityPop = JFD_GetProvinceUpgradeRequirements(iPlayer, pCity)
...
--		UpdateSpecialists(pCity);
		...
		if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_SCIENCE)) then
			...
			if iSciencePerTurn >= 0 then
				...
				Controls.SciencePerTurnLabel:SetText( Locale.ConvertTextKey("TXT_KEY_CITYVIEW_PERTURN_TEXT_NEGATIVE", iSciencePerTurn) );
			end
		end
		...
		iTourismPerTurn = iTourismPerTurn - pCity:GetNumBuilding(GameInfoTypes["BUILDING_CRIMES_JFD_TOURISM"])
		...
		--Crime Status
		if userSettingCrimeCore then
			local crime = JFD_GetCrimeTotal(iPlayer, pCity)
			local crimeBreakdownTT = JFD_GetCrimeBreakdownTT(iPlayer, pCity)
			local crimeOrganizedBreakdownTT = JFD_GetOrganizedCrimeBreakdownTT(iPlayer, pCity)
			Controls.CrimeBox:SetHide(false)
			Controls.CrimeBox:SetToolTipString(crimeBreakdownTT)
			Controls.CrimePerTurnLabel:SetText("[COLOR_JFD_CRIME_BLACK]" .. crime .. "[ENDCOLOR]")
			-- Organized Crime Status
			if userSettingOrganizedCrimeCore then
				local organizedCrimeID = JFD_GetOrganizedCrimeID(iPlayer, pCity)
				local organizedCrimeIcon = "[ICON_JFD_ORGANIZED_CRIME]"
				local organizedCrimeDesc = "TXT_KEY_JFD_CITYVIEW_CRIME_ORGANIZED"
				local threshold = JFD_GetOrganizedCrimeThreshold(iPlayer, pCity)
				local thresholdColour = "[COLOR_JFD_CRIME_BLACK]"
				if organizedCrimeID > -1 then
					thresholdColour = "[COLOR_NEGATIVE_TEXT]"
					threshold = JFD_GetOrganizedCrimePercent(iPlayer, pCity, organizedCrimeID)
					local organizedCrime = GameInfo.JFD_OrganizedCrimes[organizedCrimeID]
					organizedCrimeIcon = organizedCrime.IconString
					organizedCrimeDesc = Locale.ToUpper(organizedCrime.Description)
					Controls.CrimeIcon:SetHide(false)
					Controls.CrimeIcon:SetText(organizedCrime.IconString)
					Controls.CrimeIcon:LocalizeAndSetToolTip(organizedCrime.Help)
					Controls.OrganizedCrimeLabel:SetHide(false)
				else
					Controls.CrimeIcon:SetHide(true)
					Controls.OrganizedCrimeLabel:SetHide(true)
				end
				Controls.OrganizedCrimeLabel:LocalizeAndSetText("TXT_KEY_JFD_CITYVIEW_CRIME_ORGANIZED_TEXT", organizedCrimeIcon, organizedCrimeDesc)
				Controls.OrganizedCrimeLabel:LocalizeAndSetToolTip(crimeOrganizedBreakdownTT)
				Controls.OrganizedCrimeThreshold:SetText(thresholdColour .. threshold .. "%[ENDCOLOR]")
				Controls.OrganizedCrimeThreshold:LocalizeAndSetToolTip(crimeOrganizedBreakdownTT)
				...
				Controls.OrganizedCrimeLabel:SetHide(true)
				Controls.OrganizedCrimeThreshold:SetHide(true)
			end
		end
		--Happiness Status
		if userSettingHappinessCore then
			local excessHappiness, excessUnhappiness = JFD_GetExcessHappiness(iPlayer, pCity, true)
			if excessHappiness >= excessUnhappiness then
				...
				Controls.HappinessPerTurnLabel:SetText( Locale.ConvertTextKey("TXT_KEY_CITYVIEW_HAPPINESS_PERTURN_TEXT", excessHappiness, "[COLOR_JFD_HAPPINESS]") );
				local resistanceCounter, resistanceThreshold = JFD_GetResistanceCounter(iPlayer, pCity, true)
				Controls.ResistanceCounter:LocalizeAndSetText("[COLOR_JFD_HAPPINESS]{1_Num}/{2_Num}[ENDCOLOR]", resistanceCounter, resistanceThreshold)
			else
				...
				Controls.HappinessPerTurnLabel:SetText( Locale.ConvertTextKey("TXT_KEY_CITYVIEW_HAPPINESS_PERTURN_TEXT", excessUnhappiness, "[COLOR_JFD_UNHAPPINESS]") );
				local resistanceCounter, resistanceThreshold = JFD_GetResistanceCounter(iPlayer, pCity, true)
				Controls.ResistanceCounter:LocalizeAndSetText("[COLOR_JFD_UNHAPPINESS]{1_Num}/{2_Num}[ENDCOLOR]", resistanceCounter, resistanceThreshold)
			end
			if pCity:GetResistanceTurns() > 0 then
				Controls.HappinessIcon:SetText("[ICON_RESISTANCE]")
				Controls.HappinessLabel:SetText("[COLOR_JFD_UNHAPPINESS]" .. Locale.ConvertTextKey("TXT_KEY_JFD_CITYVIEW_RESISTANCE_TEXT") .. "[ENDCOLOR]")
			end
			...
			local excessHappiness = pCity:GetLocalHappiness()
		...
		if userSettingHealthCore then
			local excessHealth, excessDisease = JFD_GetExcessHealth(iPlayer, pCity, true)
			Controls.HealthBox:SetHide(false)
			Controls.HealthBox:SetToolTipString(JFD_GetHealthTT(iPlayer, pCity))
			if excessHealth >= excessDisease then
				...
				local plagueCounter, plagueThreshold, plagueTurns = JFD_GetPlagueCounter(iPlayer, pCity, true)
				Controls.PlagueCounter:LocalizeAndSetText("[COLOR_JFD_HEALTH]{1_Num}/{2_Num}[ENDCOLOR]", plagueCounter, plagueThreshold)
			else
				...
				local plagueCounter, plagueThreshold, plagueTurns = JFD_GetPlagueCounter(iPlayer, pCity, true)
				Controls.PlagueCounter:LocalizeAndSetText("[COLOR_JFD_DISEASE]{1_Num}/{2_Num}[ENDCOLOR]", plagueCounter, plagueThreshold)
			end
			Controls.HealthBox:SetHide(false)
			if JFD_IsCityPlagued(pCity) then
				local plagueID = JFD_GetCityPlagueID(pCity)
				local plague = GameInfo.JFD_Plagues[plagueID]
				local numPlagueTurns = JFD_GetNumPlagueTurns(pCity)
				...
				Controls.PlagueIcon:LocalizeAndSetToolTip("TXT_KEY_JFD_CITYBANNER_PLAGUE_TEXT_TT", plague.IconString, plague.Description, numPlagueTurns)
				Controls.PlagueLabel:SetHide(false)
				Controls.PlagueLabel:LocalizeAndSetText("[COLOR_JFD_PLAGUE]{1_Icon}{2_Desc}[ENDCOLOR]", plague.IconString, plague.Description)
				Controls.PlagueCounter:LocalizeAndSetText("TXT_KEY_JFD_CITYVIEW_PLAGUED_TURNS_TEXT", numPlagueTurns)
			else
		...
		--Loyalty Status
		local showLoyaltyStatus = false
		if userSettingLoyaltyCore then
			local loyaltyID, loyaltyPercent, loyaltyTT = JFD_CalculateCityLoyalty(iPlayer, pCity, true)
			if loyaltyID > -1 then
				...
				Controls.CityLoyaltyTypeLabel:LocalizeAndSetToolTip("TXT_KEY_JFD_CITYVIEW_LOYALTY_TT", loyaltyTT, loyalty.IconString, loyalty.TextColour, loyaltyPercent)
				...
				local revoltCounter, revoltThreshold, revoltRate, revoltTurns, WLTKDCounter, WLTKDThreshold, WLTKDRate, WLTKDTurns = JFD_GetLoyaltyCounter(iPlayer, pCity, loyaltyID, true)
				if revoltRate > 0 then revoltRate = "+" .. revoltRate end
				if WLTKDRate  > 0 then WLTKDRate  = "+" .. WLTKDRate  end
				if loyaltyID == GameInfoTypes["LOYALTY_JFD_PATRIOTIC"] then
					...
					Controls.RevoltLabel:LocalizeAndSetToolTip("TXT_KEY_JFD_CITYVIEW_LOYALTY_WLTKD_TT", WLTKDTurns, revoltCounter, revoltThreshold, revoltRate, WLTKDCounter, WLTKDThreshold, WLTKDRate)
					Controls.RevoltMeter:LocalizeAndSetToolTip("TXT_KEY_JFD_CITYVIEW_LOYALTY_WLTKD_TT", WLTKDTurns, revoltCounter, revoltThreshold, revoltRate, WLTKDCounter, WLTKDThreshold, WLTKDRate)
					Controls.RevoltMeter:SetHide(false)
				elseif loyaltyID == GameInfoTypes["LOYALTY_JFD_ALLEGIANT"] then
					Controls.RevoltMeter:SetPercent(WLTKDCounter/WLTKDThreshold)
					Controls.RevoltMeter:LocalizeAndSetToolTip(WLTKDTurns)
					...
					Controls.RevoltMeter:LocalizeAndSetToolTip("TXT_KEY_JFD_CITYVIEW_LOYALTY_WLTKD_TT", WLTKDTurns, revoltCounter, revoltThreshold, revoltRate, WLTKDCounter, WLTKDThreshold, WLTKDRate)
					Controls.RevoltMeter:SetHide(false)
				elseif loyaltyID == GameInfoTypes["LOYALTY_JFD_NEUTRAL"] then
					Controls.RevoltLabel:LocalizeAndSetText("TXT_KEY_JFD_CITYVIEW_TURNS", 0)
					Controls.RevoltMeter:SetPercent(0)Controls.RevoltLabel:LocalizeAndSetToolTip("TXT_KEY_JFD_CITYVIEW_LOYALTY_NEUTRAL_TT", revoltCounter, revoltThreshold, revoltRate, WLTKDCounter, WLTKDThreshold, WLTKDRate)
					Controls.RevoltMeter:LocalizeAndSetToolTip("TXT_KEY_JFD_CITYVIEW_LOYALTY_NEUTRAL_TT", revoltCounter, revoltThreshold, revoltRate, WLTKDCounter, WLTKDThreshold, WLTKDRate)
					Controls.RevoltMeter:SetHide(false)
				elseif loyaltyID == GameInfoTypes["LOYALTY_JFD_REBELLIOUS"] then
					...
					Controls.RevoltLabel:LocalizeAndSetToolTip("TXT_KEY_JFD_CITYVIEW_LOYALTY_REVOLT_TT", revoltTurns, revoltCounter, revoltThreshold, revoltRate, WLTKDCounter, WLTKDThreshold, WLTKDRate)
					Controls.RevoltMeter:LocalizeAndSetToolTip("TXT_KEY_JFD_CITYVIEW_LOYALTY_REVOLT_TT", revoltTurns, revoltCounter, revoltThreshold, revoltRate, WLTKDCounter, WLTKDThreshold, WLTKDRate)
					Controls.RevoltMeter:SetHide(false)
				elseif loyaltyID == GameInfoTypes["LOYALTY_JFD_SEPARATIST"] then
					Controls.RevoltMeter:SetPercent(revoltCounter/revoltThreshold)
					Controls.RevoltMeter:LocalizeAndSetToolTip(revoltTurns)
					Controls.RevoltMeter:SetTexture("MeterBarRevoltSeparatist.dds")
					Controls.RevoltLabel:LocalizeAndSetText("TXT_KEY_JFD_CITYVIEW_TURNS", revoltTurns)
					Controls.RevoltLabel:LocalizeAndSetToolTip("TXT_KEY_JFD_CITYVIEW_LOYALTY_REVOLT_SEPARATIST_TT", revoltTurns, revoltCounter, revoltThreshold, revoltRate, WLTKDCounter, WLTKDThreshold, WLTKDRate)
					Controls.RevoltMeter:LocalizeAndSetToolTip("TXT_KEY_JFD_CITYVIEW_LOYALTY_REVOLT_SEPARATIST_TT", revoltTurns, revoltCounter, revoltThreshold, revoltRate, WLTKDCounter, WLTKDThreshold, WLTKDRate)
					Controls.RevoltMeter:SetHide(false)
				else
		...
		-- EMIGRATION MOD --
		-- prosperity
		local prosp = GetProsperity(pCity);
		local locstr = "TXT_KEY_CITYVIEW_PERTURN_TEXT";
		if prosp < 0 then
			locstr = "TXT_KEY_CITYVIEW_PERTURN_TEXT_NEGATIVE";
		end
		Controls.ProsperityLabel:SetText( Locale.ConvertTextKey(locstr, prosp) );
		
		-- migration
		local info = GetMigrationInfo(pCity);
		if info.turns == nil then
			print("turns is nil, hide migration label");
			Controls.MigrationBox:SetHide(true);
		else
			print("show migration label");
			if info.ebar == -1 then
				print("immigration in " .. info.turns .. " turns");
				Controls.MigrationLabel:SetText( Locale.ConvertTextKey("TXT_KEY_CITYVIEW_IMMIGRATION_TURNS", info.turns) );
			else
				print("emigration in " .. info.turns .. " turns");
				Controls.MigrationLabel:SetText( Locale.ConvertTextKey("TXT_KEY_CITYVIEW_EMIGRATION_TURNS", info.turns) );
			end
			CivIconHookup(info.pair:GetOwner(), 32, Controls.EmCivIcon, Controls.EmCivIconBG, Controls.EmCivIconShadow, false, true, Controls.EmCivIconHighlight);
			Controls.MigrationBox:SetHide(false);
		end
		--------------------
		...
		if (UI.IsCityScreenViewingMode()) then
			...
			if not JFD_IsAllowedPuppetPurchase(pCity) then
				Controls.BuyPlotButton:SetDisabled( true );
...
function DoUpdateProductionInfo( bNoProduction )
	...
	if userSettingHappinessCore and (not pCity:IsProductionProcess()) then
		local excessHappiness = JFD_GetExcessHappiness(pCity:GetOwner(), pCity)
		local strProductionPerTurn = Locale.ConvertTextKey("TXT_KEY_CITY_SCREEN_PROD_PER_TURN", iProductionPerTurn) .. " [COLOR_WHITE] +[ENDCOLOR] [COLOR_JFD_HAPPINESS]" .. Locale.ConvertTextKey("TXT_KEY_CITY_SCREEN_JFD_HAPPINESS_PER_TURN", excessHappiness) .. "[ENDCOLOR]"
	...
	-- Turns left
	local productionTurnsLeft = 0;
	if userSettingHappinessCore and (not pCity:IsProductionProcess()) then
		productionTurnsLeft = JFD_GetProductionTurnsLeft(pCity:GetOwner(), pCity)
	else
		productionTurnsLeft = pCity:GetProductionTurnsLeft()
	end
...
-- CBP
	if isUsingCBP then
		...
		local happinessBreakdownTT = JFD_GetHappinessTT(pCity:GetOwner(), pCity)
		Controls.HappinessBox:LocalizeAndSetToolTip( happinessBreakdownTT );
	end
-- END
		-- EMIGRATION MOD --
	Controls.ProsperityBox:SetToolTipString(GetProsperityTooltip(pCity));
	Controls.MigrationBox:SetToolTipString(GetMigrationTooltip(pCity));
...
function JFD_ConfirmAssimilateYes()
	...
	local city = UI.GetHeadSelectedCity()
	if (city and JFD_IsCityColony(city)) then
...
function JFD_OnAssimilateButton()
	...
	if Players[playerID]:IsTurnActive() then
		...
		Controls.AssimilateLabelConfirm:LocalizeAndSetText("TXT_KEY_CITYVIEW_JFD_ASSIMILATE_BUTTON_TEXT_CONFIRM", goldCost)
	end
end
...
---------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------
function JFD_UpdateInvestmentInfo(pPlayer, pCity)
	local pPlayer = Players[Game.GetActivePlayer()]
	local pCity = pCity or UI.GetHeadSelectedCity()
	if pCity and userSettingInvestmentsCoreUI then
		local buildingProduction = pCity:GetProductionBuilding();
		local unitProduction = pCity:GetProductionUnit();
		if buildingProduction ~= -1 then
			local building = GameInfo.Buildings[buildingProduction]
			local buildingClass = GameInfo.BuildingClasses[building.BuildingClass]
			local investmentAmount = JFD_GetInvestmentValue(Game.GetActivePlayer(), false, (buildingClass.MaxGlobalInstances >= 1 or buildingClass.MaxPlayerInstances >= 1))
			if pCity:GetBuildingInvestment(buildingProduction) > 0 then
				...
				local numGold = pCity:GetBuildingPurchaseCost( buildingProduction )
				local investmentString = Locale.ConvertTextKey("TXT_KEY_CITYVIEW_QUEUE_INVEST", numGold)
				if pPlayer:GetGold() < numGold then
				...
				Controls.InvestButton:LocalizeAndSetToolTip("TXT_KEY_CITYVIEW_QUEUE_INVEST_TT", numGold, building.Description, investmentAmount)
				Controls.InvestButton:SetVoid1(buildingProduction)
				Controls.InvestButton:RegisterCallback(Mouse.eLClick, JFD_MakeInvestmentBuilding)
			end
		elseif (userSettingInvestmentsCoreUnits and unitProduction ~= -1) then
			local unit = GameInfo.Units[unitProduction]
			local unitClass = GameInfo.UnitClasses[unit.Class]
			local investmentAmount = JFD_GetInvestmentValue(Game.GetActivePlayer(), false, false, true)
			if pCity:GetUnitInvestment(unitProduction) > 0 then
				Controls.InvestButton:SetHide(false)
				Controls.InvestButton:SetDisabled(true)
				Controls.InvestButton:LocalizeAndSetText("TXT_KEY_CITYVIEW_QUEUE_INVESTED")
				Controls.InvestButton:LocalizeAndSetToolTip("TXT_KEY_CITYVIEW_QUEUE_INVESTED_TT", unit.Description, investmentAmount)
			else
				local numGold = pCity:GetUnitPurchaseCost( unitProduction )
				local investmentString = Locale.ConvertTextKey("TXT_KEY_CITYVIEW_QUEUE_INVEST", numGold)
				if pPlayer:GetGold() < numGold then
					investmentString = "[COLOR_NEGATIVE_TEXT]" .. investmentString .. "[ENDCOLOR]"
					Controls.InvestButton:SetDisabled(true)
				...
				Controls.InvestButton:SetHide(false)
				Controls.InvestButton:SetText(investmentString)
				Controls.InvestButton:LocalizeAndSetToolTip("TXT_KEY_CITYVIEW_QUEUE_INVEST_TT", numGold, unit.Description, investmentAmount)
				Controls.InvestButton:SetVoid1(unitProduction)
				Controls.InvestButton:RegisterCallback(Mouse.eLClick, JFD_MakeInvestmentUnit)
			end
		else
...
Events.SerialEventGameDataDirty.Add( JFD_UpdateInvestmentInfo );

function JFD_MakeInvestmentBuilding(buildingID)
	local city = UI.GetHeadSelectedCity()
	Game.CityPurchaseBuilding(city, buildingID, YieldTypes.YIELD_GOLD)
	Events.AudioPlay2DSound("AS2D_INTERFACE_CITY_SCREEN_PURCHASE");
end

function JFD_MakeInvestmentUnit(unitID)
	local city = UI.GetHeadSelectedCity()
