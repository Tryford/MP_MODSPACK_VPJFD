		...
		function (tInstance, iMinor, iActivePlayer, tInfo)
			...
			if (pMinor:GetAlly() ~= iActivePlayer or not(pPlayer:GetCapitalCity())) then
				...
				for pCity in pPlayer:Cities() do
					Suk_Gobekli_GenerateInteraction(pCity, iCost)
				end
...
function Suk_Gobekli_GenerateInteraction(pCity, iCost)
	...
	local tInteraction = {
		OnShow =
			function (tInstance, iMinor, iActivePlayer, tInfo)
				local pPlayer = Players[iActivePlayer]
				local sLabel = L("TXT_KEY_SUZERAINTRAIT_MINOR_CIV_SUK_GOBEKLI_TEPE_BUY", sCity, iCost)

				if (pPlayer:GetFaith() < iCost) then
					sLabel = "[COLOR_WARNING_TEXT]" .. sLabel .. "[ENDCOLOR]"
				end
		...
		OnClick =
			function (tInstance, iMinor, iActivePlayer, tInfo)
				...
				local pPlayer = Players[iActivePlayer]
				if not(pPlayer:GetFaith() < iCost) then
