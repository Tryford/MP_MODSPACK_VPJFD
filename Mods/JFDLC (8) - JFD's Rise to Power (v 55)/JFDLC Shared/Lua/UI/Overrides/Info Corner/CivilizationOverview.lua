...
local userSettingRTPSovereigntyCore = true--JFD_GetUserSetting("JFD_RTP_SOVEREIGNTY_CORE") == 1
...
function OnCivPanelUpdated()
	...
	if civilization then
		...
		Controls.LeaderIcon:LocalizeAndSetToolTip(activePlayer:GetLeaderTitle())
		Controls.CivIcon:LocalizeAndSetToolTip(activePlayer:GetCivilizationShortDescription())
		
		--Update civ description
		-- local leaderTitle = activePlayer:GetLeaderTitle()
		...
		Controls.LeaderText:LocalizeAndSetToolTip(activePlayer:GetLeaderTitle());
		...
		if currentGovernmentID > -1 then
			currentPoliticID = activePlayer:GetDominantPolitic()
			...
			if activePlayer:HasReform(reformExecutiveAbsoluteID) then
				powerDesc = "(Absolute)"
			elseif activePlayer:HasReform(reformExecutiveConstitutionalID) then
			...
			if currentGovernmentID == governmentTheocracyID then
				politicDesc = Locale.ConvertTextKey("Dominant: [COLOR_JFD_SOVEREIGNTY_FADING] {2_FontIcon}{1_PoliticDesc}[ENDCOLOR]", activePlayer:GetPoliticDescription(currentReligionID, true), "[ICON_JFD_CLERGY]")
			...
			elseif currentPoliticID and currentPoliticID > -1 then
				...
				politicDesc = Locale.ConvertTextKey("Dominant: [COLOR_JFD_SOVEREIGNTY_FADING] {3_FontIcon} {1_PoliticDesc} ({2_ShortDesc})[ENDCOLOR]", activePlayer:GetPoliticDescription(currentPoliticID), politicInfo.ShortDescription, politicInfo.FontIcon)
				Controls.Politics:SetHide(false)
				Controls.Politics:LocalizeAndSetText(politicDesc)
			end
		end
		...
		--Update religion
		local currentReligionID = 0--JFD_GetMainReligionID(activePlayerID)
		...
		if Controls.ReligionIcon ~= nil then
