--[[ BUGFIX Notes: by Mavoc
	All code changes will be marked with BUGFIX and ENDFIX for ease of finding changes
	The Unit Supply section will now take into account that trade units don't consume supplies
		From my testing the "Trade" flag on units is what makes a unit not consume supplies
		If there are non-trade units that also do not consume supplies, then I am unaware of them
		I was unable to find a simple call to get the number of consumed supplies
		So I am counting the trade units as the unit list is processed and subtracting it from the total unit count
	The trade units will now display "Trade (Automated)" instead of "Explore (Automated)"
		This was just an oversight as the UnitList.lua has the same block of code but with support for trade units
--]]
...
function BuildUnitList()
	...
	--BUGFIX: initing a trade unit counter
	local iNumTradeUnits = 0;
	--ENDFIX
    ...
    do
        ...
        local isMercenary = unit:IsContractUnit()
        ...
        if( unit:GetUnitCombatType() ~= -1 or unit:CanNuke()) then
            ...
            bFoundCivilian = true;
			--BUGFIX: increment the counter for each trade unit, doing it here as there is no need to check military units
			if unit:IsTrade() then
				iNumTradeUnits = iNumTradeUnits + 1;
			end
			--ENDFIX
        end
        ...
        if isMercenary then
			sortEntry.DisplayName = "[ICON_INVEST] " .. Locale.Lookup(unit:GetNameKey());
        end
        ...
        elseif( unit:IsAutomated()) then
			if(unit:IsWork()) then
				...
				instance.Status:SetHide( false );
			--BUGFIX: trade units should show trade and not explore, this code was added to unitlist.lua but not here for some reason
