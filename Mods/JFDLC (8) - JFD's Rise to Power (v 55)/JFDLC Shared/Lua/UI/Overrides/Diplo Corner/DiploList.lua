...
function UpdateDisplay()
	...
	local civDesc, civTT, leaderDesc, leaderTT = Players[g_iPlayer]:GetDiploListInfo()
	...
	--	TruncateString(Controls.LeaderName, textBoxSize, Locale.ConvertTextKey( PreGame.GetLeaderName( g_iPlayer ) ), "  (" .. Locale.ConvertTextKey( "TXT_KEY_YOU" ) .. ")");
		...
		TruncateString(Controls.LeaderName, textBoxSize, Locale.ConvertTextKey( leaderDesc ), "  (" .. Locale.ConvertTextKey( "TXT_KEY_YOU" ) .. ")");
	end
	...
	--else
		TruncateString(Controls.CivName, textBoxSize, civDesc);
	--end
	Controls.CivName:SetToolTipString(civTT)
	Controls.LeaderName:SetToolTipString(leaderTT)
	...
	for iPlayerLoop = 0, GameDefines.MAX_MAJOR_CIVS-1, 1 do
		...
		if (iPlayerLoop ~= g_iPlayer and pOtherPlayer:IsAlive()) then
			...
			if (g_pTeam:IsHasMet(iOtherTeam)) then
				...
				local theirLeaderType = pOtherPlayer:GetLeaderType();
				local theirLeaderInfo = GameInfo.Leaders[theirLeaderType];
				...
				local civDesc, civTT, leaderDesc, leaderTT = pOtherPlayer:GetDiploListInfo()
				...
				--else
					controlTable.LeaderName:LocalizeAndSetText( leaderDesc );
					controlTable.LeaderName:SetToolTipString( leaderTT );
				--end
				...
				controlTable.CivName:LocalizeAndSetText(civDesc);
				controlTable.CivName:SetToolTipString(civTT);
				...
				-- embassy + open borders
				if (g_pPlayer:HasEmbassyWith(iPlayerLoop) and pOtherPlayer:GetCapitalCity()) then
					controlTable.EmbassyLabel:SetHide( false );
					controlTable.EmbassyLabel:LocalizeAndSetText( "TXT_KEY_JFD_DIPLO_EMBASSY", pOtherPlayer:GetCapitalCity():GetName() );
				else
					controlTable.EmbassyLabel:SetHide( true );
				end
				if g_pPlayer:IsPlayerHasOpenBorders(iPlayerLoop) then
					controlTable.OpenBordersLabel:SetHide( false );
				else
					controlTable.OpenBordersLabel:SetHide( true );
				end
...
-----------------------------------------------------------------
-----------------------------------------------------------------
function OnVassalsButton()
	local popupInfo = {
        Type = ButtonPopupTypes.BUTTONPOPUP_MODDER_11,
    }
    Events.SerialEventGameMessagePopup(popupInfo);
end
if (not Game.IsCIVDiploActive()) then
	Controls.VassalOverviewButton:SetHide(true)
else
	Controls.VassalOverviewButton:SetHide(false)
	Controls.VassalOverviewButton:RegisterCallback( Mouse.eLClick, OnVassalsButton );
end
