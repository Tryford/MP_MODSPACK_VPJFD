...
if (Game.GetUserSetting("JFD_RTP_SOVEREIGNTY_CORE") == 0 or (not Game.GetUserSetting("JFD_RTP_SOVEREIGNTY_CORE"))) then
	Controls.GovernmentOverviewButton:SetHide(true)
end

--Hide Mercenaries Overview, if disabled.
if (Game.GetUserSetting("JFD_RTP_MERCENARIES_CORE") == 0 or (not Game.GetUserSetting("JFD_RTP_MERCENARIES_CORE"))) then
...
function RefreshYields()
	...
	if Game.GetUserSetting("JFD_RTP_PROSPERITY_CORE") == 1 then
	...
	if Game.GetUserSetting("JFD_RTP_SOVEREIGNTY_CORE") == 1 then
	...
	if Game.GetUserSetting("JFD_RTP_PIETY_CORE") == 1 then
...
function OnOpenTopPanelOptions()
	...
	if Game.GetUserSetting("JFD_RTP_PIETY_CORE") == 0 or (not activePlayer:HasStateReligion()) then
	...
	if Game.GetUserSetting("JFD_RTP_PROSPERITY_CORE") == 0 or (not activePlayer:HasCurrency()) then
	...
	if Game.GetUserSetting("JFD_RTP_SOVEREIGNTY_CORE") == 0 or (not activePlayer:HasGovernment()) then
