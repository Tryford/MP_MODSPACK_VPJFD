...
local m_DignitaryIM = InstanceManager:new( "GPInstance", "Root", Controls.DignitaryStack ); -- JFDLC
local m_DiplomatIM = InstanceManager:new( "GPInstance", "Root", Controls.DiplomatStack ); -- Gazebo CSD
local m_DirectorIM = InstanceManager:new( "GPInstance", "Root", Controls.DirectorStack ); -- JFDLC
local m_DoctorIM = InstanceManager:new( "GPInstance", "Root", Controls.DoctorStack ); -- JFDLC
local m_EngineerIM = InstanceManager:new( "GPInstance", "Root", Controls.EngineerStack );
local m_MagistrateIM = InstanceManager:new( "GPInstance", "Root", Controls.MagistrateStack ); -- JFDLC
...
local m_ProphetIM = InstanceManager:new( "GPInstance", "Root", Controls.ProphetStack ); -- JFDLC
...
function UpdateDisplay()
	...
	local bDignitaryFound = false; -- JFDLC
	local bDiplomatFound = false; -- Gazebo CSD
	local bDirectorFound = false; -- JFDLC
	local bDoctorFound = false; -- JFDLC
	local bEngineerFound = false;
	local bMagistrateFound = false; -- JFDLC
	local bMerchantFound = false;
	local bMusicianFound = false; 
	local bProphetFound = false; -- JFDLC
	...
	m_DignitaryIM:ResetInstances(); -- JFDLC
	m_DiplomatIM:ResetInstances(); -- Gazebo CSD
	m_DirectorIM:ResetInstances(); -- JFDLC
	m_DoctorIM:ResetInstances(); -- JFDLC
	m_EngineerIM:ResetInstances();
	m_MagistrateIM:ResetInstances(); -- JFDLC
	m_MerchantIM:ResetInstances();
	m_MusicianIM:ResetInstances();
	m_ScientistIM:ResetInstances();
	m_ProphetIM:ResetInstances(); -- JFDLC
			...
			for pCity in pPlayer:Cities() do
				if (checkIfSpecialist(pPlayer, pSpecialistInfo, pCity)) then
					...
					elseif (pSpecialistInfo.Type == "SPECIALIST_CIVIL_SERVANT") then -- Gazebo CSD
						...
						bDiplomatFound = true;
					elseif (pSpecialistInfo.Type == "SPECIALIST_JFD_DIGNITARY") then -- JFDLC
						instance = m_DignitaryIM:GetInstance();
						bDignitaryFound = true;
					elseif (pSpecialistInfo.Type == "SPECIALIST_JFD_DIRECTOR") then -- JFDLC
						instance = m_DirectorIM:GetInstance();
						bDirectorFound = true;
					elseif (pSpecialistInfo.Type == "SPECIALIST_JFD_DOCTOR") then -- JFDLC
						instance = m_DoctorIM:GetInstance();
						bDoctorFound = true;
					elseif (pSpecialistInfo.Type == "SPECIALIST_ENGINEER") then
						instance = m_EngineerIM:GetInstance();
						bEngineerFound = true;
					elseif (pSpecialistInfo.Type == "SPECIALIST_JFD_MAGISTRATE") then -- JFDLC
					...
					elseif (pSpecialistInfo.Type == "SPECIALIST_MERCHANT") then
						...
						bMerchantFound = true;
					elseif (pSpecialistInfo.Type == "SPECIALIST_JFD_MONK") then -- JFDLC
	...
	Controls.DignitaryStack:SortChildren( SortFunction ); -- JFDLC
	Controls.DiplomatStack:SortChildren( SortFunction ); -- Gazebo CSD
	Controls.DirectorStack:SortChildren( SortFunction ); -- JFDLC
	Controls.DoctorStack:SortChildren( SortFunction ); -- JFDLC
	Controls.EngineerStack:SortChildren( SortFunction );
	Controls.MagistrateStack:SortChildren( SortFunction ); -- JFDLC
	...
	Controls.ProphetStack:SortChildren( SortFunction ); -- JFDLC
	Controls.WriterStack:SortChildren( SortFunction );

	Controls.ArtistHeader:SetHide( not bArtistFound );
	Controls.DignitaryHeader:SetHide( not bDignitaryFound ); -- JFDLC
	Controls.DiplomatHeader:SetHide( not bDiplomatFound ); -- Gazebo CSD
	Controls.DirectorHeader:SetHide( not bDirectorFound ); -- JFDLC
	Controls.DoctorHeader:SetHide( not bDoctorFound ); -- JFDLC
	Controls.EngineerHeader:SetHide( not bEngineerFound );
	Controls.MagistrateHeader:SetHide( not bMagistrateFound ); -- JFDLC
	...
	Controls.ProphetHeader:SetHide( not bProphetFound ); -- JFDLC
	Controls.WriterHeader:SetHide( not bWriterFound );

	Controls.ArtistStack:CalculateSize();
	Controls.ArtistStack:ReprocessAnchoring();
	Controls.DignitaryStack:CalculateSize();  -- JFDLC
	Controls.DignitaryStack:ReprocessAnchoring();  -- JFDLC
	Controls.DiplomatStack:CalculateSize();  -- Gazebo CSD
	Controls.DiplomatStack:ReprocessAnchoring();  -- Gazebo CSD
	Controls.DirectorStack:CalculateSize();  -- JFDLC
	Controls.DirectorStack:ReprocessAnchoring();  -- JFDLC
	Controls.DoctorStack:CalculateSize();  -- JFDLC
	Controls.DoctorStack:ReprocessAnchoring();  -- JFDLC
	Controls.EngineerStack:CalculateSize();
	Controls.EngineerStack:ReprocessAnchoring();
	Controls.MagistrateStack:CalculateSize(); -- JFDLC
	Controls.MagistrateStack:ReprocessAnchoring(); -- JFDLC
	...
	Controls.ProphetStack:CalculateSize(); -- JFDLC
	Controls.ProphetStack:ReprocessAnchoring(); -- JFDLC
...
-- JFDLC
-------------------------------------------------
function OnDirectorToggle()
	local bWasHidden = Controls.DirectorStack:IsHidden();
	Controls.DirectorStack:SetHide( not bWasHidden );
	if( bWasHidden ) then
		Controls.DirectorToggle:LocalizeAndSetText("TXT_KEY_GP_DIRECTOR_DETAILS_COLLAPSE");
	else
		Controls.DirectorToggle:LocalizeAndSetText("TXT_KEY_GP_DIRECTOR_DETAILS");
	end
	Controls.MainStack:CalculateSize();
	Controls.MainStack:ReprocessAnchoring();
	Controls.ScrollPanel:CalculateInternalSize();
...
Controls.DirectorToggle:RegisterCallback( Mouse.eLClick, OnDirectorToggle );
-------------------------------------------------
-- JFDLC
-------------------------------------------------
