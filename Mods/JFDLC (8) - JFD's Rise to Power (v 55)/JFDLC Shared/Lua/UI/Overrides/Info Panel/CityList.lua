...
function UpdateDisplay()
    ...
    for pCity in pPlayer:Cities() do
		...
		-- JFD Crime
		if userSettingCIDCrimeCore then
			sortEntry.Crime = math.floor( pPlayer:GetCityCrimeTotal(pCity) );
			instance.Crime:SetText( sortEntry.Crime );
			instance.Crime:SetHide(false)
			instance.Crime:SetToolTipString(pPlayer:GetCityCrimeTT(pCity, true, true))
...
-- END   
		...
		elseif(pCity:IsPuppet() and (not pCity:IsColony())) then
			instance.IconCapital:SetText("[ICON_PUPPET]");
			instance.IconCapital:SetHide(false);
		elseif(pCity:IsPuppet() and pCity:IsColony()) then
