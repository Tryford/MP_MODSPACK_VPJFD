...
local m_DecisionIM = InstanceManager:new( "ResourceInstance", "Root", Controls.DecisionStack );
...
function UpdateDisplay()
	...
	for resource in GameInfo.Resources() do
		...
		if( iTotal > 0 or iExports > 0 ) then
			...
			if( resource.ResourceClassType == "RESOURCECLASS_JFD_DECISION" ) then
				instance = m_DecisionIM:GetInstance();
				bDecisionFound = true;
			end
	...
	Controls.DecisionStack:SortChildren( SortFunction );
	...
	Controls.DecisionHeader:SetHide( not bDecisionFound );
	...
	Controls.DecisionStack:CalculateSize();
	Controls.DecisionStack:ReprocessAnchoring();
...
function OnDecisionToggle()
	local bWasHidden = Controls.DecisionStack:IsHidden();
	Controls.DecisionStack:SetHide( not bWasHidden );
	if( bWasHidden ) then
		Controls.DecisionToggle:LocalizeAndSetText("TXT_KEY_RESOURCE_DECISION_DETAILS_COLLAPSE");
	else
		Controls.DecisionToggle:LocalizeAndSetText("TXT_KEY_RESOURCE_DECISION_DETAILS");
	end
	Controls.MainStack:CalculateSize();
	Controls.MainStack:ReprocessAnchoring();
	Controls.ScrollPanel:CalculateInternalSize();
end
Controls.DecisionToggle:RegisterCallback( Mouse.eLClick, OnDecisionToggle );
