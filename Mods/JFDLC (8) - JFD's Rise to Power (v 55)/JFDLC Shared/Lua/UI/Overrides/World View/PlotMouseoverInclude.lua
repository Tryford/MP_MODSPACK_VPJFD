...
function GetImprovementString(plot)
	...
	if (iImprovementType >= 0) then
		...
		--JFD Upgrade Time
		if pPlayer and plot:GetUpgradeTimeLeft(iImprovementType, pPlayer:GetID()) > 1 then
			improvementStr = improvementStr .." " .. Locale.ConvertTextKey("TXT_KEY_JFD_PLOT_HELP_TURNS_TIL_UPGRADE", plot:GetUpgradeTimeLeft(iImprovementType, pPlayer:GetID()))
		end
...
function GetYieldString(plot)
	...
	-- Tourism
	local iNumTourism = plot:CalculateYield(GameInfoTypes["YIELD_TOURISM"], true);
	if (iNumTourism > 0) then
		strYield = strYield .. "[ICON_TOURISM] " .. iNumTourism .. " ";
	end

	-- Golden Age Points	
	local iNumGoldenAgePoints = plot:CalculateYield(GameInfoTypes["YIELD_GOLDEN_AGE_POINTS"], true);
	if (iNumGoldenAgePoints > 0) then
		strYield = strYield .. "[ICON_GOLDEN_AGE] " .. iNumGoldenAgePoints .. " ";
	end

	-- Great Admiral Points
	local iNumAdmiralPoints = plot:CalculateYield(GameInfoTypes["YIELD_GREAT_ADMIRAL_POINTS"], true);
	if (iNumAdmiralPoints > 0) then
		strYield = strYield .. "[ICON_GREAT_ADMIRAL] " .. iNumAdmiralPoints .. " ";
	end

	-- Great General Points
	local iNumGeneralPoints = plot:CalculateYield(GameInfoTypes["YIELD_GREAT_GENERAL_POINTS"], true);
	if (iNumGeneralPoints > 0) then
		strYield = strYield .. "[ICON_GREAT_GENERAL] " .. iNumGeneralPoints .. " ";
	end
	
	-- JFD Health
	local iNumHealth = plot:CalculateYield(GameInfoTypes["YIELD_JFD_HEALTH"], true);
	if (iNumHealth > 0) then
		strYield = strYield .. "[ICON_JFD_HEALTH] " .. iNumHealth .. " ";
	end

	-- JFD Disease
	local iNumDisease = plot:CalculateYield(GameInfoTypes["YIELD_JFD_DISEASE"], true);
	if (iNumDisease > 0) then
		strYield = strYield .. "[ICON_JFD_DISEASE] " .. iNumDisease .. " ";
	end

	if (not plot:IsImprovementPillaged()) then
		for yield in GameInfo.Yields("IsFake = 1") do
			local yieldID = yield.ID
			local amount = Players[Game.GetActivePlayer()]:CalculateFakeYield(yieldID, plot) --* 0 + math.random( 0, 3 ) * math.random( 0, 4 )
			if amount > 0 then
				strYield = strYield .. yield.IconString .. " " .. amount .. " ";
			end
		end
	end
