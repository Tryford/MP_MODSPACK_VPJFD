...
include( "JFDLCTopPanelUtils.lua" )
...
local userSettingMiscClock			= (Game.GetUserSetting("JFDLC_MISC_TOP_PANEL_CLOCK") == 1)
...
function UpdateData()
	...
	if( iPlayerID >= 0 ) then
		...
		if (pPlayer:GetNumCities() > 0) then
			...
			local strInstantYields = "[ICON_YIELDS]";
			...
			if iGoldPerTurn < 0 then
				strGoldStr = Locale.ConvertTextKey("TXT_KEY_TOP_PANEL_GOLD_NEGATIVE", iTotalGold, iGoldPerTurn);
			end
			--end
			...
			if userSettingRTPProsperityCore then
				local strProsperityStr = Locale.ConvertTextKey("TXT_KEY_TOP_PANEL_JFD_PROSPERITY", pPlayer:GetWeightedProsperityScore())
			...
			if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_HAPPINESS)) then
				...
				if (not pPlayer:IsEmpireUnhappy()) then
					strHappiness = string.format("[ICON_HAPPINESS_1][COLOR_HAPPINESS]%i[/COLOR]", iHappiness);
				
				-- Empire Really Unhappy
				elseif (pPlayer:IsEmpireVeryUnhappy()) then
					strHappiness = string.format("[ICON_HAPPINESS_4][COLOR_UNHAPPINESS]%i[/COLOR]", -iHappiness);
				
				-- Empire Unhappy
				else
					strHappiness = string.format("[ICON_HAPPINESS_3][COLOR_UNHAPPINESS]%i[/COLOR]", -iHappiness);
				end
			end
			...
			if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_HAPPINESS)) then
				...
				if (pPlayer:GetGoldenAgeTurns() > 0) then
					...
					local GAPPerTurn = pPlayer:GetGoldenAgePointsPerTurn() + JFD_GetYieldAddins("YIELD_GOLDEN_AGE"):YieldSum()
			...
			if userSettingRTPSovereigntyCore then
				if pPlayer:HasGovernment() then
					...
					strSovereigntyStr = Locale.ConvertTextKey("TXT_KEY_JFD_SOVEREIGNTY_TOP", pSovereigntyColour, iSovereignty, pPlayer:GetMaxSovereignty()) 
			...
			-- Update Piety
			-----------------------------
			if (userSettingRTPPietyCore and pPlayer:HasStateReligion()) then
				local iPietyPerTurn;
				...
				--JFD's Top Panel Addins ENDS
				--===============================	
				local strPietyStr = pPlayer:GetPietyTopPanelInfo(iPietyPerTurn)
			...
			if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_RELIGION)) then
				...
				iFaithPerTurn = iFaithPerTurn + pPlayer:GetExtraFaithFromGAP()
				--===============================
				--JFD's Top Panel Addins BEGINS
				--===============================
			...
			for pResource in GameInfo.Resources("ResourceClassType IS NOT 'RESOURCECLASS_JFD_DECISIONS'") do
				...
				if Game.GetResourceUsageType(iResourceLoop) == ResourceUsageTypes.RESOURCEUSAGE_STRATEGIC then
					...
					if pResource.Type == "RESOURCE_JFD_SHACKLES" then
						bShowResource = pPlayer:CanPurchaseSlaves() -- JFD Slavery
					end
		...
		if (not userSettingMiscClock) then Controls.CurrentTime:SetHide(true) end
...
function ProsperityTipHandler( control )
	...
	local iProsperity, strText = Players[Game.GetActivePlayer()]:CalculateProsperity(true);
	tipControlTable.TooltipLabel:SetText( strText );
	tipControlTable.TopPanelMouseover:SetHide(false);
...
function SovereigntyTipHandler( control )
	...
	local strText = Players[Game.GetActivePlayer()]:GetSovereigntyTopPanelInfoTT()
...
function PietyTipHandler( control )
	...
	local strText = Players[Game.GetActivePlayer()]:GetPietyTopPanelInfoTT()
...
function FaithTipHandler( control )
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_RELIGION)) then
		...
		-- Faith from Religion
		local iFaithFromReligion = pPlayer:GetFaithPerTurnFromReligion() + pPlayer:GetExtraFaithFromGAP()
...
function ResourcesTipHandler( control )
	...
	for pResource in GameInfo.Resources() do
		...
		if (Game.GetResourceUsageType(iResourceLoop) == ResourceUsageTypes.RESOURCEUSAGE_STRATEGIC) then
			...
			if pResource.Type == "RESOURCE_JFD_SHACKLES" then
				bShowResource = pPlayer:CanPurchaseSlaves()
			end
	...
	for pResource in GameInfo.Resources("Type IN ('RESOURCE_JFD_DIGNITARIES', 'RESOURCE_MAGISTRATES', 'RESOURCE_JFD_CAPTAINS', 'RESOURCE_JFD_CLERICS', 'RESOURCE_JFD_INVENTORS')") do
		local iResourceLoop = pResource.ID;
		iNumAvailable = pPlayer:GetNumResourceAvailable(iResourceLoop, true);
		iNumUsed = pPlayer:GetNumResourceUsed(iResourceLoop);
		iNumTotal = pPlayer:GetNumResourceTotal(iResourceLoop, true);
