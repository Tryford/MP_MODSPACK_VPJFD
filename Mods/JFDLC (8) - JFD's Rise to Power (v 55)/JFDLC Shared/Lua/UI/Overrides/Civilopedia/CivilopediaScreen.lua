...
--local isUsingCIVDiplomacy = Game.IsCIVDiploActive()
--
----------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------
--
-- Mods to support ShowInPedia entries on the following tables (in addition to Builds and Units)
--
-- Beliefs, Buildings, Civilizations, Concepts, Eras, FakeFeatures, Features, Improvements,
-- LeagueProjectRewards, LeagueProjects, MinorCivilizations, Policies, PolicyBranchTypes, Projects,
-- Religions, Resolutions, Resources, Routes, Specialists, Technologies, Terrains and UnitPromotions
--
-- Use SQL to add the ShowInPedia column to the required table(s), eg
--   ALTER TABLE Buildings ADD ShowInPedia INTEGER DEFAULT 1;
-- (It is recommended to place each ALTER TABLE statement in its own .sql file, in case of conflicts.)
-- Use XML/SQL to set ShowInPedia to 0 for entries you don't want to appear in the Civilopedia, eg
--   <ShowInPedia>0</ShowInPedia>
--
-- The Civilopedia will display exactly what you tell it to, so if you add a prereq tech to a building,
-- but that tech has ShowInPedia=0, there will be no indication that a tech is required to construct it.
--
...
local gShowInPediaShowSupport = {Builds=true, Units=true}

function ShowInPediaShow(table, data)
	if data then
		for row in GameInfo.JFD_Civilopedia_HideFromPedia("Type = '" .. data.Type .. "'") do
			return false
		end
	end
	return true
...
--
-- Replacement DB.Query iterator
--
--   for building in DB.Query(sql) do ... end
--   for building in ShowInPediaQuery(table, sql) do ... end
--
--   for replacedUnit in DB.Query(sql, unitClassType) do ... end
--   for replacedUnit in ShowInPediaQuery(table, sql, unitClassType) do ... end
--

function ShowInPediaQuery(table, ...)
	local p = {...}

	local next = coroutine.create(function ()
		for row in DB.Query(unpack(p)) do
			if ShowInPediaShow(table, row) then
				coroutine.yield(row)
			end
		end
...
--
--   for thisPromotion in GameInfo.UnitPromotions() do ... end
--   for thisPromotion in ShowInPediaIterator("UnitPromotions") do ... end
--
--   for policy in GameInfo.Policies("PolicyBranchType = '" .. branch.Type .. "'") do ... end
--   for policy in ShowInPediaIterator("Policies", "PolicyBranchType = '" .. branch.Type .. "'") do ... end
--

function ShowInPediaIterator(table, condition)
	local next = coroutine.create(function ()
...
--  local freePolicy = ShowInPediaLookup("Policies", branch.FreePolicy)
--
-- MUST always be followed by a check for nil
--

function ShowInPediaLookup(table, key)
	local row = GameInfo[table][key]

	if not(ShowInPediaShow(table, row)) then
		return nil
	end
...
--
-- Replacement GameInfo sizeof (#) operator
--
--   local numReligions = #GameInfo.Religions
--   local numReligions = ShowInPediaSizeOf("Religions")
--

function ShowInPediaSizeOf(table)
	local iSize = 0

	for _ in ShowInPediaIterator(table) do
		iSize = iSize + 1
	end
...
--	for thisConcept in GameInfo.Concepts() do
	for thisConcept in ShowInPediaIterator("Concepts") do
...
--	for era in GameInfo.Eras() do
	for era in ShowInPediaIterator("Eras") do
...
-- 		for tech in GameInfo.Technologies("Era = '" .. era.Type .. "'") do
 		for tech in ShowInPediaIterator("Technologies", "Era = '" .. era.Type .. "'") do
...
--		for unit in DB.Query("SELECT Units.ID, Units.Description, Units.PortraitIndex, Units.IconAtlas From Units where Units.FaithCost > 0 and Units.Cost == -1 and not RequiresFaithPurchaseEnabled and ShowInPedia == 1") do
		for unit in ShowInPediaQuery("Units", "SELECT Units.* From Units where Units.FaithCost > 0 and Units.Cost == -1 and not RequiresFaithPurchaseEnabled and ShowInPedia == 1") do
...
--		for unit in UnitsByEra(era.Type) do
		for unit in ShowInPediaQuery("Units", sql, era.Type) do
...
--			for unit in DB.Query("SELECT Units.ID, Units.Description, Units.PortraitIndex, Units.IconAtlas From Units where PreReqTech is NULL and Special is NULL and (Units.FaithCost = 0 or RequiresFaithPurchaseEnabled or Units.Cost >= 0) and Units.ShowInPedia = 1") do
			for unit in ShowInPediaQuery("Units", "SELECT Units.* From Units where PreReqTech is NULL and Special is NULL and (Units.FaithCost = 0 or RequiresFaithPurchaseEnabled or Units.Cost >= 0) and Units.ShowInPedia = 1") do
				AddArticle(eraID, tableid, unit);
				tableid = tableid + 1;
			end
		end
...
--	for thisPromotion in GameInfo.UnitPromotions() do
	for thisPromotion in ShowInPediaIterator("UnitPromotions") do
...
--		for building in DB.Query("SELECT Buildings.ID, Buildings.Description, Buildings.PortraitIndex, Buildings.IconAtlas from Buildings inner join  BuildingClasses on Buildings.BuildingClass = BuildingClasses.Type where Buildings.FaithCost > 0 and Buildings.Cost == -1 and BuildingClasses.MaxGlobalInstances < 0 and (BuildingClasses.MaxPlayerInstances <> 1 or Buildings.SpecialistCount > 0) and BuildingClasses.MaxTeamInstances < 0;") do
		for building in ShowInPediaQuery("Buildings", "SELECT Buildings.* from Buildings inner join  BuildingClasses on Buildings.BuildingClass = BuildingClasses.Type where Buildings.FaithCost > 0 and Buildings.Cost == -1 and BuildingClasses.MaxGlobalInstances < 0 and (BuildingClasses.MaxPlayerInstances < 1 or Buildings.SpecialistCount > 0) and BuildingClasses.MaxTeamInstances < 0;") do
...
--		for building in BuildingsByEra(era.Type) do
		for building in ShowInPediaQuery("Buildings", sql, era.Type) do
...
--			for building in DB.Query(sql) do
			for building in ShowInPediaQuery("Buildings", sql) do
				AddArticle(eraID, tableid, building);
				tableid = tableid + 1;
			end
		end
...
--	for building in GameInfo.Buildings() do
	for building in ShowInPediaIterator("Buildings") do
		-- exclude wonders etc.
...
--	for building in GameInfo.Projects() do
	for building in ShowInPediaIterator("Projects") do
...
--	for branch in GameInfo.PolicyBranchTypes() do
	for branch in ShowInPediaIterator("PolicyBranchTypes") do
...
-- 		for policy in GameInfo.Policies("PolicyBranchType = '" .. branch.Type .. "'") do
 		for policy in ShowInPediaIterator("Policies", "PolicyBranchType = '" .. branch.Type .. "'") do
			local article = {};
...
--	for person in GameInfo.Specialists() do
	for person in ShowInPediaIterator("Specialists") do
		local article = {};
...
--	for unit in GameInfo.Units() do
	for unit in ShowInPediaIterator("Units") do
...
--	for row in GameInfo.Civilizations() do
	for row in ShowInPediaIterator("Civilizations") do
		if row.Type ~= "CIVILIZATION_MINOR" and row.Type ~= "CIVILIZATION_BARBARIAN" then
			local addToList = true
			for civ in GameInfo.Civilizations("Type = '" .. row.Type .. "' AND DerivativeCiv IS NOT NULL") do
				if GameInfo.Civilizations[civ.DerivativeCiv] then
					addToList = false
				end
			end
			if addToList then
...
--	for trait in GameInfo.MinorCivTraits() do
	for trait in ShowInPediaIterator("MinorCivTraits") do
...
-- 		for cityState in GameInfo.MinorCivilizations("MinorCivTrait = '" .. trait.Type .. "'") do
 		for cityState in ShowInPediaIterator("MinorCivilizations", "MinorCivTrait = '" .. trait.Type .. "'") do
...
--	for row in GameInfo.Terrains() do
	for row in ShowInPediaIterator("Terrains") do
		local article = {};
...
--	for row in GameInfo.Features() do
	for row in ShowInPediaIterator("Features") do
		local article = {};
		local name = Locale.ConvertTextKey( row.Description )
		article.entryName = name;
		article.entryID = row.ID + 1000;
...
--	for row in GameInfo.FakeFeatures() do
	for row in ShowInPediaIterator("FakeFeatures") do
		local article = {};
		local name = Locale.ConvertTextKey( row.Description )
		article.entryName = name;
...
-- 		for resource in GameInfo.Resources( "ResourceUsage = '" .. resourceType .. "'" ) do
 		for resource in ShowInPediaIterator("Resources",  "ResourceUsage = '" .. resourceType .. "'" ) do
...
--	for row in GameInfo.Improvements() do
	for row in ShowInPediaIterator("Improvements") do
...
--	for row in GameInfo.Routes() do
	for row in ShowInPediaIterator("Routes") do
		local article = {};
		local name = Locale.ConvertTextKey( row.Description );
		article.entryName = name;
		article.entryID = row.ID + 1000;
...
--		for religion in GameInfo.Religions("Type <> 'RELIGION_PANTHEON'") do
		for religion in ShowInPediaIterator("Religions", "Type <> 'RELIGION_PANTHEON'") do
...
-- 		for belief in GameInfo.Beliefs(condition) do
 		for belief in ShowInPediaIterator("Beliefs", condition) do
...
--		for resolution in GameInfo.Resolutions() do
		for resolution in ShowInPediaIterator("Resolutions") do
...
--		for leagueProject in GameInfo.LeagueProjects() do
		for leagueProject in ShowInPediaIterator("LeagueProjects") do
...
--			local prereq = GameInfo.Technologies[row.PrereqTech];
			local prereq = ShowInPediaLookup("Technologies", row.PrereqTech);
			if (prereq) then
...
--			local leadsTo = GameInfo.Technologies[row.TechType];
			local leadsTo = ShowInPediaLookup("Technologies", row.TechType);
			if (leadsTo) then
		...
		-- New - update the leads to techs for OR links --

		for row in GameInfo.Technology_ORPrereqTechs( prereqCondition ) do
			local leadsTo = GameInfo.Technologies[row.TechType];
			local thisLeadsToInstance = g_LeadsToTechManager:GetInstance();
			if thisLeadsToInstance then
				local textureOffset, textureSheet = IconLookup( leadsTo.PortraitIndex, buttonSize, leadsTo.IconAtlas );				
		...
		-- End of That OR Stuff -- Cecil Lizard
...
--		for thisUnitInfo in GameInfo.Units( prereqCondition ) do
		for thisUnitInfo in ShowInPediaIterator("Units",  prereqCondition ) do
...
--		for thisBuildingInfo in GameInfo.Buildings( prereqCondition ) do
		for thisBuildingInfo in ShowInPediaIterator("Buildings",  prereqCondition ) do
...
--		for thisProjectInfo in GameInfo.Projects( otherPrereqCondition ) do
		for thisProjectInfo in ShowInPediaIterator("Projects",  otherPrereqCondition ) do
...
--		for revealedResource in GameInfo.Resources( revealCondition ) do
		for revealedResource in ShowInPediaIterator("Resources",  revealCondition ) do
...
--		for thisBuildInfo in GameInfo.Builds{PrereqTech = techType, ShowInPedia = 1} do
		for thisBuildInfo in ShowInPediaIterator("Builds", "PrereqTech = '" .. techType .. "'") do
...
CivilopediaCategory[CategoryUnits].SelectArticle = function( unitID, shouldAddToList )
	...
	if unitID ~= -1 then
		...
		if thisUnit.CombatClass then
			if thisUnit.IsMounted then
				Controls.CombatTypeLabel:SetText( Locale.ConvertTextKey( GameInfo.UnitCombatInfos[thisUnit.CombatClass].Description .. "(Ranged)" ) );
			else
		...
		local charges = 0
		for row in GameInfo.Unit_JFD_BuildCharges("UnitType = '" .. thisUnit.Type .. "'") do
			charges = charges + row.NumCharges
		end
		if charges > 0 then
			Controls.ChargesLabel:SetText( tostring(charges).." [ICON_WORKER]" );
			Controls.ChargesFrame:SetHide( false );
		else
			Controls.ChargesFrame:SetHide( true );
		end
...
--			local promotion = GameInfo.UnitPromotions[row.PromotionType];
			local promotion = ShowInPediaLookup("UnitPromotions", row.PromotionType);
			if promotion then
...
--			local requiredResource = GameInfo.Resources[row.ResourceType];
			local requiredResource = ShowInPediaLookup("Resources", row.ResourceType);
...
--			local prereq = GameInfo.Technologies[thisUnit.PrereqTech];
			local prereq = ShowInPediaLookup("Technologies", thisUnit.PrereqTech);
			if prereq then
				local thisPrereqInstance = g_PrereqTechManager:GetInstance();
				if thisPrereqInstance then
					local textureOffset, textureSheet = IconLookup( prereq.PortraitIndex, buttonSize, prereq.IconAtlas );
...
--			local obs = GameInfo.Technologies[thisUnit.ObsoleteTech];
			local obs = ShowInPediaLookup("Technologies", thisUnit.ObsoleteTech);
			if obs then
...
--				local obs = GameInfo.Units[iUnitUpgrade];
				local obs = ShowInPediaLookup("Units", iUnitUpgrade);
				if obs then
...
--				local upgradeUnit = GameInfo.Units[unitClass.DefaultUnit];
				local upgradeUnit = ShowInPediaLookup("Units", unitClass.DefaultUnit);
				if (upgradeUnit) then
					local thisUpgradeInstance = g_UpgradeManager:GetInstance();
					if thisUpgradeInstance then
...
--			for replacedUnit in DB.Query("SELECT u.ID, u.Description, u.PortraitIndex, u.IconAtlas from Units as u inner join UnitClasses as uc on u.Type = uc.DefaultUnit where uc.Type = ?", unitClassType) do
			for replacedUnit in ShowInPediaQuery("Units", "SELECT Units.* from Units as u inner join UnitClasses as uc on u.Type = uc.DefaultUnit where uc.Type = ?", unitClassType) do
		...
		for civilizationType, _ in pairs(specificCivs) do
			...
			if(leader ~= nil) then
				...
				if thisLeaderInstance then
					...
					UpdateSmallButton( buttonAdded, thisLeaderInstance.LeaderImage, thisLeaderInstance.LeaderButton, textureSheet, textureOffset, CategoryCivilizations, Locale.ConvertTextKey( leader.Description ), civ.ID + 1000 );
					buttonAdded = buttonAdded + 1;
				end
			end
		end
...
--			local thisReq = GameInfo.UnitPromotions[thisPromotion.PromotionPrereqOr1];
			local thisReq = ShowInPediaLookup("UnitPromotions", thisPromotion.PromotionPrereqOr1);
			if thisReq then
...
--			local thisReq = GameInfo.UnitPromotions[thisPromotion.PromotionPrereqOr2];
			local thisReq = ShowInPediaLookup("UnitPromotions", thisPromotion.PromotionPrereqOr2);
			if thisReq then
				local thisRequiredPromotionInstance = g_RequiredPromotionsManager:GetInstance();
				if thisRequiredPromotionInstance then
					local textureOffset, textureSheet = IconLookup( thisReq.PortraitIndex, buttonSize, thisReq.IconAtlas );
...
--			local thisReq = GameInfo.UnitPromotions[thisPromotion.PromotionPrereqOr3];
			local thisReq = ShowInPediaLookup("UnitPromotions", thisPromotion.PromotionPrereqOr3);
			if thisReq then
				local thisRequiredPromotionInstance = g_RequiredPromotionsManager:GetInstance();
				if thisRequiredPromotionInstance then
					local textureOffset, textureSheet = IconLookup( thisReq.PortraitIndex, buttonSize, thisReq.IconAtlas );
...
--			local thisReq = GameInfo.UnitPromotions[thisPromotion.PromotionPrereqOr4];
			local thisReq = ShowInPediaLookup("UnitPromotions", thisPromotion.PromotionPrereqOr4);
			if thisReq then
				local thisRequiredPromotionInstance = g_RequiredPromotionsManager:GetInstance();
				if thisRequiredPromotionInstance then
					local textureOffset, textureSheet = IconLookup( thisReq.PortraitIndex, buttonSize, thisReq.IconAtlas );
...
--			local thisReq = GameInfo.UnitPromotions[thisPromotion.PromotionPrereqOr5];
			local thisReq = ShowInPediaLookup("UnitPromotions", thisPromotion.PromotionPrereqOr5);
			if thisReq then
				local thisRequiredPromotionInstance = g_RequiredPromotionsManager:GetInstance();
				if thisRequiredPromotionInstance then
					local textureOffset, textureSheet = IconLookup( thisReq.PortraitIndex, buttonSize, thisReq.IconAtlas );
...
--			local thisReq = GameInfo.UnitPromotions[thisPromotion.PromotionPrereqOr6];
			local thisReq = ShowInPediaLookup("UnitPromotions", thisPromotion.PromotionPrereqOr6);
			if thisReq then
				local thisRequiredPromotionInstance = g_RequiredPromotionsManager:GetInstance();
				if thisRequiredPromotionInstance then
					local textureOffset, textureSheet = IconLookup( thisReq.PortraitIndex, buttonSize, thisReq.IconAtlas );
...
--		for tLeagueProject in GameInfo.LeagueProjects() do
		for tLeagueProject in ShowInPediaIterator("LeagueProjects") do
...
--						local tReward = GameInfo.LeagueProjectRewards[tLeagueProject["RewardTier" .. iTier]];
						local tReward = ShowInPediaLookup("LeagueProjectRewards", tLeagueProject["RewardTier" .. iTier]);
...
--							if (GameInfo.Buildings[tReward.Building] ~= nil and GameInfo.Buildings[tReward.Building].ID == buildingID) then
							if (ShowInPediaLookup("Buildings", tReward.Building) ~= nil and GameInfo.Buildings[tReward.Building].ID == buildingID) then
		...
		local GetBuildingYieldChange = function(buildingID, yieldType)
			if(Game ~= nil) then
				return Game.GetBuildingYieldChange(buildingID, GameInfo.Yields[yieldType].ID);
			else
		...
		local GetBuildingYieldModifier = function(buildingID, yieldType)
			if(Game ~= nil) then
				return Game.GetBuildingYieldModifier(buildingID, GameInfo.Yields[yieldType].ID);
			else
				local yieldModifier = 0;
				local buildingType = GameInfo.Buildings[buildingID].Type;
		...
		-- update the Crime % mods
		local crimeItems = {};
		local iCrime = GetBuildingYieldModifier(buildingID, "YIELD_JFD_CRIME");
		if(iCrime ~= 0) then
			table.insert(crimeItems, "" .. tostring(iCrime).."% [ICON_JFD_CRIME_BLACK]");
		end
		
		-- update the Crime
		local iCrime = GetBuildingYieldChange(buildingID, "YIELD_JFD_CRIME");
		if(iCrime ~= 0) then
			table.insert(crimeItems, "+" .. tostring(iCrime).." [ICON_JFD_CRIME_BLACK]");
		end
		local buildingType = GameInfo.Buildings[buildingID].Type;
		for row in GameInfo.Building_JFD_CrimeMods(condition) do
			table.insert(crimeItems, "" .. tostring(row.CrimeMod).."% " .. GameInfo.JFD_Crimes[row.CrimeType].IconString);
		end
			
		if(#crimeItems > 0) then
			Controls.CrimeLabel:SetText( table.concat(crimeItems, ", ") );
			Controls.CrimeFrame:SetHide( false );
		end
		
		-- update the Culture
		local cultureItems = {};
		local iCulture = GetBuildingYieldModifier(buildingID, "YIELD_CULTURE");
		iCulture = iCulture + GameInfo.Buildings[buildingID].CultureRateModifier
		if(iCulture > 0) then
			table.insert(cultureItems, "+" .. tostring(iCulture).."% [ICON_CULTURE]" );
		end

		-- update the Culture Change
		...
		-- update the Faith
		local faithItems = {};
		local iCulture = GetBuildingYieldModifier(buildingID, "YIELD_FAITH");
		if(iCulture > 0) then
			table.insert(faithItems, "+" .. tostring(iCulture).."% [ICON_PEACE]" );
		end

		-- update the Faith Change
		local iCulture = GetBuildingYieldChange(buildingID, "YIELD_FAITH");
		if(iCulture > 0) then
			table.insert(faithItems, "+" .. tostring(iCulture).." [ICON_PEACE]" );
		end
		...
		-- JFD update the Health
		local healthItems = {};
		local iHealth = GetBuildingYieldModifier(buildingID, "YIELD_JFD_HEALTH");
		if(iHealth > 0) then
			table.insert(healthItems, "+" .. tostring(iHealth).."% [ICON_JFD_HEALTH]" );
		end

		-- update the Health Change
		local iHealth = GetBuildingYieldChange(buildingID, "YIELD_JFD_HEALTH");
		if(iHealth > 0) then
			table.insert(healthItems, "+" .. tostring(iHealth).." [ICON_JFD_HEALTH]" );
		end

		if(#healthItems > 0) then
			Controls.HealthLabel:SetText( table.concat(healthItems, ", ") );
			Controls.HealthFrame:SetHide( false );
		end
		
		-- JFD update the Disease
		local diseaseItems = {};
		local iDisease = GetBuildingYieldModifier(buildingID, "YIELD_JFD_DISEASE");
		if(iDisease > 0) then
			table.insert(diseaseItems, "+" .. tostring(iDisease).."% [ICON_JFD_DISEASE]" );
		end

		-- update the Disease Change
		local iDisease = GetBuildingYieldChange(buildingID, "YIELD_JFD_DISEASE");
		if(iDisease > 0) then
			table.insert(diseaseItems, "+" .. tostring(iDisease).." [ICON_JFD_DISEASE]" );
		end

		if(#diseaseItems > 0) then
			Controls.DiseaseLabel:SetText( table.concat(diseaseItems, ", ") );
			Controls.DiseaseFrame:SetHide( false );
		end
		...
		-- JFD update the Loyalty
		local loyaltyItems = {};
		local iLoyalty = GetBuildingYieldModifier(buildingID, "YIELD_JFD_LOYALTY");
		if(iLoyalty ~= 0) then
			if iLoyalty > 0 then iLoyalty = "+" .. iLoyalty end
			table.insert(loyaltyItems, "" .. tostring(iLoyalty).."% [ICON_JFD_BLUE_FIST]" );
		end
		if(#loyaltyItems > 0) then
			Controls.LoyaltyLabel:SetText( table.concat(loyaltyItems, ", ") );
			Controls.LoyaltyFrame:SetHide( false );
		end
		
		-- JFD update the Power
		local power = GameInfo.Resources["RESOURCE_JFD_POWER"]
		if power then
			local powerItems = {};
			local iPower = 0
			for row in GameInfo.Building_ResourceQuantity("BuildingType = '" .. buildingType .. "' AND ResourceType = 'RESOURCE_JFD_POWER'") do
				iPower = iPower + row.Quantity
			end	
			if(iPower ~= 0) then
				if iPower > 0 then iPower = "+" .. iPower end
				table.insert(powerItems, "" .. tostring(iPower).." [ICON_JFD_POWER]" );
			end
			if(#powerItems > 0) then
				Controls.PowerLabel:SetText( table.concat(powerItems, ", ") );
				Controls.PowerFrame:SetHide( false );
			end
		end
		...
		if iGPType ~= nil then
			...
			if (iNumPoints > 0) then
				if (ShowInPediaLookup("Specialists", iGPType)) then
...
--			local prereq = GameInfo.Technologies[thisBuilding.PrereqTech];
			local prereq = ShowInPediaLookup("Technologies", thisBuilding.PrereqTech);
			if prereq then
				local thisPrereqInstance = g_PrereqTechManager:GetInstance();
				if thisPrereqInstance then
					local textureOffset, textureSheet = IconLookup( prereq.PortraitIndex, buttonSize, prereq.IconAtlas );
...
--			local thisSpec = GameInfo.Specialists[thisBuilding.SpecialistType];
			local thisSpec = ShowInPediaLookup("Specialists", thisBuilding.SpecialistType);
			if(thisSpec)  then
...
--				local thisBuildingInfo = GameInfo.Buildings[buildingClass.DefaultBuilding];
				local thisBuildingInfo = ShowInPediaLookup("Buildings", buildingClass.DefaultBuilding);
...
--					defaultBuilding = GameInfo.Buildings[classrow.DefaultBuilding];
					defaultBuilding = ShowInPediaLookup("Buildings", classrow.DefaultBuilding);
				end
				if defaultBuilding then
--					thisCiv = GameInfo.Civilizations[row.CivilizationType];
					thisCiv = ShowInPediaLookup("Civilizations", row.CivilizationType);
					break;
				end
			end
		end
		...
		if thisBuilding.Help then
			...
			if (thisBuilding.Help ~= thisBuilding.Strategy) then
				local help = Locale.ConvertTextKey(thisBuilding.Help)
				-- Additional Help
				for row in GameInfo.Building_JFD_HelpTexts("BuildingType = '" .. thisBuilding.Type .. "' AND IsWrittenFirst = 1 AND HelpText IS NOT NULL AND CivilizationType IS NULL") do
					help = Locale.ConvertTextKey(row.HelpText) .. help
				end
				-- Additional Help
				for row in GameInfo.Building_JFD_HelpTexts("BuildingType = '" .. thisBuilding.Type .. "' AND IsWrittenFirst = 0 AND HelpText IS NOT NULL AND CivilizationType IS NULL") do
					help = help .. Locale.ConvertTextKey(row.HelpText)
				end
...
--				local prereq = GameInfo.Technologies[thisProject.TechPrereq];
				local prereq = ShowInPediaLookup("Technologies", thisProject.TechPrereq);
				if prereq then
					local thisPrereqInstance = g_PrereqTechManager:GetInstance();
					if thisPrereqInstance then
						local textureOffset, textureSheet = IconLookup( prereq.PortraitIndex, buttonSize, prereq.IconAtlas );
...
--					local era = GameInfo.Eras[branch.EraPrereq];
					local era = ShowInPediaLookup("Eras", branch.EraPrereq);
					if era then
...
--			local requiredPolicy = GameInfo.Policies[row.PrereqPolicy];
			local requiredPolicy = ShowInPediaLookup("Policies", row.PrereqPolicy);
...
CivilopediaCategory[CategoryCivilizations].SelectArticle = function( rawCivID, shouldAddToList )
	...
	if rawCivID < 1000 then
		if rawCivID ~= -1 then
			...
			-- local derivativeCiv = thisCiv.DerivativeCiv or thisCiv.Type
			local derivativeCiv = nil
			-- if derivativeCiv and GameInfo.Civilizations[thisCiv.DerivativeCiv] then
				-- derivativeCiv = GameInfo.Civilizations[derivativeCiv]
				-- if (not derivativeCiv) then derivativeCiv = true end
			-- end
			if thisCiv and (not derivativeCiv) and thisCiv.Type ~= "CIVILIZATION_MINOR" and thisCiv.Type ~= "CIVILIZATION_BARBARIAN" then
				...
				for otherCiv in GameInfo.Civilizations("DerivativeCiv = '" .. thisCiv.Type .. "'") do
					local otherLeader = GameInfo.Leaders[GameInfo.Civilization_Leaders( "CivilizationType = '" .. otherCiv.Type .. "'" )().LeaderheadType]
					if otherLeader then
						-- local leader = GameInfo.Leaders[GameInfo.Civilization_Leaders( "CivilizationType = '" .. thisCiv.Type .. "'" )().LeaderheadType]; -- todo: add support for multiple leaders
						local thisLeaderInstance = g_LeadersManager:GetInstance();
						if thisLeaderInstance then
							local textureOffset, textureSheet = IconLookup( otherLeader.PortraitIndex, buttonSize, otherLeader.IconAtlas );
							if textureOffset == nil then
								textureSheet = defaultErrorTextureSheet;
								textureOffset = nullOffset;
							end
							UpdateSmallButton( buttonAdded, thisLeaderInstance.LeaderImage, thisLeaderInstance.LeaderButton, textureSheet, textureOffset, CategoryCivilizations, Locale.ConvertTextKey( otherLeader.Description ), otherCiv.ID + 1000 );
							buttonAdded = buttonAdded + 1;
						end
					end
				end
...
--						local thisUnitInfo = GameInfo.Units[thisOverride.UnitType];
						-- local thisUnitInfo = ShowInPediaLookup("Units", thisOverride.UnitType);
				...
				-- list of UUs of other leaders
				-- for thisOtherCiv in GameInfo.JFD_Civilopedia_CivilizationLeaderType(condition) do
					-- for thisOverride in GameInfo.Civilization_UnitClassOverrides( "CivilizationType = '" .. thisOtherCiv.TheirCivilizationType .. "'" ) do
						-- if thisOverride.UnitType ~= nil then
							-- local thisUnitInfo = GameInfo.Units[thisOverride.UnitType];
							-- local thisUnitInfo = ShowInPediaLookup("Units", thisOverride.UnitType);
							-- if thisUnitInfo then
...
--						local thisBuildingInfo = GameInfo.Buildings[thisOverride.BuildingType];
						-- local thisBuildingInfo = ShowInPediaLookup("Buildings", thisOverride.BuildingType);
						-- if thisBuildingInfo then
				...
				-- list of UBs of other leaders
				-- for thisOtherCiv in GameInfo.JFD_Civilopedia_CivilizationLeaderType(condition) do
					-- for thisOverride in GameInfo.Civilization_BuildingClassOverrides( "CivilizationType = '" .. thisOtherCiv.TheirCivilizationType .. "'" ) do
						-- if(thisOverride.BuildingType ~= nil) then
							-- local thisBuildingInfo = GameInfo.Buildings[thisOverride.BuildingType];
							-- local thisBuildingInfo = ShowInPediaLookup("Buildings", thisOverride.BuildingType);
							-- if thisBuildingInfo then
...
--				for thisImprovement in GameInfo.Improvements( condition ) do
				-- for thisImprovement in ShowInPediaIterator("Improvements",  condition ) do
				...
				-- list of UIs of other leaders
				-- for thisOtherCiv in GameInfo.JFD_Civilopedia_CivilizationLeaderType(condition) do
					-- local thisCondition = "CivilizationType = '" .. thisOtherCiv.TheirCivilizationType .. "'"
					-- for thisImprovement in ShowInPediaIterator("Improvements",  thisCondition ) do
						-- local thisImprovementInstance = g_UniqueImprovementsManager:GetInstance();
						-- if thisImprovementInstance then
		...
		if civID ~= -1 then
			...
			if thisCiv and thisCiv.Type ~= "CIVILIZATION_MINOR" and thisCiv.Type ~= "CIVILIZATION_BARBARIAN" then
				...
				local derivativeCiv = thisCiv.DerivativeCiv
				local motherCivID = thisCiv.ID
				if derivativeCiv and GameInfo.Civilizations[derivativeCiv] then
					motherCivID = GameInfo.Civilizations[derivativeCiv].ID
				end
				if thisCivInstance then
					...
					UpdateSmallButton( buttonAdded, thisCivInstance.CivilizationImage, thisCivInstance.CivilizationButton, textureSheet, textureOffset, CategoryCivilizations, Locale.ConvertTextKey( thisCiv.ShortDescription ), motherCivID );
					buttonAdded = buttonAdded + 1;
				end
				...
				-- list of UUs
 				g_UniqueUnitsManager:ResetInstances();
				buttonAdded = 0;
				for thisOverride in GameInfo.Civilization_UnitClassOverrides("CivilizationType = '" .. thisCiv.Type .. "'" ) do
					if thisOverride.UnitType ~= nil then
--						local thisUnitInfo = GameInfo.Units[thisOverride.UnitType];
						local thisUnitInfo = ShowInPediaLookup("Units", thisOverride.UnitType);
						if thisUnitInfo then
				...
				g_UniqueBuildingsManager:ResetInstances();
				buttonAdded = 0;
				for thisOverride in GameInfo.Civilization_BuildingClassOverrides( "CivilizationType = '" .. thisCiv.Type .. "'" ) do
					if(thisOverride.BuildingType ~= nil) then
--						local thisBuildingInfo = GameInfo.Buildings[thisOverride.BuildingType];
						local thisBuildingInfo = ShowInPediaLookup("Buildings", thisOverride.BuildingType);
						if thisBuildingInfo then
				...
				buttonAdded = 0;
--				for thisImprovement in GameInfo.Improvements( condition ) do
				for thisImprovement in ShowInPediaIterator("Improvements",  "CivilizationType = '" .. thisCiv.Type .. "'" ) do
					local thisImprovementInstance = g_UniqueImprovementsManager:GetInstance();
					if thisImprovementInstance then
		...
		if terrainId ~= -1 then
			...
			if thisTerrain then
				...
				if numYields == 0 then
					...
					Controls.YieldLabel:SetToolTipString( Locale.ConvertTextKey( yieldString ) );
				end
...
--					local thisFeature = GameInfo.Features[row.FeatureType];
					local thisFeature = ShowInPediaLookup("Features", row.FeatureType);
					if thisFeature then
...
--					local thisResource = GameInfo.Resources[row.ResourceType];
					local thisResource = ShowInPediaLookup("Resources", row.ResourceType);
...
--					for thisResource in GameInfo.Resources() do
					for thisResource in ShowInPediaIterator("Resources") do
...
--					local thisTerrain = GameInfo.Features[row.TerrainType];
					local thisTerrain = ShowInPediaLookup("Features", row.TerrainType);
					if thisTerrain then
...
--				local prereq = GameInfo.Technologies[thisResource.TechReveal];
				local prereq = ShowInPediaLookup("Technologies", thisResource.TechReveal);
				if (prereq) then
...
--				local thisTerrain = GameInfo.Terrains[row.TerrainType];
				local thisTerrain = ShowInPediaLookup("Terrains", row.TerrainType);
				if thisTerrain then
...
--				local thisImprovement = GameInfo.Improvements[row.ImprovementType];
				local thisImprovement = ShowInPediaLookup("Improvements", row.ImprovementType);
...
--			for row in GameInfo.Builds( condition ) do
			for row in ShowInPediaIterator("Builds",  condition ) do
				if row.PrereqTech then
--					prereq = GameInfo.Technologies[row.PrereqTech];
					prereq = ShowInPediaLookup("Technologies", row.PrereqTech);
				end
			end
			...
			for row in GameInfo.Improvement_Yields( condition ) do
				...
				if row.YieldType == "YIELD_JFD_DEVELOPMENT" then
					yieldString = yieldString..tostring(row.Yield/10)..GameInfo.Yields[row.YieldType].IconString.." ";
				else
					yieldString = yieldString..tostring(row.Yield)..GameInfo.Yields[row.YieldType].IconString.." ";
				end
			end
...
--				local thisCiv = GameInfo.Civilizations[thisImprovement.CivilizationType];
				local thisCiv = ShowInPediaLookup("Civilizations", thisImprovement.CivilizationType);
				if thisCiv then
					local leader = GameInfo.Leaders[GameInfo.Civilization_Leaders( "CivilizationType = '" .. thisCiv.Type .. "'" )().LeaderheadType]
					g_LeadersManager:ResetInstances();
					local thisLeaderInstance = g_LeadersManager:GetInstance();
...
--		offset = #GameInfo.Religions;
		offset = ShowInPediaSizeOf("Religions");
	end
...
--			local thisReligion = GameInfo[t][id];
			local thisReligion = ShowInPediaLookup(t, id);
...
--			local thisBelief = GameInfo[t][id];
			local thisBelief = ShowInPediaLookup(t, id);
			...
			if (thisBelief ~= nil) then
				...
				if (thisBelief.Description ~= nil) then
					UpdateTextBlock( "[ICON_BULLET]" .. Locale.ConvertTextKey( thisBelief.Description ).. "." , Controls.SummaryLabel, Controls.SummaryInnerFrame, Controls.SummaryFrame );
				end
			end
		end
	end
...
--			local thisResolution = GameInfo[t][id];
			local thisResolution = ShowInPediaLookup(t, id);
...
--			local thisLeagueProject = GameInfo[t][id];
			local thisLeagueProject = ShowInPediaLookup(t, id);
...
--				local tReward = GameInfo.LeagueProjectRewards[reward];
				local tReward = ShowInPediaLookup("LeagueProjectRewards", reward);
				if (tReward ~= nil) then
...
function ClearArticle()
	...
	Controls.CrimeFrame:SetHide( true );
	...
	Controls.DiseaseFrame:SetHide( true );
	Controls.DefenseFrame:SetHide( true );
	...
	Controls.CrimeModFrame:SetHide( true );
	Controls.LoyaltyFrame:SetHide( true );
	Controls.PowerFrame:SetHide( true );
