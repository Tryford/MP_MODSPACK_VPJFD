...
function OnDisplay()
	...
	if minorPlayer:HasStateReligion() then
		...
		Controls.StateReligionLabel:LocalizeAndSetToolTip(minorPlayer:GetStateReligionChanceTT())
...
--			strText = L("TXT_KEY_CITY_STATE_MEETING", strShortDescKey, strGiftString, L("TXT_KEY_MINOR_SPEAK_AGAIN", strShortDescKey) )
	...
	if not isAtWar then
		...
		-- No PtP
			...
			if minorPlayer:CanMajorStartProtection(activePlayerID) then
				...
				strProtectTT = strProtectTT .. minorPlayer:GetPledgeProtectionInvalidReason(iActivePlayer);
	...
	if gk_mode then
		-- PushClaim (Claims)
		if (userSettingRTPClaimsCoreClaims and minorPlayer:GetCapitalCity() and (not activePlayer:IsAtWarWith(minorPlayerID))) then
			local minorPlayerPop = minorPlayer:GetTotalPopulation() or 0
			local iPushClaimCost = minorPlayer:GetBuyoutCost(activePlayerID)
			local numCauses, causesTT = activePlayer:GetClaimCauses(minorPlayerID, true)
			local claimID = activePlayer:GetMinorClaimID(minorPlayerID)
			...
			local claimGoldCost, claimDignitaryCost, claimResistanceCost = activePlayer:GetClaimCost(minorPlayerID, claimID)
			local claimHelp = L(claim.Help, claimDignitaryCost, claimGoldCost, claimResistanceCost, claimHelpCauses)
			local canPushClaim = activePlayer:CanPushMinorClaim(minorPlayerID, claimID)
...
function OnPushClaimButtonClicked()
	...
	local minorPlayer = Players[g_minorCivID]
	local claim = GameInfo.JFD_Claims[activePlayer:GetMinorClaimID(g_minorCivID)]
...
function OnMarriageButtonClicked()
	...
	local pMinor = Players[g_minorCivID];
