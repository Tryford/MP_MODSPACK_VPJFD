...
local userSettingsSDSocietyCulturalVic = (Game.GetUserSetting("JFD_SD_SOCIETY_VICTORY_CHANGES_CULTURAL") == 1)

local isExtraVicConditionsActive = Game.IsExtraVictoryConditionsActive()
if isExtraVicConditionsActive then
-- EVC Start
-- include( "ReligiousVictory" );
	include( "EconomicVictory" );
-- EVC End
end
...
-- EVC Start
-- Religious Details Variables
local g_ReligiousRowsIM = InstanceManager:new( "ReligiousRow", "RowStack", Controls.ReligiousStack );
local g_ReligiousRowList = {};
local g_ReligiousItemIMList = {};
local g_ReligiousItemList = {};

MAX_RELIGIOUS_ICONS_PER_ROW = 8;

-- Economic Details Variables
local g_EconomicRowsIM = nil
local g_EconomicRowList = nil
local g_EconomicItemIMList = nil
local g_EconomicItemList = nil
local g_EconomicIM = nil
local g_EconomicList = nil

if isExtraVicConditionsActive then
	g_EconomicRowsIM = InstanceManager:new( "EconomicRow", "RowStack", Controls.EconomicStack );
	g_EconomicRowList = {};
	g_EconomicItemIMList = {};
	g_EconomicItemList = {};
	g_EconomicIM = InstanceManager:new( "EconomicCiv", "Civ", Controls.EconomicCompareStack );
	g_EconomicList = {};
end

MAX_ECONOMIC_ICONS_PER_ROW = 12;

local db = Modding.OpenSaveData(); -- Persisting data will be used to get new victory selection settings
-- EVC End
...
function OnSpaceRaceDetails()
	Controls.RightHandScreen:SetHide(true);
...
-- EVC Start
function OnEconomicDetails()
	Controls.RightHandScreen:SetHide(true);
	Controls.EconomicScreen:SetHide(false);
	Controls.BackButton:SetHide(true);
	Controls.EconomicClose:SetHide(false);
	
	-- Populate Detail Screens
	PopulateEconomicScreen();
end
if isExtraVicConditionsActive then
	Controls.EconomicVictoryDetails:RegisterCallback( Mouse.eLClick, OnEconomicDetails );
-- EVC End
else
	Controls.EconomicBox:SetHide(true)
end
----------------------------------------------------------------
----------------------------------------------------------------
...
-- EVC Start
function OnEconomicClose()
	Controls.RightHandScreen:SetHide(false);
	Controls.EconomicScreen:SetHide(true);
	Controls.BackButton:SetHide(false);
	Controls.EconomicClose:SetHide(true);
	
	-- Delete instanced parts of culture screen
	DeleteEconomicScreen();
end
if isExtraVicConditionsActive then
	Controls.EconomicClose:RegisterCallback( Mouse.eLClick, OnEconomicClose );
-- EVC End
end
...
function PopulateCultural()
	...
	if(cultureVictory ~= nil and PreGame.IsVictory(cultureVictory.ID))then
		...
		if (userSettingsSDSocietyCulturalVic or Game.IsCBOActive()) then
...
-- EVC Start (original code by Barbarian King, adapted for Piety)
local victoryReligiousID = GameInfoTypes["VICTORY_JFD_RELIGIOUS"]
function PopulateReligious()
	-- local isVictoryEnabled = true;
	-- if db.GetValue("EVC_VICTORY_"..religiousVictory.ID) == 0 then
		-- isVictoryEnabled = false;
	-- end
	-- print("Religious Victory: "..tostring(db.GetValue("EVC_VICTORY_"..religiousVictory.ID)));
	if (PreGame.IsVictory(victoryReligiousID) and victoryReligiousID) then
	-- if(religiousVictory ~= nil and isVictoryEnabled) then
		-- print("Religious Victory enabled");
		for i, v in pairs(g_ReligiousItemIMList) do
			v:ResetInstances();
		end
		g_ReligiousRowsIM:ResetInstances();
		
		g_ReligiousRowList = {};
		g_ReligiousItemList = {};
		g_ReligiousItemIMList = {};
		...
		-- Loop through all the civs that have created religions
		for iPlayerLoop = 0, GameDefines.MAX_CIV_PLAYERS-1, 1 do

			local pPlayer = Players[iPlayerLoop];
			
			if(not pPlayer:IsMinorCiv() and pPlayer:IsEverAlive() and pPlayer:HasCreatedReligion()) then
				-- Create new row if one does not alread exist.
				if(curRow == nil) then
					numRows = numRows + 1;
					curRow = g_ReligiousRowsIM:GetInstance();
					g_ReligiousRowList[numRows] = curRow;
					
					numItemIM = numItemIM + 1;
					curItemIM = InstanceManager:new( "ReligiousItem", "IconFrame", curRow.RowStack );
					g_ReligiousItemIMList[numItemIM] = curItemIM;
					
				end
							
				local item = curItemIM:GetInstance();
				g_ReligiousItemList[numItems] = item;
				numItems = numItems + 1;
				...
				-- Get global variables concerning religious victory and progress
				-- local iSpread = 0;
				-- if(g_EVCReligiousSpread ~= nil) then
					-- if(g_EVCReligiousSpread[eReligion] ~= nil) then
						-- iSpread = g_EVCReligiousSpread[eReligion];
					-- end
				-- end

				-- local iFollowers = 0;
				-- if(g_EVCReligiousFollowers ~= nil) then
					-- if(g_EVCReligiousFollowers[eReligion] ~= nil) then
						-- iFollowers = g_EVCReligiousFollowers[eReligion];
					-- end
				-- end

				-- local iMaxFollowers = 0;
				-- if(g_EVCMaxFollowers ~= nil) then
					-- iMaxFollowers = g_EVCMaxFollowers;
				-- end

				-- local iControlledHolyCities = 0;
				-- if(g_EVCReligiousHolyCities ~= nil) then
					-- if(g_EVCReligiousHolyCities[eReligion] ~= nil) then
						-- iControlledHolyCities = g_EVCReligiousHolyCities[eReligion];
					-- end
				-- end

				-- local iMaxHolyCities = 0;
				-- if(g_EVCMaxHolyCities ~= nil) then
					-- iMaxHolyCities = g_EVCMaxHolyCities;
				-- end

				-- Calculate victory progress bar size
				-- local spread = iSpread;
				-- if iSpread >= 70 then
					-- spread = 70;
				-- end
				-- local followers = iFollowers;
				-- if iFollowers >= iMaxFollowers then
					-- followers = iMaxFollowers;
				-- end

				-- local victoryPercent = (spread/70)*0.33 + (followers/iMaxFollowers)*0.33 + (iControlledHolyCities/iMaxHolyCities)*0.34;

				-- Get icon for religion
				IconHookup(religion.PortraitIndex, 48, religion.IconAtlas, item.Icon);

				local width, height = 64,64;	
				
				-- Rather than use the entire size of the control to display percentage, we're only using the area inside the icon frame.
				-- So instead of 0-64, we're using 12-54.
				-- local newHeight = 42 * (1 - victoryPercent) + 12;
				
				-- item.IconDark:SetSizeVal(width, newHeight);
				-- item.IconDark:SetTextureSizeVal(width, newHeight);
				-- item.IconDark:NormalizeTexture();
			
				item.IconFrameGlow:SetHide(victoryPercent ~= 1);

				local activeTeam = Teams[pActivePlayer:GetTeam()];
				local bHasMet = true;
				local statusKey = "";

				-- Determine which tooltip should show
				if(iPlayerLoop ~= iActivePlayer) then
					bHasMet = activeTeam:IsHasMet(pPlayer:GetTeam());
					statusKey = bHasMet and "TXT_KEY_VP_JFD_RELIGIOUS_STATUS" or "TXT_KEY_VP_JFD_RELIGIOUS_STATUS_UNMET";
				else
					statusKey = "TXT_KEY_VP_JFD_RELIGIOUS_STATUS_PLAYER";
				end

				-- local txt = Locale.Lookup(statusKey, religion.IconString, religion.Description, iSpread, iFollowers, iMaxFollowers, iControlledHolyCities, iMaxHolyCities, pPlayer:GetNameKey());
				-- item.IconFrame:SetToolTipString(txt);
			
				curCol = curCol + 1;
				if(curCol >= MAX_RELIGIOUS_ICONS_PER_ROW) then
					curRow = nil;
					curCol = 0;
				end
			end
		...
		Controls.ReligiousStack:CalculateSize();
		Controls.ReligiousStack:ReprocessAnchoring();
		Controls.ReligiousScrollPanel:CalculateInternalSize();

		if (numRows == 0) then
			Controls.NoReligionsFounded:SetHide(false);
		else
			Controls.NoReligionsFounded:SetHide(true);
		end
	
		Controls.ReligiousVictoryProgress:SetHide(false);
		Controls.ReligiousVictoryDisabled:SetHide(true);
	else
		Controls.ReligiousVictoryProgress:SetHide(true)
		Controls.ReligiousVictoryDisabled:SetHide(false)
		Controls.NoReligionsFounded:SetHide(true)
	end
end

function PopulateEconomic()
	--print("Economic Victory screen setup");
	...
	--if(economicVictory ~= nil and PreGame.IsVictory(economicVictory.ID))then
	if(economicVictory ~= nil and bVictoryEnabled) then
		print("Economic Victory enabled");

		local iActivePlayer = Game.GetActivePlayer();
		local pActivePlayer = Players[iActivePlayer];

		-- Accumulated Gold
		local iPlayerGold = g_EVCEconomicGold[iActivePlayer];
		local bDone = false;
		if iPlayerGold >= g_EVCGoldTarget then
			bDone = true;
		end
		local goldStatusKey = bDone and "TXT_KEY_VP_ECONOMIC_GOLD_DONE" or "TXT_KEY_VP_ECONOMIC_GOLD";
		Controls.EconomicVictoryGold:SetText(Locale.Lookup(goldStatusKey, iPlayerGold, g_EVCGoldTarget));

		-- GPT Bar
		Controls.EconomicVictoryGPT:SetText(Locale.Lookup("TXT_KEY_VP_ECONOMIC_GPT", g_EVCGPTTarget));
		local iStreakTurns = g_EVCEconomicGPTStreak[iActivePlayer];
		local fComplete = iStreakTurns/10;
		Controls.GPTBar:SetPercent(fComplete);
		Controls.GPTBar:SetToolTipString(Locale.Lookup("TXT_KEY_VP_ECONOMIC_GPT_TOOLTIP", iStreakTurns));

		-- Luxury list
		Controls.EconomicVictoryLuxuries:SetText(Locale.Lookup("TXT_KEY_VP_ECONOMIC_LUXURIES"));

		for i, v in pairs(g_EconomicItemIMList) do
			v:ResetInstances();
		end
		g_EconomicRowsIM:ResetInstances();
		...
		for k,v in pairs(g_EVCMapNumLuxResources) do
			local iResource = k;
			local resource = GameInfo.Resources[iResource];

			if(curRow == nil) then
				numRows = numRows + 1;
				curRow = g_EconomicRowsIM:GetInstance();
				g_EconomicRowList[numRows] = curRow;
					
				numItemIM = numItemIM + 1;
				curItemIM = InstanceManager:new( "EconomicItem", "IconFrame", curRow.RowStack );
				g_EconomicItemIMList[numItemIM] = curItemIM;
					
			end
							
			local item = curItemIM:GetInstance();
			g_EconomicItemList[numItems] = item;
			numItems = numItems + 1;

			IconHookup(resource.PortraitIndex, 45, resource.IconAtlas, item.Icon);

			local bHasResource = false;
			if pActivePlayer:GetNumResourceTotal(iResource, true) > 0 then
				item.IconDark:SetHide(true);
				bHasResource = true;
			end

			local statusKey = bHasResource and "TXT_KEY_VP_ECONOMIC_LUXURY_TOOLTIP_AQUIRED" or "TXT_KEY_VP_ECONOMIC_LUXURY_TOOLTIP";
			local txt = Locale.Lookup(statusKey, resource.Description);
			item.IconFrame:SetToolTipString(txt);

			curCol = curCol + 1;
			if(curCol >= MAX_ECONOMIC_ICONS_PER_ROW) then
				curRow = nil;
				curCol = 0;
			end
		end
	
		Controls.EconomicVictoryProgress:SetHide(false);
		Controls.EconomicVictoryDisabled:SetHide(true);
	else
		Controls.EconomicVictoryProgress:SetHide(true);
		Controls.EconomicVictoryDisabled:SetHide(false);
	end
end
-- EVC End
...
-- EVC Start
function PopulateEconomicScreen()
	g_EconomicIM:ResetInstances();
	g_EconomicList = {};
	...
	for iPlayer = 0, GameDefines.MAX_CIV_PLAYERS-1, 1 do
		...
		if(not pPlayer:IsMinorCiv() and pPlayer:IsEverAlive()) then
			-- Create Instance
			local controlTable = g_EconomicIM:GetInstance();
			local i = iPlayer + 1;
			g_EconomicList[i] = controlTable;
			...
			-- Accumulated Gold
			local iPlayerGold = g_EVCEconomicGold[iPlayer];
			controlTable.EconomicVictoryGold:SetText(Locale.Lookup("TXT_KEY_VP_ECONOMIC_GOLD", iPlayerGold, g_EVCGoldTarget));

			-- GPT Bar
			controlTable.EconomicVictoryGPT:SetText(Locale.Lookup("TXT_KEY_VP_ECONOMIC_GPT", g_EVCGPTTarget));
			local iStreakTurns = g_EVCEconomicGPTStreak[iPlayer];
			local fComplete = iStreakTurns/10;
			controlTable.GPTBar:SetPercent(fComplete);
			controlTable.GPTBar:SetToolTipString(Locale.Lookup("TXT_KEY_VP_ECONOMIC_GPT_TOOLTIP", iStreakTurns));

			-- Luxury list
			controlTable.EconomicVictoryLuxuries:SetText(Locale.Lookup("TXT_KEY_VP_ECONOMIC_LUXURIES"));
			controlTable.LuxuryStack:DestroyAllChildren();
			
			local LuxuryRowsIM = InstanceManager:new( "EconomicRow", "RowStack", controlTable.LuxuryStack );
			local LuxuryRowList = {};
			local LuxuryItemIMList = {};
			local LuxuryItemList = {};
			...
			for k,v in pairs(g_EVCMapNumLuxResources) do
				...
				if(curRow == nil) then
					numRows = numRows + 1;
					curRow = LuxuryRowsIM:GetInstance();
					LuxuryRowList[numRows] = curRow;
					
					numItemIM = numItemIM + 1;
					curItemIM = InstanceManager:new( "EconomicItem", "IconFrame", curRow.RowStack );
					LuxuryItemIMList[numItemIM] = curItemIM;
					
				end
							
				local item = curItemIM:GetInstance();
				LuxuryItemList[numItems] = item;
				numItems = numItems + 1;
	...
	Controls.EconomicCompareStack:CalculateSize();
	Controls.EconomicCompareStack:ReprocessAnchoring();
	Controls.EconomicScrollPanel:CalculateInternalSize();
end
-- EVC End
