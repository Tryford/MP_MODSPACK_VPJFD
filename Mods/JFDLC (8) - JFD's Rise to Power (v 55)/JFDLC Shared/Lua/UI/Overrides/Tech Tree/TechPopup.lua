    ...
    if (not Game.IsOption(GameOptionTypes.GAMEOPTION_NO_SCIENCE)) then
 		for tech in GameInfo.Technologies() do
 			...
 			if player:CanResearch( techID ) then
 				...
 				-- end
				local techInfo = {};
				local description = Locale.ConvertTextKey( tech.Description )
				techInfo.Desc = description;
				techInfo.ID = techID;
				techs[numTechs] = techInfo;
				numTechs = numTechs + 1;
			end
		end
	end
	table.sort(techs, function(a, b) return a.Desc < b.Desc end);
...
function AddTechButton( tech, turnsLeft, iDiscover)
	...
	if userSettingPITInnovationsCore then
		...
		if tech.Innovations > 0 then
			if IconHookup( tech.PortraitIndex, 64, tech.IconAtlas, thisTechButtonInstance.InnovationTechPortrait ) then
				thisTechButtonInstance.InnovationTechPortrait:SetHide( false );
			else
				thisTechButtonInstance.InnovationTechPortrait:SetHide( true );
			end
			local text = ""
			...
			for row in GameInfo.Technology_JFD_Innovations("TechType = '" .. tech.Type .. "' AND Help IS NOT NULL") do
				...
				text = Locale.ConvertTextKey(row.Help, "+" .. scienceBoost .. " [ICON_RESEARCH]");
			end
			thisTechButtonInstance.AvailableInnovationBox:SetHide( false );
			thisTechButtonInstance.AvailableInnovationText:SetText(text);
			thisTechButtonInstance.AvailableInnovationText:LocalizeAndSetToolTip(toolTip);
			thisTechButtonInstance.AvailableInnovationText:SetHide( false );
			thisTechButtonInstance.CurrentInnovationBox:SetHide( false );
			thisTechButtonInstance.CurrentInnovationText:SetText(text);
			thisTechButtonInstance.CurrentInnovationText:LocalizeAndSetToolTip(toolTip);
			thisTechButtonInstance.CurrentInnovationText:SetHide( false );
			thisTechButtonInstance.InnovationProgressMeterInternal:SetPercent( numInnovations/maxInnovations );
			thisTechButtonInstance.InnovationProgressMeterInternal:LocalizeAndSetToolTip(toolTip);
			thisTechButtonInstance.InnovationProgressMeter:SetHide( false );
		else
			thisTechButtonInstance.AvailableInnovationBox:SetHide( true );
			thisTechButtonInstance.AvailableInnovationText:SetHide( true );
			thisTechButtonInstance.CurrentInnovationBox:SetHide( true );
			thisTechButtonInstance.InnovationProgressMeter:SetHide( true );
		end
	else
		thisTechButtonInstance.InnovationProgressMeter:SetHide( true );
		thisTechButtonInstance.AvailableInnovationBox:SetHide( true );
