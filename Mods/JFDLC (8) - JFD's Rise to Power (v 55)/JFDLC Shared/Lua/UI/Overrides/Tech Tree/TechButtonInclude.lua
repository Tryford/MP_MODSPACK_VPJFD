...
function AddSmallButtonsToTechButton( thisTechButtonInstance, tech, maxSmallButtons, textureSize, buttonAlpha )
	if (not buttonAlpha) then buttonAlpha = 1 end
	...
	for buttonNum = 1, maxSmallButtons, 1 do
		...
		thisTechButtonInstance[buttonName]:SetAlpha(buttonAlpha);
	end
	...
	-- add the stuff granted by this tech here
  	for thisUnitInfo in GameInfo.Units(string.format("PreReqTech = '%s'", techType)) do
		...
		if thisUnitInfo.ShowInPedia and validUnitBuilds[thisUnitInfo.Class] == thisUnitInfo.Type then
			local iMinorCivGift = thisUnitInfo.MinorCivGift
			if (iMinorCivGift ~= 1) then
				local unitInfo = {};
				local description = Locale.ConvertTextKey( thisUnitInfo.Description )
				unitInfo.Desc = description;
				unitInfo.ThisUnitInfo = thisUnitInfo;
				units[unitsNum] = unitInfo;
				unitsNum = unitsNum + 1;
			end
		end
 	end
	table.sort(units, function(a, b) return a.Desc < b.Desc end);
	for i, v in ipairs(units) do
		local buttonName = "B"..tostring(buttonNum);
		local thisButton = thisTechButtonInstance[buttonName];
		if thisButton then
			AdjustArtOnGrantedUnitButton( thisButton, v.ThisUnitInfo, textureSize, buttonAlpha );
			buttonNum = buttonNum + 1;
		end
	end
 	
	local buildings = {}
	local buildingsNum = 1
 	for thisBuildingInfo in GameInfo.Buildings{PrereqTech = techType} do
 		-- if this tech grants this player the ability to construct this building
		local buildingClass = GameInfo.BuildingClasses[thisBuildingInfo.BuildingClass]
		if validBuildingBuilds[thisBuildingInfo.BuildingClass] == thisBuildingInfo.Type and (buildingClass.MaxGlobalInstances ~= 1 and buildingClass.MaxPlayerInstances ~= 1) then
			local buildingInfo = {};
			local description = Locale.ConvertTextKey( thisBuildingInfo.Description )
			buildingInfo.Desc = description;
			buildingInfo.ThisBuildingInfo = thisBuildingInfo;
			buildings[buildingsNum] = buildingInfo;
			buildingsNum = buildingsNum + 1;
		end
 	end
	table.sort(buildings, function(a, b) return a.Desc < b.Desc end);
	for i, v in ipairs(buildings) do
		local buttonName = "B"..tostring(buttonNum);
		local thisButton = thisTechButtonInstance[buttonName];
		if thisButton then
			AdjustArtOnGrantedBuildingButton( thisButton, v.ThisBuildingInfo, textureSize, buttonAlpha );
			buttonNum = buttonNum + 1;
		end
	end
	...
	for thisBuildingInfo in GameInfo.Buildings(string.format("PreReqTech = '%s'", techType)) do
 		-- if this tech grants this player the ability to construct this building
		local buildingClass = GameInfo.BuildingClasses[thisBuildingInfo.BuildingClass]
		if validBuildingBuilds[thisBuildingInfo.BuildingClass] == thisBuildingInfo.Type and (buildingClass.MaxGlobalInstances == 1 or buildingClass.MaxPlayerInstances == 1) then
			local buildingInfo = {};
			local description = Locale.ConvertTextKey( thisBuildingInfo.Description )
			buildingInfo.Desc = description;
			buildingInfo.ThisBuildingInfo = thisBuildingInfo;
			wonders[wondersNum] = buildingInfo;
			wondersNum = wondersNum + 1;
		end
 	end
	table.sort(wonders, function(a, b) return a.Desc < b.Desc end);
	for i, v in ipairs(wonders) do
 	...
 	for thisResourceInfo in GameInfo.Resources(string.format("TechReveal = '%s'", techType)) do
		...
		local thisButton = thisTechButtonInstance[buttonName];
		if thisButton then
			AdjustArtOnGrantedResourceButton( thisButton, thisResourceInfo, textureSize, buttonAlpha );
			buttonNum = buttonNum + 1;
		end
 	end
 	...
 	for thisProjectInfo in GameInfo.Projects(string.format("TechPrereq = '%s'", techType)) do
		...
		local thisButton = thisTechButtonInstance[buttonName];
 		if thisButton then
			AdjustArtOnGrantedProjectButton( thisButton, thisProjectInfo, textureSize, buttonAlpha );
 			buttonNum = buttonNum + 1;
 		end
	end
	...
	for row in GameInfo.Processes(processCondition) do
		...
		if thisButton then
			...
			thisButton:SetToolTipString( Locale.ConvertTextKey( row.Strategy ) );
		end
		buttonNum = buttonNum + 1;
	end	
	...
	for thisBuildInfo in GameInfo.Builds{PrereqTech = techType, ShowInTechTree  = 1} do
		if thisBuildInfo.ImprovementType then
			if validImprovementBuilds[thisBuildInfo.ImprovementType] == thisBuildInfo.ImprovementType then
				local buildInfo = {};
				local description = Locale.ConvertTextKey( GameInfo.Improvements[thisBuildInfo.ImprovementType].Description )
				buildInfo.Desc = description;
				buildInfo.ThisBuildInfo = thisBuildInfo;
				builds[buildNum] = buildInfo;
				buildNum = buildNum + 1;
 			end
		else
			local buildInfo = {};
			local description = Locale.ConvertTextKey( thisBuildInfo.Description )
			buildInfo.Desc = description;
			buildInfo.ThisBuildInfo = thisBuildInfo;
			builds[buildNum] = buildInfo;
			buildNum = buildNum + 1;
	...
	table.sort(builds, function(a, b) return a.Desc < b.Desc end);
	for i, v in ipairs(builds) do
		local buttonName = "B"..tostring(buttonNum);
		local thisButton = thisTechButtonInstance[buttonName];
		if thisButton then
			AdjustArtOnGrantedActionButton( thisButton, v.ThisBuildInfo, textureSize, buttonAlpha );
			buttonNum = buttonNum + 1;
		end
	end
	
	-- first to free
	for row in GameInfo.Technologies_JFD_FreeUnitTypes("TechType = '" .. techType .. "'") do
		...
		local buttonName = "B"..tostring(buttonNum);
		local thisButton = thisTechButtonInstance[buttonName];
		local thisButtonStar = thisTechButtonInstance["Star"];
		if thisButton then
			local thisUnitInfo = GameInfo.Units[unitID]
			-- Tooltip
			...
			thisButton:SetToolTipString( Locale.ConvertTextKey( "TXT_KEY_JFD_TECH_FIRST_TO_FREE", thisUnitInfo.Description ) );
			if Game.CountKnownTechNumTeams(GameInfoTypes[row.TechType]) > 0 then
				thisButton:SetAlpha(0.33)
			else
				thisButton:SetAlpha(buttonAlpha)
			end
 			buttonNum = buttonNum + 1;
 		end
	end
	...
	for i,v in pairs(sortedYieldChanges) do
		local civilizationType = GameInfo.Improvements[v[1]].CivilizationType
		if (not civilizationType) or (civilizationType and civilizationType == civType) then 
			local buttonName = "B"..tostring(buttonNum);
			local thisButton = thisTechButtonInstance[buttonName];
	...
	for row in GameInfo.Improvement_TechNoFreshWaterYieldChanges(condition) do
		local civilizationType = GameInfo.Improvements[row.ImprovementType].CivilizationType
		if (not civilizationType) or (civilizationType and civilizationType == civType) then 
			local buttonName = "B"..tostring(buttonNum);
			local thisButton = thisTechButtonInstance[buttonName];
			if thisButton then
	...
	for row in GameInfo.Technologies_JFD_MiscEffects("TechType = '" .. tech.Type .. "'") do
		local buttonName = "B"..tostring(buttonNum);
		local thisButton = thisTechButtonInstance[buttonName];
		if thisButton then
			IconHookup(row.PortraitIndex, textureSize, row.IconAtlas, thisButton );
			thisButton:SetHide( false );
			thisButton:SetToolTipString( Locale.ConvertTextKey( row.EffectToolTip ) );
			buttonNum = buttonNum + 1;
		end
	end
	
	if tech.ExtraWorkBoatBuildCharges > 0 then
		local buttonName = "B"..tostring(buttonNum);
		local thisButton = thisTechButtonInstance[buttonName];
		if thisButton then
			IconHookup( 0, textureSize, "GENERIC_FUNC_ATLAS", thisButton );
			thisButton:SetHide( false );
			thisButton:SetToolTipString( Locale.ConvertTextKey( "TXT_KEY_JFD_EXTRA_WORKBOAT_CHARGES", tech.ExtraWorkBoatBuildCharges ) );
			buttonNum = buttonNum + 1;
		end
	end
	...
	if (Game.IsCIVDiploActive() and tech.MapTrading) then
		local buttonName = "B"..tostring(buttonNum);
		local thisButton = thisTechButtonInstance[buttonName];
		if thisButton then
			IconHookup( 0, textureSize, "GENERIC_FUNC_ATLAS", thisButton );
	...
	if (tech.ResearchAgreementTradingAllowed and not g_bResearchAgreementTrading) then
		local buttonName = "B"..tostring(buttonNum);
		local thisButton = thisTechButtonInstance[buttonName];
		if thisButton then
			IconHookup( 0, textureSize, "GENERIC_FUNC_ATLAS", thisButton );
	...
	if (Game.IsCIVDiploActive() and tech.VassalageTradingAllowed and (not g_bNoVassalage)) then
		local buttonName = "B"..tostring(buttonNum);
		local thisButton = thisTechButtonInstance[buttonName];
		if thisButton then
			IconHookup( 0, textureSize, "GENERIC_FUNC_ATLAS", thisButton );
...
function AdjustArtOnGrantedUnitButton( thisButton, thisUnitInfo, textureSize, buttonAlpha )
...
function AdjustArtOnGrantedBuildingButton( thisButton, thisBuildingInfo, textureSize, buttonAlpha )
	-- if we have one, update the building (or wonder) picture
	if thisButton then
		...
		thisButton:SetTextureOffset( textureOffset );
		thisButton:SetHide( false );
		if (Game.AnyoneHasWonder(thisBuildingInfo.ID) and GameInfo.BuildingClasses[thisBuildingInfo.BuildingClass].MaxGlobalInstances == 1) then -- JFD Hide Built Wonders
			thisButton:SetAlpha(0.33);
		else
			thisButton:SetAlpha(buttonAlpha);
		end
...
function AdjustArtOnGrantedProjectButton( thisButton, thisProjectInfo, textureSize, buttonAlpha )
...
function AdjustArtOnGrantedResourceButton( thisButton, thisResourceInfo, textureSize, buttonAlpha )
	if thisButton then
...
function AdjustArtOnGrantedActionButton( thisButton, thisBuildInfo, textureSize, buttonAlpha )
	if thisButton then
