...
--include( "JFD_PIT_Utils.lua" );
local activePlayerID = Game.GetActivePlayer()
include("JFD_PIT_InnovationsUtils.lua")
local userSettingCIDInnovationsCore = (Game.GetUserSetting("JFD_PIT_INNOVATIONS_CORE") == 1)
...
local hORTexture = "TechBranchH.dds";
local vORTexture = "TechBranchV.dds";
local ORAlpha = 1
...
function InitialSetup()
	...
	for row in GameInfo.Technology_ORPrereqTechs() do
		...
		if tech and prereq then
			...
			if tech.GridX - prereq.GridX > 1 then
				local hConnection = g_PipeManager:GetInstance();
				hConnection.TechPipeIcon:SetOffsetVal(prereq.GridX*blockSpacingX + blockSizeX + 128, (tech.GridY-5)*blockSpacingY + 12 + extraYOffset);
				hConnection.TechPipeIcon:SetTexture(hORTexture);
				hConnection.TechPipeIcon:SetColor(ORAlpha);
				local size = { x = (tech.GridX-prereq.GridX - 1)*blockSpacingX - 12; y = 42; };
				hConnection.TechPipeIcon:SetSize(size);
			end
			...
			if tech.GridY - prereq.GridY == 1 or tech.GridY - prereq.GridY == -1 then
				local vConnection = g_PipeManager:GetInstance();
				vConnection.TechPipeIcon:SetOffsetVal((prereq.GridX)*blockSpacingX + blockSizeX + 96, ((tech.GridY-5)*blockSpacingY) - (((tech.GridY-prereq.GridY) * blockSpacingY) / 2) + extraYOffset);
				vConnection.TechPipeIcon:SetTexture(vORTexture);
				vConnection.TechPipeIcon:SetAlpha(ORAlpha);
				local size = { x = 32; y = (blockSpacingY / 2) + 8; };
				vConnection.TechPipeIcon:SetSize(size);
			end
	...
	for pipeIndex, thisPipe in pairs(techPipes) do
		...
		if thisPipe.rightConnectionType >= 1 then
			...
			if thisPipe.IsOR then
				startPipe.TechPipeIcon:SetAlpha(ORAlpha);
			end
			...
			if thisPipe.rightConnectionType == 1 then
				...
				if thisPipe.IsOR then
					pipe.TechPipeIcon:SetAlpha(ORAlpha);
				end
...
function RefreshDisplay()
	...
	for thisEra = 0, #eraBlocks, 1  do
		...
		--JFD PIT
		--local progressPercent = JFD_GetEraScienceProgress(activePlayerID, thisEra)
		--local researchNeeded = JFD_GetEraScienceCost(activePlayerID, thisEra)
		--thisEraBlockInstance.EraResearchMeter:SetPercent(progressPercent/researchNeeded)
	end		
	
	g_NeedsFullRefresh = false;
end

function RefreshDisplayOfSpecificTech( tech, overrideFullRefresh)
	...
	if (numFreeTechs > 0) then
 	...
 	if (g_NeedsFullRefresh) or overrideFullRefresh then
	...
	if userSettingCIDInnovationsCore then
		local pTeamTechs = Teams[Game.GetActiveTeam()]:GetTeamTechs()
		local numInnovations, maxInnovations = Players[Game.GetActivePlayer()]:GetNumInnovations(pTeamTechs, tech.ID)
		if tech.Innovations > 0 then
			if IconHookup( tech.PortraitIndex, 64, tech.IconAtlas, thisTechButton.InnovationTechPortrait ) then
				thisTechButton.InnovationTechPortrait:SetHide( false );
			else
				thisTechButton.InnovationTechPortrait:SetHide( true );
			end
			local toolTip = Locale.ConvertTextKey("Innovations: {1_Num}/{2_Num} [ICON_JFD_INNOVATION]", numInnovations, maxInnovations)
			for row in GameInfo.Technology_JFD_Innovations("TechType = '" .. tech.Type .. "' AND Help IS NOT NULL") do
				local scienceBoost = Players[Game.GetActivePlayer()]:GetInnovationScienceBoost(pTeamTechs, tech.ID, row.ScienceBoost)
				toolTip = toolTip .. "[NEWLINE]-------------------------[NEWLINE]";
				toolTip = toolTip .. Locale.ConvertTextKey(row.Help, "+" .. scienceBoost .. " [ICON_RESEARCH]");
			end
			thisTechButton.InnovationProgressMeterInternal:SetPercent( numInnovations/maxInnovations );
			thisTechButton.InnovationProgressMeterInternal:LocalizeAndSetToolTip(toolTip);
			thisTechButton.InnovationProgressMeter:SetHide( false );
			thisTechButton.TechPortrait:SetHide( true );
		else
			thisTechButton.TechPortrait:SetHide( false );
			thisTechButton.InnovationProgressMeter:SetHide( true );
		end
	else
		thisTechButton.InnovationProgressMeter:SetHide( true );
	end
end
--LuaEvents.JFD_RefreshDisplayOfSpecificTech.Add(RefreshDisplayOfSpecificTech)
