...
-- END
		...
		if userSettingCIDHealthCore then
			...
			if targetDisease ~= 0 then
				myBonuses = myBonuses .. "  " .. Locale.Lookup("TXT_KEY_CHOOSE_INTERNATIONAL_TRADE_ROUTE_ITEM_JFD_DISEASE", targetDisease);
			end	
			local originDisease = pOriginPlayer:GetNumDiseaseFromTradeRoute(pOriginCity)
			if originDisease ~= 0 then
				theirBonuses = theirBonuses .. "  " .. Locale.Lookup("TXT_KEY_CHOOSE_INTERNATIONAL_TRADE_ROUTE_ITEM_JFD_DISEASE", originDisease);
			end	
		end
		...
		if userSettingCIDSlaveryCore then
			...
			if targetShackles ~= 0 then
				theirBonuses = theirBonuses .. "  " .. Locale.Lookup("TXT_KEY_CHOOSE_INTERNATIONAL_TRADE_ROUTE_ITEM_JFD_SLAVES", targetShackles);
			end	
			local originShackles = pOriginPlayer:GetNumShacklesFromTradeRoute(pTargetCity, nil)
			if originShackles ~= 0 then
				myBonuses = myBonuses .. "  " .. Locale.Lookup("TXT_KEY_CHOOSE_INTERNATIONAL_TRADE_ROUTE_ITEM_JFD_SLAVES", originShackles);
			end	
		end
		-------------------------------------------
		-- JFDLC 
...
-- CBP
		...
		if isUsingCBO and (pTargetCity ~= nil and pPlayer ~= nil and tradeRoute.Category == 1) then
