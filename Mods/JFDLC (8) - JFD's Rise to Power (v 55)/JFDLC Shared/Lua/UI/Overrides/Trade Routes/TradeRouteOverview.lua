...
function DisplayData()
    ...
    for i,v in ipairs(g_Data) do
		...
		-- JFDLC 
		-------------------------------------------
		local iOriginOwner = v.FromID
		local pOriginPlayer = Players[iOriginOwner]
		local pOriginCity = v.FromCity
		local iTargetOwner = v.ToID
		local pTargetPlayer = Players[iTargetOwner]
		local pTargetCity = v.ToCity
			
		--JFD Health
		if userSettingCIDHealthCore then
			local targetDisease = pOriginPlayer:GetNumDiseaseFromTradeRoute(pTargetCity)
			if(targetDisease > 0) then
				targetCityDisease = fromCitySlaves
				instance.FromDisease:LocalizeAndSetToolTip("TXT_KEY_TRO_COL_DISEASE_FROM_TT", targetDisease);
				instance.FromDisease:SetText("[COLOR_JFD_DISEASE]" .. targetDisease .. "[ENDCOLOR]");					
			end
			local originDisease = pTargetPlayer:GetNumDiseaseFromTradeRoute(pOriginCity)
			if(originDisease > 0) then
				originCityDisease = toCitySlaves
				instance.ToDisease:LocalizeAndSetToolTip("TXT_KEY_TRO_COL_DISEASE_TO_TT", originDisease);	
				instance.ToDisease:SetText("[COLOR_JFD_DISEASE]" .. originDisease .. "[ENDCOLOR]");
			end
		end

		--JFD Slavery
		if userSettingCIDSlaveryCore then
			local targetShackles = pTargetPlayer:GetNumShacklesFromTradeRoute(pOriginCity, nil)
			if(targetShackles > 0) then
				fromSlaves = fromCitySlaves
				instance.FromSlaves:LocalizeAndSetToolTip("TXT_KEY_TRO_COL_SLAVES_FROM_TT", targetShackles);
				instance.FromSlaves:SetText("[COLOR_JFD_SLAVES]" .. targetShackles .. "[ENDCOLOR]");					
			end
			local originShackles = pOriginPlayer:GetNumShacklesFromTradeRoute(pTargetCity, nil)
			if(originShackles > 0) then
				toSlaves = toCitySlaves
				instance.ToSlaves:LocalizeAndSetToolTip("TXT_KEY_TRO_COL_SLAVES_TO_TT", originShackles);	
				instance.ToSlaves:SetText("[COLOR_JFD_SLAVES]" .. originShackles .. "[ENDCOLOR]");
			end
		end
		-------------------------------------------
		-- JFDLC 
