...
local userSettingCIDHappinessCoreExcessProduction	= (Game.GetUserSetting("JFD_CID_HAPPINESS_CORE_EXCESS_INTO_PRODUCTION") == 1)
...
local userSettingMiscCityDistricts			 		= (Game.GetUserSetting("JFDLC_MISC_CITY_DISTRICTS") == 1)
...
local g_CivilianInstanceManager = InstanceManager:new( "ProdButton", "Button", Controls.CivilianButtonStack );
local g_TradeInstanceManager = InstanceManager:new( "ProdButton", "Button", Controls.TradeButtonStack );
...
local g_ButtonSizeProduction = 45
local g_ButtonSizePurchase = 55
local g_UnitNameProductionOffset = 8
local g_UnitNamePurchaseOffset = 8
------------------------------------------------
-- global constants
------------------------------------------------
...
-- EDIT
	...
	-- JFD Slavery
	local isSlave = false
	if ePurchaseEnum == g_PURCHASE_UNIT_SLAVERY then
    ...
    if g_IsProductionMode then
		...
		if (eOrder == OrderTypes.ORDER_TRAIN) then
			if isSlave then
				LuaEvents.JFD_CityPurchasedSlaveUnit(city:GetOwner(), city, iData, player:GetUnitShackleCost(iData))
			elseif isLevy then
				LuaEvents.JFD_CityPurchasedLevyUnit(city:GetOwner(), city, iData, player:GetUnitLevyCost(iData))
			else
...
local civilianHeadingOpen = true;
local tradeHeadingOpen = true;
...
function OnCivilianHeaderSelected()
	civilianHeadingOpen = not civilianHeadingOpen;
	UpdateWindow( GetCurrentCity() );
end

function OnTradeHeaderSelected()
	tradeHeadingOpen = not tradeHeadingOpen;
	UpdateWindow( GetCurrentCity() );
end
...
function UpdateWindow( city )
    g_CivilianInstanceManager:ResetInstances();
    g_TradeInstanceManager:ResetInstances();
	...
	if userSettingCIDHappinessCore then
		if excessHappiness >= excessUnhappiness then
			Controls.Happiness:SetText( "[ICON_HAPPINESS_1][COLOR_HAPPINESS]" .. excessHappiness .. "[ENDCOLOR]" );
		else
			Controls.Happiness:SetText( "[ICON_HAPPINESS_4][COLOR_UNHAPPINESS]" .. excessUnhappiness .. "[ENDCOLOR]" );
		end
		local toolTipHappiness, toolTipResistanceSentiment = player:GetCityHappinessTT(city)
		Controls.Happiness:LocalizeAndSetToolTip( toolTipHappiness .. "[NEWLINE]------------------[NEWLINE]" .. toolTipResistanceSentiment );	
	else
		Controls.Happiness:SetText( "[ICON_HAPPINESS_1][COLOR_HAPPINESS]" .. excessHappiness .. "[ENDCOLOR]" );
		if isUsingCBO then
			Controls.Happiness:LocalizeAndSetToolTip( player:GetCityHappinessTTForCBO(city) );
		end
	end
	--JFD Health
	if userSettingCIDHealthCore then
		...
		local toolTipHealth, toolTipPlagueSentiment = player:GetCityHealthTT(city)
		Controls.Health:LocalizeAndSetToolTip( toolTipHealth .. "[NEWLINE]------------------[NEWLINE]" .. toolTipPlagueSentiment );	
	else
		Controls.Health:SetText( "[COLOR_WHITE][ICON_STRENGTH]" .. city:GetStrengthValue()/100 .. "[ENDCOLOR]" );
	end
	--JFD Loyalty
	if userSettingCIDLoyaltyCore then
		local loyaltyID, loyaltyPercent, loyaltyTT = player:CalculateCityLoyalty(city, true)
	...
	if userSettingCIDCrimeCore then
		local crimeTT = player:GetCityCrimeTT(city, true, true)
		Controls.Crime:SetText( "[ICON_JFD_CRIME_BLACK][COLOR_JFD_CRIME_BLACK]" .. Game.GetRound(player:GetCityCrimeTotal(city)) .. "[ENDCOLOR]" );
		Controls.Crime:LocalizeAndSetToolTip( crimeTT );
	else
    ...
    -- Units
	local canPurchaseLevies = player:CanPurchaseLevies(city)
	local canPurchaseSlaves = player:CanPurchaseSlaves()
	for unit in GameInfo.Units() do
 		local unitID = unit.ID;
 		if g_IsProductionMode then
			...
			if city:CanTrain( unitID, 0, 1 ) then
				...
				elseif unit.Trade then
					table.insert(listTrade, {
					ID = unitID,
					Description = unit.Description,
					DisplayDescription = Locale.Lookup(unit.Description),
					OrderType = OrderTypes.ORDER_TRAIN,
			...
			local costPopulation = player:GetUnitLevyCost(unitID)
 			local costShackles = player:GetUnitShackleCost(unitID)
 				
			local canPurchaseFaith = city:IsCanPurchase(false, false, unitID, -1, -1, YieldTypes.YIELD_FAITH)
			local canPurchaseGold = city:IsCanPurchase(false, false, unitID, -1, -1, YieldTypes.YIELD_GOLD)
			local canPurchasePopulation = (canPurchaseLevies and city:CanTrain( unitID, 0, 1 ) and costPopulation > 0)
			local canPurchaseShackles = (canPurchaseSlaves and (city:CanTrain( unitID, 0, 1 ) or player:IsUnitShacklePurchaseOnly(unitID)) and costShackles > 0)
			if canPurchaseGold or canPurchaseFaith or canPurchasePopulation or canPurchaseShackles then
				local isDisabledFaith = false;
				local isDisabledGold = false;
				local isDisabledPopulation = false;
				local isDisabledShackles = false;
     			-- test w/o visible (ie can train right now)
    			...
    			if (not city:IsCanPurchase(true, true, unitID, -1, -1, YieldTypes.YIELD_GOLD)) then
	 			...
	 			if (not city:IsCanPurchase(true, true, unitID, -1, -1, YieldTypes.YIELD_FAITH)) then
    				isDisabledFaith = true;
				end
				if (city:GetPopulation() < GameDefines["JFD_CID_PROVINCES_LEVY_MIN_POP"] or city:GetGarrisonedUnit() or city:GetResistanceTurns() > 0) then
					isDisabledPopulation = true;
				end
				if player:GetNumShackles() < costShackles then
					isDisabledShackles = true;
				end
				if (not city:CanTrain( unitID, 0 )) and (not unit.IsPurchaseOnlySlave) then
					isDisabledShackles = true;
				end
					
 				local costFaith = city:GetUnitFaithPurchaseCost( unitID, true );
 				local costGold = city:GetUnitPurchaseCost( unitID );
 				if unit.Combat > 0 or unit.RangedCombat > 0 then
					table.insert(listUnits, {
 					ID = unitID,
 					Description = unit.Description,
 					...
 					CostFaith = costFaith,
 					CostGold = costGold,
 					CostPopulation = costPopulation,
 					CostShackles = costShackles,
 					CanPurchaseFaith = canPurchaseFaith,
 					CanPurchaseGold = canPurchaseGold,
 					CanPurchasePopulation = canPurchasePopulation,
 					CanPurchaseShackles = canPurchaseShackles,
 					DisabledGold = isDisabledGold,
 					DisabledFaith = isDisabledFaith,
 					DisabledPopulation = isDisabledPopulation,
 					DisabledShackles = isDisabledShackles,
 					SortPriority = GetUnitSortPriority(unit)
 				});
				elseif unit.Trade then
					table.insert(listTrade, {
	...
	for building in GameInfo.Buildings() do
 		...
 		if g_IsProductionMode then
			...
			local canPurchaseFaith = city:IsCanPurchase(false, false, -1, buildingID, -1, YieldTypes.YIELD_FAITH)
			local canPurchaseGold = city:IsCanPurchase(false, false, -1, buildingID, -1, YieldTypes.YIELD_GOLD)
			if canPurchaseGold or canPurchaseFaith then
				local col = 2;
				local thisBuildingClass = GameInfo.BuildingClasses[building.BuildingClass];
				if thisBuildingClass.MaxGlobalInstances > 0 then
					col = 3;
				...
				local costFaith = city:GetBuildingFaithPurchaseCost( buildingID );
	...
	numCivilianButtons = #listCivilians;
	numTradeButtons = #listTrade;
	...
	table.sort(listCivilians, GenericSort);
	table.sort(listTrade, GenericSort);
	...
	for _, unit in ipairs(listCivilians) do
		if g_IsProductionMode then
			AddProductionButton(unit.ID, unit.Description, unit.OrderType, unit.Cost, 6, unit.Disabled, unit.YieldType);
		else
			AddPurchaseButton(unit.ID, unit.Description, unit.OrderType, unit.CostFaith, unit.CostGold, unit.CostPopulation, unit.CostShackles, 6, unit.CanPurchaseFaith, unit.CanPurchaseGold, unit.CanPurchasePopulation, unit.CanPurchaseShackles, unit.DisabledFaith, unit.DisabledGold, unit.DisabledPopulation, unit.DisabledShackles); 
		end
	end
	for _, unit in ipairs(listTrade) do
		if g_IsProductionMode then
			AddProductionButton(unit.ID, unit.Description, unit.OrderType, unit.Cost, 7, unit.Disabled, unit.YieldType);
		else
			AddPurchaseButton(unit.ID, unit.Description, unit.OrderType, unit.CostFaith, unit.CostGold, unit.CostPopulation, unit.CostShackles, 7, unit.CanPurchaseFaith, unit.CanPurchaseGold, unit.CanPurchasePopulation, unit.CanPurchaseShackles, unit.DisabledFaith, unit.DisabledGold, unit.DisabledPopulation, unit.DisabledShackles); 
		end
	end
	for _, unit in ipairs(listUnits) do
		if g_IsProductionMode then
			...
			AddPurchaseButton(unit.ID, unit.Description, unit.OrderType, unit.CostFaith, unit.CostGold, unit.CostPopulation, unit.CostShackles, 1, unit.CanPurchaseFaith, unit.CanPurchaseGold, unit.CanPurchasePopulation, unit.CanPurchaseShackles, unit.DisabledFaith, unit.DisabledGold, unit.DisabledPopulation, unit.DisabledShackles); 
		end
	end
	for _, building in ipairs(listBuildings) do
		if g_IsProductionMode then
			...
			AddPurchaseButton(building.ID, building.Description, building.OrderType, building.CostFaith, building.CostGold, building.CostPopulation, building.CostShackles, 2, building.CanPurchaseFaith, building.CanPurchaseGold, building.CanPurchasePopulation, building.CanPurchaseShackles, building.DisabledFaith, building.DisabledGold, building.DisabledPopulation, building.DisabledShackles); 
		end
	end
	for _ , nationalWonder in ipairs(listNationalWonders) do
		if g_IsProductionMode then
			AddProductionButton(nationalWonder.ID, nationalWonder.Description, nationalWonder.OrderType, nationalWonder.Cost, 4, nationalWonder.Disabled, nationalWonder.YieldType);
		else
			AddPurchaseButton(nationalWonder.ID, nationalWonder.Description, nationalWonder.OrderType, nationalWonder.CostFaith, nationalWonder.CostGold, nationalWonder.CostPopulation, nationalWonder.CostShackles, 4, nationalWonder.CanPurchaseFaith, nationalWonder.CanPurchaseGold, nationalWonder.CanPurchasePopulation, nationalWonder.CanPurchaseShackles, nationalWonder.DisabledFaith, nationalWonder.DisabledGold, nationalWonder.DisabledPopulation, nationalWonder.DisabledShackles); 
		end
	end
	for _ , wonder in ipairs(listWonders) do
		if g_IsProductionMode then
			...
			AddPurchaseButton(wonder.ID, wonder.Description, wonder.OrderType, wonder.CostFaith, wonder.CostGold, wonder.CostPopulation, wonder.CostShackles, 3, wonder.CanPurchaseFaith, wonder.CanPurchaseGold, wonder.CanPurchasePopulation, wonder.CanPurchaseShackles, wonder.DisabledFaith, wonder.DisabledGold, wonder.DisabledPopulation, wonder.DisabledShackles); 
		end
	end
	
	if civilianHeadingOpen then
		local localizedLabel = "[ICON_MINUS] "..Locale.ConvertTextKey( "TXT_KEY_POP_CIVILIAN_UNITS" );
		Controls.CivilianButtonLabel:SetText(localizedLabel);
		Controls.CivilianButtonStack:SetHide( false );
	else
		local localizedLabel = "[ICON_PLUS] "..Locale.ConvertTextKey( "TXT_KEY_POP_CIVILIAN_UNITS" );
		Controls.CivilianButtonLabel:SetText(localizedLabel);
		Controls.CivilianButtonStack:SetHide( true );
	end
	if numCivilianButtons > 0 then
		Controls.CivilianButton:SetHide( false );
	else
		Controls.CivilianButton:SetHide( true );
		Controls.CivilianButton:SetHide( true );
	end
	Controls.CivilianButton:RegisterCallback( Mouse.eLClick, OnCivilianHeaderSelected );
	
	if tradeHeadingOpen then
		local localizedLabel = "[ICON_MINUS] "..Locale.ConvertTextKey( "TXT_KEY_POP_TRADE_UNITS" );
		Controls.TradeButtonLabel:SetText(localizedLabel);
		Controls.TradeButtonStack:SetHide( false );
	else
		local localizedLabel = "[ICON_PLUS] "..Locale.ConvertTextKey( "TXT_KEY_POP_TRADE_UNITS" );
		Controls.TradeButtonLabel:SetText(localizedLabel);
		Controls.TradeButtonStack:SetHide( true );
	end
	if numTradeButtons > 0 then
		Controls.TradeButton:SetHide( false );
	else
		Controls.TradeButton:SetHide( true );
		Controls.TradeButton:SetHide( true );
	end
	Controls.TradeButton:RegisterCallback( Mouse.eLClick, OnTradeHeaderSelected );
	...
	Controls.CivilianButtonStack:CalculateSize();
	Controls.CivilianButtonStack:ReprocessAnchoring();
...
function AddProductionButton( id, description, orderType, turnsLeft, column, isDisabled, ePurchaseYield)	
	...
	if column == 1 or column == 6 or column == 7 then -- we are a unit
		iUnit = id;
		if column == 6 then
			controlTable = g_CivilianInstanceManager:GetInstance();
		elseif column == 7 then
			controlTable = g_TradeInstanceManager:GetInstance();
		else
	...
	elseif column == 2 or column == 3 or column == 4 then -- we are a building, wonder, or project
		if column == 2 then
    ...
    if g_IsProductionMode then
		...
		controlTable.UnitName:SetOffsetY( g_UnitNameProductionOffset );
		
		controlTable.Button:SetSizeY(g_ButtonSizeProduction)
		controlTable.DisabledBox:SetSizeY(g_ButtonSizeProduction)
		controlTable.PurchaseStack:SetHide(true)
		controlTable.NumTurns:SetHide(false)
		controlTable.Mouseover:SetHide(false)
	else
		controlTable.Button:SetSizeY(g_ButtonSizePurchase)
		controlTable.DisabledBox:SetSizeY(g_ButtonSizePurchase)
		controlTable.PurchaseStack:SetHide(false)
		controlTable.NumTurns:SetHide(true)
		controlTable.Mouseover:SetHide(true)
	end
...
function AddPurchaseButton( id, description, orderType, costFaith, costGold, costPopulation, costShackles, column, canPurchaseFaith, canPurchaseGold, canPurchasePopulation, canPurchaseShackles, isDisabledFaith, isDisabledGold, isDisabledPopulation, isDisabledShackles)	
	local controlTable;
	local pCity = GetCurrentCity();
	if (pCity == nil) then
		return;
	...
	if column == 1 or column == 6 or column == 7 then -- we are a unit
		...
		controlTable.ProductionButtonImage:SetTexture(textureSheet);
		controlTable.ProductionButtonImage:SetTextureOffset(textureOffset);
		controlTable.UnitName:SetOffsetY( g_UnitNamePurchaseOffset );
		...
		-- Disabled help text
		local strDisabledFaith = ""
		local strDisabledGold = ""
		local strDisabledPopulation = ""
		local strDisabledShackles = ""
		if (isDisabledGold) then
			strDisabledGold = pCity:GetPurchaseUnitTooltip(id);
			if (strDisabledGold ~= nil and strDisabledGold ~= "") then
				strDisabledGold = strToolTip .. "[NEWLINE]----------------[COLOR_WARNING_TEXT]" .. strDisabledGold .. "[ENDCOLOR]";
			end
		end
		if (isDisabledFaith) then
			strDisabledFaith = pCity:GetFaithPurchaseUnitTooltip(id);
			if (strDisabledFaith ~= nil and strDisabledFaith ~= "") then
				strDisabledFaith = strToolTip .. "[NEWLINE]----------------[COLOR_WARNING_TEXT]" .. strDisabledFaith .. "[ENDCOLOR]";
			end
		end
		if (isDisabledPopulation) then
			local minPopReq = GameDefines["JFD_CID_PROVINCES_LEVY_MIN_POP"]
			...
			if (strDisabledInfo ~= nil and strDisabledInfo ~= "") and pCity:GetPopulation() < minPopReq then
				strDisabledPopulation = strDisabledPopulation .. "[NEWLINE]----------------[COLOR_WARNING_TEXT]" .. strDisabledInfo .. "[ENDCOLOR]";
			end
			if pCity:GetGarrisonedUnit() then
				strDisabledPopulation = strDisabledPopulation .. "[NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_PURCHASE_JFD_LEVIES_INFO_GARRISON") 
			end
			strDisabledPopulation = strToolTip .. strDisabledPopulation
		else
			strDisabledPopulation = strToolTip .. "[NEWLINE]----------------" .. Locale.ConvertTextKey("TXT_KEY_PURCHASE_JFD_LEVIES_INFO") 
		end
		if (isDisabledShackles) then
			if pCity:CanTrain( id, 0) then
				strDisabledShackles = strToolTip .. "[NEWLINE]----------------[COLOR_WARNING_TEXT]" .. Locale.ConvertTextKey("TXT_KEY_CANNOT_PURCHASE_JFD_NO_SLAVES") .. "[ENDCOLOR]";
			else
				for row in GameInfo.Unit_ResourceQuantityRequirements("UnitType = '" .. thisUnitInfo.Type .. "'") do
					...
					strDisabledShackles = strToolTip .. "[NEWLINE][COLOR_WARNING_TEXT]" .. Locale.ConvertTextKey("TXT_KEY_NO_ACTION_UNIT_LACKS_RESOURCES", row.Cost, resource.IconString, resource.Description) .. "[ENDCOLOR]";
				end
			end
		else
			if userSettingCIDHappinessCore then
				strDisabledShackles = strToolTip .. "[NEWLINE]----------------" .. Locale.ConvertTextKey("TXT_KEY_PURCHASE_JFD_SLAVES_UNHAPPINESS") .. "[ENDCOLOR]"
			end
		end
		controlTable.Button:SetToolTipString(strToolTip);
		controlTable.GoldButton:SetToolTipString(strDisabledGold);
		controlTable.FaithButton:SetToolTipString(strDisabledFaith);
		controlTable.PopulationButton:SetToolTipString(strDisabledPopulation);
		controlTable.ShacklesButton:SetToolTipString(strDisabledShackles);
		controlTable.FaithButton:SetDisabled(isDisabledFaith)
		controlTable.GoldButton:SetDisabled(isDisabledGold)
		controlTable.PopulationButton:SetDisabled(isDisabledPopulation)
		controlTable.ShacklesButton:SetDisabled(isDisabledShackles)
		controlTable.FaithButton:SetHide(not canPurchaseFaith)
		controlTable.GoldButton:SetHide(not canPurchaseGold)
		controlTable.PopulationButton:SetHide(not canPurchasePopulation)
		controlTable.ShacklesButton:SetHide(not canPurchaseShackles)
	elseif column == 2 or column == 3 or column == 4 then -- we are a building, wonder, or project
		if column == 2 then
			controlTable = g_BuildingInstanceManager:GetInstance();
		elseif column == 3 then
		...
		if orderType == OrderTypes.ORDER_MAINTAIN then
			...
			-- Disabled help text
			local strDisabledFaith = pCity:GetFaithPurchaseBuildingTooltip(id);
			local strDisabledGold = pCity:GetPurchaseBuildingTooltip(id);
			if (strDisabledFaith) then
				if (strDisabledFaith ~= nil and strDisabledFaith ~= "") then
					strDisabledFaith = strToolTip .. "[NEWLINE][COLOR_WARNING_TEXT]" .. strDisabledFaith .. "[ENDCOLOR]";
				end
			end
			if (strDisabledGold) then
				if (strDisabledGold ~= nil and strDisabledGold ~= "") then
					strDisabledGold = strToolTip .. "[NEWLINE][COLOR_WARNING_TEXT]" .. strDisabledGold .. "[ENDCOLOR]";
				end
			end
    ...
    controlTable.UnitName:SetText( nameString );
    if costFaith > 0 then
		controlTable.FaithButton:SetText( costFaith.." [ICON_PEACE]" );
	end
	if costGold > 0 then
		if userSettingCIDInvestmentsCore and (column == 2 or column == 3 or column == 4) then
			controlTable.UnitName:SetOffsetY( g_UnitNamePurchaseOffset );
			controlTable.GoldButton:SetText( costGold.." [ICON_INVEST]" );
		else
			controlTable.GoldButton:SetText( costGold.." [ICON_GOLD]" );
		end
	end
	if costPopulation and costPopulation > 0 then
		controlTable.PopulationButton:SetText( costPopulation.." [ICON_CITIZEN]" );
	end
	if costShackles and costShackles > 0 then
		controlTable.ShacklesButton:SetText( costShackles.." [ICON_JFD_SHACKLES]" );
	end
	controlTable.Button:SetSizeY(g_ButtonSizePurchase)
	controlTable.DisabledBox:SetSizeY(g_ButtonSizePurchase)
	controlTable.PurchaseStack:SetHide(false)
	...
	if (orderType == OrderTypes.ORDER_TRAIN) then
		controlTable.ShacklesButton:SetVoid1( g_PURCHASE_UNIT_SLAVERY );
		controlTable.ShacklesButton:SetVoid2( id );
		controlTable.GoldButton:SetVoid1( g_PURCHASE_UNIT_GOLD );
		controlTable.GoldButton:SetVoid2( id );
		controlTable.FaithButton:SetVoid1( g_PURCHASE_UNIT_FAITH );
		controlTable.FaithButton:SetVoid2( id );
		controlTable.PopulationButton:SetVoid1( g_PURCHASE_UNIT_LEVIES );
		controlTable.PopulationButton:SetVoid2( id );
	elseif (orderType == OrderTypes.ORDER_CONSTRUCT) then
		controlTable.GoldButton:SetVoid1( g_PURCHASE_BUILDING_GOLD );
		controlTable.GoldButton:SetVoid2( id );
		controlTable.FaithButton:SetVoid1( g_PURCHASE_BUILDING_FAITH );
		controlTable.FaithButton:SetVoid2( id );
	end
	controlTable.Button:RegisterCallback( Mouse.eRClick, GetProdHelp );
	controlTable.Button:ClearCallback(Mouse.eLClick);
	...
	if(isDisabled) then
		...
		controlTable.GoldButton:RegisterCallback( Mouse.eLClick, ProductionSelected );
		controlTable.FaithButton:RegisterCallback( Mouse.eLClick, ProductionSelected );
		controlTable.PopulationButton:RegisterCallback( Mouse.eLClick, ProductionSelected );
		controlTable.ShacklesButton:RegisterCallback( Mouse.eLClick, ProductionSelected );
		controlTable.DisabledBox:SetHide(true);
	end
...
function CouldConstruct(pCity, iBuilding)
  ...
  if (userSettingCIDProvincesCore and building.IsProvince) then
	if pCity:IsCapital() then return false end
	if pCity:IsHasBuilding(iBuilding) then return false end
