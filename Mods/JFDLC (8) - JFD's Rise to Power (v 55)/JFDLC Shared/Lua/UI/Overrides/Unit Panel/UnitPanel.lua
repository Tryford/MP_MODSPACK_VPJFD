...
function UpdateUnitActions( unit )
    ...
    do
        local action = GameInfoActions[iAction];
		local element = {};
		local desc = action.TextKey or action.Type or "Action"..(buttonIndex - 1);
		local name = Locale.ConvertTextKey( desc )
		element.id = iAction;
		element.name = name;
		element.order = action.OrderPriority;
		actions[actionCount] = element;
		actionCount = actionCount + 1;
	end	 
	table.sort(actions, function(a, b) return a.order > b.order end);
	for _, thisAction in pairs(actions) do
   -- loop over all the game actions 
   -- for iAction = 0, #GameInfoActions, 1 
    -- do
        local iAction = thisAction.id
        local action = GameInfoActions[iAction];
...
function UpdateUnitPromotions(unit)
    ...
    for unitPromotion in GameInfo.UnitPromotions() do
		local isHide = false
		for row in GameInfo.UnitPromotions_Hide("PromotionType = '" .. unitPromotion.Type .. "' AND UnitPanel = 1") do
			isHide = true
			break
		end
	    local unitPromotionID = unitPromotion.ID;
        if (unit:IsHasPromotion(unitPromotionID) and not unit:IsTrade() and (not isHide)) then
...
function UpdateUnitPromotionsCondensed(unit)
  ...
  for unitPromotion in GameInfo.UnitPromotions() do
	  ...
	  if (unit:IsHasPromotion(unitPromotion.ID) and (not isHide)) then
...
function UpdateUnitStats(unit)
	...
	if userSettingPITProgressBuildCharges then
		local numCharges, numChargesMax = activePlayer:GetNumUnitBuildCharges(unit, unit:GetUnitType()) 
		if numCharges > 0 then
			Controls.UnitStatNameCharges:LocalizeAndSetText("TXT_KEY_UPANEL_JFD_CHARGES");
			Controls.UnitStatCharges:LocalizeAndSetText("TXT_KEY_UPANEL_JFD_CHARGES_STATS", numCharges, numChargesMax);
			Controls.UnitStatCharges:LocalizeAndSetToolTip("TXT_KEY_UPANEL_JFD_CHARGES_TT", numCharges, numChargesMax);
			Controls.UnitStatNameCharges:LocalizeAndSetToolTip("TXT_KEY_UPANEL_JFD_CHARGES_TT", numCharges, numChargesMax);
			Controls.BuildChargesBox:SetHide(false)
		else
			Controls.BuildChargesBox:SetHide(true)
		end
	else
		Controls.BuildChargesBox:SetHide(true)
	end
	...
	if userSettingSDTalentsCore then
		local numTalents, numTalentsMax = activePlayer:GetNumTalents(unit, unit:GetUnitType()) 
		if numTalents > 0 then
			Controls.UnitStatNameTalents:LocalizeAndSetText("TXT_KEY_UPANEL_JFD_TALENTS");
			Controls.UnitStatTalents:LocalizeAndSetText("TXT_KEY_UPANEL_JFD_TALENTS_STATS", numTalents, numTalentsMax);
			Controls.UnitStatTalents:LocalizeAndSetToolTip("TXT_KEY_UPANEL_JFD_TALENTS_TT", numTalents, numTalentsMax);
			Controls.UnitStatNameTalents:LocalizeAndSetToolTip("TXT_KEY_UPANEL_JFD_TALENTS_TT", numTalents, numTalentsMax);
			Controls.TalentsBox:SetHide(false)
		else
			Controls.TalentsBox:SetHide(true)
		end
	else
		Controls.TalentsBox:SetHide(true)
	end
	-- Distance
	if (userSettingCIDColoniesCore and userSettingCIDColoniesDistanceRadius <= 3) then
		if (activePlayer:CanFoundColonies() and unit:IsFoundAbroad() or unit:IsFound()) then
			local distance, distanceThreshold = activePlayer:GetUnitDistance(unit)
			Controls.UnitStatNameDistance:LocalizeAndSetText("TXT_KEY_UPANEL_JFD_DISTANCE");
			if unit:IsUnitClass(GameInfoTypes.UNITCLASS_JFD_COLONIST) then
				Controls.UnitStatDistance:LocalizeAndSetText("TXT_KEY_UPANEL_JFD_DISTANCE_STATS_COLONIST", distance);
				Controls.UnitStatDistance:LocalizeAndSetToolTip("TXT_KEY_UPANEL_JFD_DISTANCE_TT_COLONIST", distance);
				Controls.UnitStatNameDistance:LocalizeAndSetToolTip("TXT_KEY_UPANEL_JFD_DISTANCE_TT_COLONIST", distance);
			else
				Controls.UnitStatDistance:LocalizeAndSetText("TXT_KEY_UPANEL_JFD_DISTANCE_STATS", distance, distanceThreshold);
				Controls.UnitStatDistance:LocalizeAndSetToolTip("TXT_KEY_UPANEL_JFD_DISTANCE_TT", distance, distanceThreshold);
				Controls.UnitStatNameDistance:LocalizeAndSetToolTip("TXT_KEY_UPANEL_JFD_DISTANCE_TT", distance, distanceThreshold);
			end
			Controls.UnitDistanceBox:SetHide(false)
		else
			Controls.UnitDistanceBox:SetHide(true)
		end
	else
		Controls.UnitDistanceBox:SetHide(true)
	end
...
function TipHandler( control )
	...
	local strBuildChargesString = "";
	local strTalentsString = "";
	...
	elseif (action.Type == "MISSION_TRADE") then
		...
		local unitClass = GameInfo.UnitClasses[unit:GetUnitClassType()].Type
		if unitClass == "UNITCLASS_MERCHANT" then
			strToolTip = strToolTip .. "[NEWLINE]" .. Locale.Lookup("TXT_KEY_MISSION_CONDUCT_TRADE_MISSION_HELP");
		else
			strToolTip = strToolTip .. "[NEWLINE]" .. Locale.Lookup("TXT_KEY_MISSION_CONDUCT_JFD_TRADE_MISSION_DIGNITARY_HELP");
		end
		if (not bDisabled) then
			strToolTip = strToolTip .. "[NEWLINE]----------------[NEWLINE]";
	...
	-- Join Council
	elseif (action.Type == "MISSION_JFD_JOIN_COUNCIL") then
		-- Add spacing for all entries after the first
		if (bFirstEntry) then
			bFirstEntry = false;
		elseif (not bFirstEntry) then
		...
		local unitClass = GameInfo.UnitClasses[unit:GetUnitClassType()].Type
		local privyCouncillor = pActivePlayer:GetPrivyCouncillorType(unitClass)
		local privyCouncillorDesc = GameInfo.JFD_PrivyCouncillors[privyCouncillor].Description
		strToolTip = strToolTip .. "[NEWLINE]" .. Locale.Lookup("TXT_KEY_MISSION_JFD_JOIN_COUNCIL_HELP", privyCouncillorDesc);
    -- Help text
	...
	if (bBuild) then
		...
		if userSettingPITProgressBuildCharges then
			local iBuildCharges = pActivePlayer:GetUnitBuildChargeCost(iBuildID, unit:GetUnitType())
			if (iBuildCharges > 0) then
				strBuildChargesString = " ..." .. Locale.ConvertTextKey("TXT_KEY_BUILD_NUM_CHARGES", iBuildCharges);
			end
		end
		...
		do
			...
			if (iYieldChange ~= 0) then
				...
				--JFD
				elseif (iYield == GameInfoTypes["YIELD_JFD_HEALTH"]) then
					strBuildYieldString = strBuildYieldString .. Locale.ConvertTextKey("TXT_KEY_BUILD_HEALTH_STRING", iYieldChange);
				elseif (iYield == GameInfoTypes["YIELD_JFD_DISEASE"]) then
					strBuildYieldString = strBuildYieldString .. Locale.ConvertTextKey("TXT_KEY_BUILD_DISEASE_STRING", iYieldChange);
				end
				
				bFirstYield = false;
			end
		end
		local improvementID = GameInfoTypes[GameInfo.Builds[iBuildID].ImprovementType]
		for yield in GameInfo.Yields("IsFake = 1") do
			local yieldID = yield.ID
			if yieldID then
				local amount = pActivePlayer:CalculateFakeYield(yieldID, pPlot, improvementID) 
				if amount > 0 then
					strBuildYieldString = strBuildYieldString .. "[NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_BUILD_YIELD_STRING", amount, yield.IconString, yield.Description);
				end
			end
		end
	...
	-- Title
    local text = action.TextKey or action.Type or "Action"..(buttonIndex - 1);
    local strTitleString = "[COLOR_POSITIVE_TEXT]" .. Locale.ConvertTextKey( text ) .. "[ENDCOLOR]".. strBuildTurnsString.. strBuildChargesString
    tipControlTable.UnitActionText:SetText( strTitleString );
	local unitClass = GameInfo.UnitClasses[unit:GetUnitClassType()].Type
	--JFD Dignitary hack
	if action.Type == "MISSION_TRADE" and unitClass == "UNITCLASS_JFD_GREAT_DIGNITARY" then
		tipControlTable.UnitActionText:SetText( "[COLOR_POSITIVE_TEXT]" .. Locale.ConvertTextKey("TXT_KEY_MISSION_CONDUCT_JFD_TRADE_MISSION_DIGNITARY_DESC") .. "[ENDCOLOR]");
	end
