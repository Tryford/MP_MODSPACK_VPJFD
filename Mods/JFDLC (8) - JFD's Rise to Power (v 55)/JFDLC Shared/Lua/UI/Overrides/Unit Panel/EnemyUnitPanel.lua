...
function UpdateUnitPromotions(pUnit)
	...
	if pUnit then
		...
		for unitPromotion in GameInfo.UnitPromotions() do
			...
			local isHide = false
			for row in GameInfo.UnitPromotions_Hide("PromotionType = '" .. unitPromotion.Type .. "' AND EnemyUnitPanel = 1") do
				isHide = true
				break
			end
			if ((not isHide) and pUnit:IsHasPromotion(unitPromotionID)) then
...
function UpdateCombatOddsUnitVsCity(pMyUnit, pCity)
	...
	if (pMyUnit ~= nil) then
		...
		if (iMyStrength > 0) then
			...
			for policy in GameInfo.Policies("CityStateCombatModifier <> 0") do
				if pMyPlayer:HasPolicy(policy.ID) then
					iModifier = iModifier + policy.CityStateCombatModifier 
				end
			end
