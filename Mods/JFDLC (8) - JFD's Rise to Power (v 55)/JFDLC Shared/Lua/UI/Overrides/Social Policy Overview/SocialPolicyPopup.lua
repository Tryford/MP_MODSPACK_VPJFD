...
function UpdateDisplay()
	...
	if userSettingSDIdealsCore then
		Controls.ToIdeologyTab:SetOffsetVal(0,51)
		Controls.CultureTrim2:SetHide(false)
		Controls.SwitchIdeologyIdealsBox:SetHide(false)
		Controls.IdeologyBox:SetHide(true)
		Controls.IdealsBox:SetHide(false)
		JFD_UpdateCulturalBox()
    else
		Controls.ToIdeologyTab:SetOffsetVal(0,0)
		Controls.CultureTrim2:SetHide(true)
		Controls.SwitchIdeologyIdealsBox:SetHide(true)
		Controls.IdeologyBox:SetHide(false)
		Controls.IdealsBox:SetHide(true)
	end
	local player = Players[Game.GetActivePlayer()];
	LuaEvents.JFD_Tutorial_PushTutorialPopup(player:GetID(), "IsOnSocialPolicyScreen")
    ...
    if (iDominantBranch ~= -1) then
		...
		local strText = player:GetLeaderTitle();
...
--	if numUnlockedBranches > 0 then
		...
		while policyBranchInfo ~= nil and not policyBranchInfo.PurchaseByLevel do
			...
			local TitleCapName = "TitleCap"..numString;
			...
			local thisTitleCap = Controls[TitleCapName];
			...
			--Policy Branches
			local imageName = "Image" ..numString;
			local imageDisabled = "ImageDisabled" ..numString;
			local imageBack = "BranchBack" ..numString;
			local imageMouseoverName = "ImageMouseover" ..numString;
			local thisImage = Controls[imageName];
			local thisImageDisabled = Controls[imageDisabled];
			local thisImageBack = Controls[imageBack];
			local thisImageMouseover = Controls[imageMouseoverName];
			local currentEraType = GameInfo.Eras[player:GetCurrentEra() ].Type
			local eraImage = nil
			for row in GameInfo.PolicyBranch_JFD_EraImages("PolicyBranchType = '" .. policyBranchInfo.Type .. "' AND EraType = '" .. currentEraType .. "'") do
				eraImage = row.Image
				break
			end
			if eraImage then
				thisImage:SetTexture(eraImage)
				thisImageDisabled:SetTexture(eraImage)
				thisImageBack:SetTexture(eraImage)
				thisImageMouseover:SetTexture(eraImage)
			end
...
--END
			...
			thisImage:LocalizeAndSetToolTip(policyBranchInfo.Help)
			thisImageBack:LocalizeAndSetToolTip(policyBranchInfo.Help)
			thisTitleCap:SetText(Locale.ToUpper(policyBranchInfo.Description))
	...
	while policyInfo ~= nil do
		...
		if (iBranch ~= nil and not GameInfo.PolicyBranchTypes[iBranch].PurchaseByLevel and GameInfo.PolicyBranchTypes[iBranch].FreePolicy) then
...
function OnYes( )
	...
	if GameInfo.Policies[m_gPolicyID].IsIdeal then
		Events.AudioPlay2DSound("AS2D_INTERFACE_POLICY");	
		Events.AudioPlay2DSound("AS2D_SOUND_JFD_IDEAL");	
	else
		Events.AudioPlay2DSound("AS2D_INTERFACE_POLICY");	
	end
...
function JFD_UpdateCulturalBox()
	if g_OpenBox == "Ideology" then
		Controls.IdealsBox:SetHide(true)
		Controls.IdeologyBox:SetHide(false)
		Controls.SwitchIdeologyIdealsBoxLabel:LocalizeAndSetText("TXT_KEY_POLICYSCREEN_SHOW_IDEALS");
		Controls.SwitchIdeologyIdealsBoxLabel:LocalizeAndSetToolTip("TXT_KEY_POLICYSCREEN_SHOW_IDEALS_TT");
	else
		Controls.IdealsBox:SetHide(false)
		Controls.IdeologyBox:SetHide(true)
		Controls.SwitchIdeologyIdealsBoxLabel:LocalizeAndSetText("TXT_KEY_POLICYSCREEN_SHOW_IDEOLOGY");
		Controls.SwitchIdeologyIdealsBoxLabel:LocalizeAndSetToolTip("TXT_KEY_POLICYSCREEN_SHOW_IDEOLOGY_TT");
	end
	
	local idealTag = activePlayer:GetCultureIdealTag()
	local cultureID = activePlayer:GetCultureType()
	local culture = GameInfo.JFD_CultureTypes[cultureID]
	local backgroundImage = "IdealBackground_" .. idealTag .. ".dds"
	Controls.IdealImage:SetTexture(backgroundImage)
	Controls.YourCulture:LocalizeAndSetText(culture.ShortDescription)
	local numButton = 1
	for ideal in GameInfo.JFD_Ideals("IdealTag = '" .. idealTag .. "'") do
		...
		local idealID = ideal.ID
		local idealDesc = Locale.ConvertTextKey(ideal.Description)
		local idealHelp = Locale.ConvertTextKey(ideal.Help)
		local idealPolicyID = GameInfoTypes[ideal.PolicyType]
		if idealPolicyID then
			local cultureCost = activePlayer:GetNextPolicyCost()
			local isIdealDisabled = false
			local isIdealLocked = false
			if activePlayer:HasPolicy(idealPolicyID) then
				idealDesc = "[COLOR_YELLOW][ICON_CHECKBOX]" .. idealDesc .. "[ENDCOLOR]"	
			end		
			local canAdoptIdeal, isNotValid, idealReqHelp = activePlayer:CanAdoptIdeal(idealID, true)
			if activePlayer:HasIdeal(idealID) then
				isIdealDisabled = true
				idealHelp = idealHelp .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_JFD_IDEAL_COMPLETED")
			elseif (isNotValid and ideal.MaxValidEra) then
				idealDesc = "[COLOR_FADING_NEGATIVE_TEXT][ICON_JFD_CROSSBOX]" .. idealDesc .. "[ENDCOLOR]"
				isIdealDisabled = true
				isIdealLocked = true
				idealHelp = idealHelp .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_JFD_IDEAL_CANNOT_UNLOCK_NOT_VALID", GameInfo.Eras[ideal.MaxValidEra].Description)
			elseif (not canAdoptIdeal) then
				idealDesc = "[COLOR_DARK_GREY]" .. idealDesc .. "[ENDCOLOR]"
				isIdealDisabled = true
				isIdealLocked = true
				idealHelp = idealHelp .. "[NEWLINE]------------------[NEWLINE]" .. idealReqHelp
				idealHelp = idealHelp .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_JFD_IDEAL_CANNOT_UNLOCK")
			elseif (activePlayer:GetJONSCulture() < cultureCost and activePlayer:GetNumFreePolicies() <= 0) then
				idealDesc = "[COLOR_DARK_GREY]" .. idealDesc .. "[ENDCOLOR]"
				isIdealDisabled = true
				-- idealHelp = idealHelp .. "[NEWLINE][NEWLINE]" .. idealReqHelp
				idealHelp = idealHelp .. "[NEWLINE]------------------[NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_POLICY_BRANCH_CANNOT_UNLOCK_CULTURE", cultureCost)
				idealHelp = idealHelp .. "[NEWLINE]------------------[NEWLINE]" .. idealReqHelp
			else
				idealHelp = idealHelp .. "[NEWLINE]------------------[NEWLINE]" .. idealReqHelp
			end
			thisButtonLabel:SetText(idealDesc)
			thisButton:SetDisabled(isIdealDisabled)
			thisButtonLock:SetHide(not isIdealLocked)
			thisButton:SetToolTipString(idealHelp)
...
function OnSwitchIdeologyIdealsBox()
	if g_OpenBox == "Ideals" then
		g_OpenBox = "Ideology"
	else
		g_OpenBox = "Ideals"
	end
...
if userSettingSDIdealsCore then
	Controls.SwitchIdeologyIdealsBox:RegisterCallback( Mouse.eLClick, OnSwitchIdeologyIdealsBox );
end
-------------------------------------------------------------------------------
-------------------------------------------------------------------------------
