...
print("Custom Notification Panel loaded -- Original Author: bc1, adapted for CP, CID, and RtP by JFD")
-------------------------------------------------------------------------------
...
for k, v, w in ([[
	...
	-- JFD
	NOTIFICATION_JFD_CHOOSE_GOVERNMENT										ChooseGovernment		popup
	NOTIFICATION_JFD_GOVERNMENT_ACTIVE_PLAYER								GovernmentActivePlayer	
	NOTIFICATION_JFD_DECISION												Decision				popup			
	NOTIFICATION_JFD_EPITHET												Epithet					B					
	NOTIFICATION_JFD_MERCENARY												Mercenary					
	NOTIFICATION_JFD_ORGANIZED_CRIME										OrganizedCrime			B
	NOTIFICATION_JFD_REFORM													Reform					
	NOTIFICATION_JFD_PLAGUE													Plague					B
	NOTIFICATION_JFD_PIETY													Piety					
...
-- todo reset finger animation - requires style modification
		...
		elseif type == NotificationTypes.NOTIFICATION_JFD_GOVERNMENT_ACTIVE_PLAYER then
			...
			if player:HasGovernment() then
				...
				IconHookup( government.PortraitIndex, 80, government.IconAtlas, instance.GovtImage )
			end
	...
	if name then
		...
		if not instance then
			...
			if button then
				...
				if type == NotificationTypes.NOTIFICATION_JFD_CHOOSE_GOVERNMENT then 
					if Players[Game.GetActivePlayer()]:CanHaveAnyGovernment() then
						button:RegisterCallback( Mouse.eLClick, function() LuaEvents.JFD_RTP_Sovereignty_ShowChooseGovernmentPopup() end )
				...
				if type == NotificationTypes.NOTIFICATION_JFD_GOVERNMENT_ACTIVE_PLAYER then
					button:RegisterCallback( Mouse.eLClick, function() LuaEvents.JFD_RTP_Sovereignty_ShowGovernmentOverview() end )
				elseif type == NotificationTypes.NOTIFICATION_JFD_MERCENARY then
					button:RegisterCallback( Mouse.eLClick, function() LuaEvents.JFD_RTP_Mercenaries_ShowMercenaryOverview() end )
				elseif type == NotificationTypes.NOTIFICATION_JFD_DECISION then
					button:RegisterCallback( Mouse.eLClick, function() LuaEvents.JFD_OnEnactDecisionsPopup() end )
				elseif type == NotificationTypes.NOTIFICATION_JFD_REFORM then
					button:RegisterCallback( Mouse.eLClick, function() LuaEvents.JFD_RTP_Sovereignty_ShowGovernmentOverview() end )
				elseif type == NotificationTypes.NOTIFICATION_JFD_PIETY then
					button:RegisterCallback( Mouse.eLClick, function() Events.SerialEventGameMessagePopup{Type = ButtonPopupTypes.BUTTONPOPUP_RELIGION_OVERVIEW,}; end)
				end
			end
		end
		if bundled then
...
function OnEndTurnDirty()
	for thisID, thisType in pairs(g_ActiveNotifications) do
		if (thisType == "ChooseGovernment" and Players[Game.GetActivePlayer()]:HasGovernment()) then
			RemoveNotificationID(thisID);
		end
	end
end
Events.SerialEventGameDataDirty.Add(OnEndTurnDirty)
