...
-- EVC Start
local db = Modding.OpenSaveData(); -- Persisting data will be used to record selection of new victories
-- EVC End
	...
	FullSync = function()
		...
		for row in GameInfo.Victories() do
			...
			-- if row.ID <= 4 then
				victoryCondition.GameOptionRoot:SetCheck(PreGame.IsVictory(row.ID));
			-- else
				-- if db.GetValue("EVC_VICTORY_"..row.ID) == 0 then
					-- victoryCondition.GameOptionRoot:SetCheck(false);
				-- else
					-- victoryCondition.GameOptionRoot:SetCheck(true);
				-- end
			-- end
			...
			-- else
				-- victoryCondition.GameOptionRoot:RegisterCheckHandler( function(bCheck)
					-- db.SetValue("EVC_VICTORY_"..row.ID, bCheck);
					--print("bCheck: "..tostring(bCheck));
					--print("Saved Value: "..tostring(db.GetValue("EVC_VICTORY_"..row.ID)));
				-- end);
			-- end
			-- EVC End
		end
