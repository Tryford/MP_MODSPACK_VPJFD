...
table.insert(g_Interactions.BaseMenu, {
	OnShow =
		function (tInstance, minorID, playerID, tInfo)
			...
			if userSettingRTPClaimsCoreClaims and (not player:IsAtWarWith(minorPlayerID)) then
				local minorPlayerPop = minorPlayer:GetTotalPopulation() or 0
				local iPushClaimCost = minorPlayer:GetBuyoutCost(playerID)
				local numCauses, causesTT = player:GetClaimCauses(minorID, true)
				local claimID = player:GetMinorClaimID(minorID)
				...
				local claimGoldCost, claimDignitaryCost, claimResistanceCost = player:GetClaimCost(minorID, claimID)
				local claimHelp = convertTextKey(claim.Help, claimDignitaryCost, claimGoldCost, claimResistanceCost, claimHelpCauses)
				local canPushClaim = player:CanPushMinorClaim(minorID, claimID)
...
if userSettingRTPClaimsCoreClaims then
