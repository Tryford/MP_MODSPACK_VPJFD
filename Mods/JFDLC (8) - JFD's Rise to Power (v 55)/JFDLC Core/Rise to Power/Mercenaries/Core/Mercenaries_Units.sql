...
SELECT	'UNITCLASS_JFD_GAESATAE',			'UNIT_JFD_GAESATAE',	'TXT_KEY_UNIT_JFD_GAESATAE'
WHERE EXISTS (SELECT * FROM JFD_GlobalUserSettings WHERE Type = 'JFD_RTP_MERCENARIES_CORE' AND Value = 1);
		
INSERT INTO UnitClasses 	
		(Type,								DefaultUnit,			Description)
SELECT	'UNITCLASS_JFD_HESSIAN',			'UNIT_JFD_HESSIAN',		'TXT_KEY_UNIT_JFD_HESSIAN'
WHERE EXISTS (SELECT * FROM JFD_GlobalUserSettings WHERE Type = 'JFD_RTP_MERCENARIES_CORE' AND Value = 1);

INSERT INTO UnitClasses 	
		(Type,								DefaultUnit,			Description)
SELECT	'UNITCLASS_JFD_POTSDAM_GIANT',		'UNIT_JFD_HESSIAN',		'TXT_KEY_UNIT_JFD_POTSDAM_GIANT'
WHERE EXISTS (SELECT * FROM JFD_GlobalUserSettings WHERE Type = 'JFD_RTP_MERCENARIES_CORE' AND Value = 1);
...
--------------------------------------------------------------------------------------------------------------------------
INSERT INTO Units 		
		(Type, 						IsPurchaseOnlyMerc, Class,					 PolicyType,			 Combat,	Cost,	 FaithCost,		PrereqTech,	RequiresFaithPurchaseEnabled, Moves, CombatClass, Domain, DefaultUnitAI, Description, 					Strategy, 								Help, 								Civilopedia,						MilitarySupport, MilitaryProduction, Pillage, IgnoreBuildingDefense, AdvancedStartCost, CombatLimit, ObsoleteTech, XPValueDefense, GoodyHutUpgradeUnitClass, UnitArtInfo, 					UnitFlagIconOffset, UnitFlagAtlas,					PortraitIndex, 	IconAtlas)
...
-- Foreign Legion
UPDATE Units
SET Combat = 50, IsPurchaseOnlyMerc = 1, MoveAfterPurchase = 0, Help = 'TXT_KEY_UNIT_JFD_HELP_FRENCH_FOREIGNLEGION', Strategy = 'TXT_KEY_UNIT_JFD_FRENCH_FOREIGNLEGION_STRATEGY'
WHERE Type = 'UNIT_FRENCH_FOREIGNLEGION'
AND EXISTS (SELECT * FROM JFD_GlobalUserSettings WHERE Type = 'JFD_RTP_MERCENARIES_CORE' AND Value = 1);	

-- Galley
UPDATE Units
SET IsPurchaseOnlyMerc = 1
...
-- Landsneckht
UPDATE Units
SET PrereqTech = NULL, IsPurchaseOnlyMerc = 1, MoveAfterPurchase = 0, Help = 'TXT_KEY_UNIT_JFD_HELP_GERMAN_LANDSKNECHT', Strategy = 'TXT_KEY_UNIT_JFD_GERMAN_LANDSKNECHT_STRATEGY'
...
INSERT INTO JFD_Civilopedia_HideFromPedia 	
		(Type)
SELECT	Type
FROM Units WHERE Type IN ('UNIT_JFD_GAESATAE', 'UNIT_JFD_HESSIAN', 'UNIT_FRENCH_FOREIGNLEGION', 'UNIT_JFD_POTSDAM_GIANT')
AND EXISTS (SELECT * FROM JFD_GlobalUserSettings WHERE Type = 'JFD_RTP_MERCENARIES_CORE' AND Value = 1);
--==========================================================================================================================
--==========================================================================================================================