...
-- USER SETTINGS
--=======================================================================================================================
local userSettingRTPMercenariesCore						= (Game.GetUserSetting("JFD_RTP_MERCENARIES_CORE") == 1)
local userSettingRTPMercenariesCoreAICap				= (Game.GetUserSetting("JFD_RTP_MERCENARIES_CORE_AI_CAP"))
local userSettingRTPMercenariesCoreFirstNotification	= (Game.GetUserSetting("JFD_RTP_MERCENARIES_CORE_FIRST_NOTIFICATION_TURNS"))
local userSettingRTPMercenariesCoreSecondNotification	= (Game.GetUserSetting("JFD_RTP_MERCENARIES_CORE_SECOND_NOTIFICATION_TURNS"))
local userSettingRTPPietyReligiousOrders				= (Game.GetUserSetting("JFD_RTP_PIETY_TECH_ADDITIONS_RELIGIOUS_ORDERS") == 1)
--=======================================================================================================================
-- UTILITIES
--=======================================================================================================================
-- GLOBALS
...
local modSpeed 					   = ((GameInfo.GameSpeeds[Game.GetGameSpeedType()].BuildPercent)/100)
local defineMercenariesExtendTurns = Game.GetRound(GameDefines["JFD_RTP_MERCENARIES_DEFAULT_EXTENSION_TURNS"]*modSpeed)
--=======================================================================================================================
-- AI FUNCTIONS
--=======================================================================================================================
------------------------------------------------------------------------------------------------------------------------
-- CONTRACTS
------------------------------------------------------------------------------------------------------------------------
--JFD_RTP_Mercenaries_PlayerDoTurnAI
function JFD_RTP_Mercenaries_PlayerDoTurnAI(playerID)
	local player = Players[playerID]
	if player:IsHuman() then return end
	if player:IsBarbarian() then return end
	if (not player:IsAlive()) then return end
	if (not player:IsAtWar()) then return end
	if (not player:CanTakeContracts()) then return end
	if player:GetNumActivePlayerContracts() >= userSettingRTPMercenariesCoreAICap then return end
	...
	if (not player:IsMinorCiv()) then
		flavourMercenary = player:GetFlavorValue("FLAVOR_JFD_MERCENARY") 
	end	
	if Game.GetRandom(1,10) <= flavourMercenary then	
		...
		for _, v in ipairs(Game.GetInactiveContractUnitList()) do
			...
			if player:CanHaveContract(contract.ID, contract.UnitID) then
				validContracts[count] = contract.ID
				count = count + 1
			end
		end
		if #validContracts > 0 then
			local contractToTakeID = validContracts[Game.GetRandom(1,#validContracts)]
			if contractToTakeID then player:StartContract(contractToTakeID) end
		end
	end
end
if userSettingRTPMercenariesCore then
	GameEvents.PlayerDoTurn.Add(JFD_RTP_Mercenaries_PlayerDoTurnAI)
end
--=======================================================================================================================
-- CORE FUNCTIONS
--=======================================================================================================================
...
function JFD_RTP_Mercenaries_GetContractDuration(playerID, contractID)
	...
	return Game.GetRound(contractDuraction*modSpeed)
end
if userSettingRTPMercenariesCore then
...
function JFD_RTP_Mercenaries_ContractsUpdated(teamID, eraID)
	...
	for contract in GameInfo.Contracts() do
		...
		if Game.IsContractAvailable(contractID) then
			unitID = player:GetContractUnit(contractID)
		end
...
function JFD_RTP_Mercenaries_ContractedStarted(playerID, contractID, contractDuration, contractMaintenance)
	...
	local contract = GameInfo.Contracts[contractID]
	local contractCost = player:GetContractCosts(contractID)
	...
	for unit in player:Units() do
		if unit:IsSpecificContractUnit(contractID) then
			...
			if (contract.IsSlavery and promotionEnslavedID) then
...
function JFD_RTP_Mercenaries_ContractBroken(playerID, contractID)
	...
	if Game.GetRandom(1,100) <= contract.MutinyChance then
	...
	for contract in GameInfo.Contracts() do
		...
		if player:PlayerHasContract(contractID) then
			if player:IsHuman() then
				...
				if turns == userSettingRTPMercenariesCoreFirstNotification then
				...
				elseif turns == userSettingRTPMercenariesCoreSecondNotification then
					JFD_SendNotification(playerID, "NOTIFICATION_JFD_MERCENARY", convertTextKey("TXT_KEY_JFD_CONTRACTS_NOTIFICATION_IMMINENT_FINISH_CONTRACT", contract.Description, turns), convertTextKey("TXT_KEY_JFD_CONTRACTS_NOTIFICATION_IMMINENT_FINISH_CONTRACT_SHORT"), false, -1, -1)
				end
			end
			if (player:GetGold() <= 0 and player:CalculateGoldRate() <= 0) then
				local contractChance = Game.GetRandom(1,100)
...
function JFD_RTP_Mercenaries_CanTrain(playerID, unitID)
	...
	if (unitID == unitGalleyID and playerID ~= 63) then
		return false
	elseif (unitID == unitGalleyID and (not userSettingRTPMercenariesCore)) then
		return false
	else
		return (not GameInfo.Units[unitID].IsPurchaseOnlyMerc)
	end
	return true
end
...
function JFD_RTP_Mercenaries_PopulateUnitClasses()
	...
	if (resolutionMercenariesID and Game.IsResolutionPassed(resolutionMercenariesID, -1)) then
		...
		if activePlayer:CanTakeContracts() then
			mercenaryUnitsManager:ResetInstances()
				...
				if insertIntoTable then
					if g_MercenaryContractsShowOnly ~= "ShowAll" then
						if g_MercenaryContractsShowOnly == "ShowOnlyHolyOrders" then
							local canHaveContract, canShowContract = activePlayer:CanHaveContract(contractID, unitID)
							if ((not contractInfo.IsHolyOrder) or (not activePlayer:CanTakeContracts(true)) or (not canShowContract)) then
								insertIntoTable = false
							else
								insertIntoTable = true
							end
						elseif (g_MercenaryContractsShowOnly == "ShowOnlyMercenaries" and (not unit.IsPurchaseOnlyMerc)) then
							insertIntoTable = false
						else
...
g_Tabs["MercenaryContracts"].RefreshContent = JFD_RTP_Mercenaries_PopulateUnitClasses
	...
	if g_UnitClassSelectedID then
		...
		for _, v in ipairs(Game.GetInactiveContractUnitList()) do
			...
			if contractUnitClassID == g_UnitClassSelectedID then
				local contractInfo = GameInfo.Contracts[contractID]
				local canHaveContract, canShowContract = activePlayer:CanHaveContract(contractID, contractUnitID)
				local contractCost = activePlayer:GetContractCosts(contractID, contractUnitID)
		...
		if(#mercenaryContracts > 0) then
			...
			for _,contract in ipairs(mercenaryContracts) do
				...
				local canHaveContract, canSeeContract, disabledTT = activePlayer:CanHaveContract(contractID, contractUnitID, false, false, true)
...
function JFD_RTP_Mercenaries_PopulateContractOverview()
	...
	if (not g_ContractSelectedID) or (not g_UnitClassSelectedID) or (not g_UnitSelectedID) then
		...
		if g_UnitSelectedID then
			local contractCost, contractCostBreak, contractCostExtend = activePlayer:GetContractCosts(g_ContractSelectedID, g_UnitSelectedID)
			local contractUnit = GameInfo.Units[g_UnitSelectedID]
			local contractYieldText = contract.YieldText
			local overviewText = activePlayer:GetContractOverview(g_ContractSelectedID, g_UnitSelectedID)
			...
			if activePlayer:PlayerHasContract(g_ContractSelectedID) then
				local canBreakContract, canExtendContract = activePlayer:CanHaveContract(g_ContractSelectedID, g_UnitSelectedID, true, true)
				...
				if Game.IsContractActive(g_ContractSelectedID) then
					...
					local canHaveSelectedContract = activePlayer:CanHaveContract(g_ContractSelectedID, g_UnitSelectedID)
