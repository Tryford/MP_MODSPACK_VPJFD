...
local Event_JFD_RTP_Piety_BadOmen = {}
	...
	Event_JFD_RTP_Piety_BadOmen.Outcomes[1].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local faithCost = player:GetEraAdjustedValue(mathCeil(15*iMod))
			local faithReward = player:GetEraAdjustedValue(mathCeil(25*iMod))
	...
	Event_JFD_RTP_Piety_BadOmen.Outcomes[2].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local growthCost = player:GetEraAdjustedValue(mathCeil(10*iMod))
			local faithReward = player:GetEraAdjustedValue(mathCeil(25*iMod))
	...
	Event_JFD_RTP_Piety_BadOmen.Outcomes[3].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local cultureCost = player:GetEraAdjustedValue(mathCeil(10*iMod))
			local faithReward = player:GetEraAdjustedValue(mathCeil(25*iMod))
	...
	Event_JFD_RTP_Piety_BadOmen.Outcomes[4].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local goldCost = player:GetEraAdjustedValue(mathCeil(40*iMod))
			local faithReward = player:GetEraAdjustedValue(mathCeil(25*iMod))
...
local Event_JFD_RTP_Piety_GoodOmen = {}
	...
	Event_JFD_RTP_Piety_GoodOmen.Outcomes[1].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local goldenAgeReward = player:GetEraAdjustedValue(mathCeil(50*iMod))
	...
	Event_JFD_RTP_Piety_GoodOmen.Outcomes[2].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local faithReward = player:GetEraAdjustedValue(mathCeil(30*iMod))
...
local Event_JFD_RTP_Piety_HeathenGuard = {}
	...
	Event_JFD_RTP_Piety_HeathenGuard.Outcomes[1].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local garrisonedUnit = player:GetCapitalCity():GetGarrisonedUnit()
			local goldReward = player:GetEraAdjustedValue(mathCeil((33*garrisonedUnit:GetLevel())*iMod))
...
local Event_JFD_RTP_Piety_Apocalypse = {}
	...
	Event_JFD_RTP_Piety_Apocalypse.CanFunc = (
		function(player)			
			...
			local pietyCost = player:GetEraAdjustedValue(mathCeil(10 * iMod))
	...
	Event_JFD_RTP_Piety_Apocalypse.Outcomes[1].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local pietyReward = player:GetEraAdjustedValue(mathCeil(9*iMod))
			local faithCost = player:GetEraAdjustedValue(mathCeil(47*iMod))
			if player:GetFaith() < faithCost then return false end
	...
	Event_JFD_RTP_Piety_Apocalypse.Outcomes[2].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local pietyLevelMod = player:GetPietyLevelRewardMod()
			local goldCost = player:GetEraAdjustedValue(mathCeil(76*iMod))
			local pietyReward = player:GetEraAdjustedValue(mathCeil(10*iMod))
			local faithReward = player:GetEraAdjustedValue(mathCeil((30*iMod)*pietyLevelMod))
			if player:GetGold() < goldCost then return false end
	...
	Event_JFD_RTP_Piety_Apocalypse.Outcomes[3].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local pietyCost = player:GetEraAdjustedValue(mathCeil(8*iMod))
			if player:GetPiety() < pietyCost then return false end
...
local Event_JFD_RTP_Piety_Philosopher = {}
	...
	Event_JFD_RTP_Piety_Philosopher.Outcomes[1].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local pietyCost = player:GetEraAdjustedValue(mathCeil(16 * iMod))
			if player:GetPiety() < pietyCost then return false end
			local pietyLevelMod = player:GetPietyLevelRewardMod()
			local cultureReward = player:GetEraAdjustedValue(mathCeil((20*iMod)*pietyLevelMod))
	...
	Event_JFD_RTP_Piety_Philosopher.Outcomes[3].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local cultureCost = player:GetEraAdjustedValue(mathCeil(68*iMod))
			local pietyReward = player:GetEraAdjustedValue(mathCeil(10*iMod))
			local pietyLevelMod = player:GetPietyLevelRewardMod()
			local faithReward = player:GetEraAdjustedValue(mathCeil((30*iMod)*pietyLevelMod))
			if player:GetJONSCulture() < cultureCost then return false end
...
local Event_JFD_RTP_Piety_Corruption = {}
	...
	Event_JFD_RTP_Piety_Corruption.Outcomes[1].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local pietyReward = player:GetEraAdjustedValue(mathCeil(20*iMod))
			local goldCost = player:GetEraAdjustedValue(mathCeil(176*iMod))
			if player:GetGold() < goldCost then return false end
	...
	Event_JFD_RTP_Piety_Corruption.Outcomes[2].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local pietyCost = player:GetEraAdjustedValue(mathCeil(9*iMod))
			local pietyLevelMod = player:GetPietyLevelRewardMod()
			local goldReward = player:GetEraAdjustedValue(mathCeil((30*iMod)*pietyLevelMod))
			if player:GetPiety() < pietyCost then return false end
...
local Event_JFD_RTP_Piety_CourtHeretic = {}
	...
	Event_JFD_RTP_Piety_CourtHeretic.Outcomes[2].CanFunc = (
		function(player)	
			local playerID = player:GetID()
			local pietyCost = player:GetEraAdjustedValue(mathCeil(22*iMod))
			local goldCost = player:GetEraAdjustedValue(mathCeil(63*iMod))
			local pietyLevelMod = player:GetPietyLevelRewardMod()
			local faithReward = player:GetEraAdjustedValue(mathCeil((78*iMod)*pietyLevelMod))
			if player:GetPiety() < pietyCost then return false end
	...
	Event_JFD_RTP_Piety_CourtHeretic.Outcomes[3].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local faithCost = player:GetEraAdjustedValue(mathCeil(66*iMod))
			local pietyReward = player:GetEraAdjustedValue(mathCeil(12*iMod))
			if player:GetFaith() < faithCost then return false end
...
local Event_JFD_RTP_Piety_PaganUprising = {}
	...
	Event_JFD_RTP_Piety_PaganUprising.Outcomes[1].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local pietyReward = player:GetPietyPercent(10, true)
...
local Event_JFD_RTP_Piety_Witchcraft = {}
	...
	Event_JFD_RTP_Piety_Witchcraft.CanFunc = (
		function(player)			
			...
			local pietyCost = player:GetEraAdjustedValue(mathCeil(20*iMod))
			if player:GetPiety() < pietyCost then return false end
			local goldCost = player:GetEraAdjustedValue(mathCeil(206*iMod))
			if player:GetGold() < goldCost then return false end
			local cultureCost = player:GetEraAdjustedValue(mathCeil(43*iMod))
			if player:GetJONSCulture() < cultureCost then return false end
	...
	Event_JFD_RTP_Piety_Witchcraft.Outcomes[1].DoFunc = (
		function(player) 
			...
			local pietyCost = player:GetEraAdjustedValue(mathCeil(20*iMod))
			player:ChangePiety(-pietyCost)
			local reward = player:GetEraAdjustedValue(mathCeil((GetRandom(10,25)*iMod)*pietyLevelMod))
			local yieldType = "TXT_KEY_JFD_SCIENCE"
			local getRandom = GetRandom(1, 4)
			if getRandom == 1 or getRandom == 2 then 
				reward = player:GetEraAdjustedValue(mathCeil((GetRandom(20,150)*iMod)*pietyLevelMod))
				player:ChangeGold(reward) 
				yieldType = "TXT_KEY_JFD_GOLD" 
			elseif getRandom == 3 then 
				reward = player:GetEraAdjustedValue(mathCeil((GetRandom(20, 50)*iMod)*pietyLevelMod))
	...
	Event_JFD_RTP_Piety_Witchcraft.Outcomes[2].CanFunc = (
		function(player)			
			...
			local goldCost = player:GetEraAdjustedValue(mathCeil(206*iMod))
			local pietyReward = player:GetEraAdjustedValue(mathCeil(18*iMod))
	...
	Event_JFD_RTP_Piety_Witchcraft.Outcomes[3].CanFunc = (
		function(player)			
			...
			local cultureCost = player:GetEraAdjustedValue(mathCeil(43*iMod))
			local pietyLevelMod = player:GetPietyLevelRewardMod()
			local faithReward = player:GetEraAdjustedValue(mathCeil(((cultureCost*1.9)*iMod)*pietyLevelMod))
...
local Event_JFD_RTP_Piety_Diaspora = {}
	...
	Event_JFD_RTP_Piety_Diaspora.CanFunc = (
		function(player)	
			...
			if currentEraID < eraMedievalID or currentEraID >= eraModernID then return false end
			if (not player:HasStateReligion()) then return false end
			Event_JFD_RTP_Piety_Diaspora.Weight = player:GetPietyLevelEventWeight()
	...
	Event_JFD_RTP_Piety_Diaspora.Outcomes[1].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local pietyCost = player:GetEraAdjustedValue(mathCeil(11*iMod))
			if player:GetPiety() < pietyCost then return false end
			local pietyLevelMod = player:GetPietyLevelRewardMod()
			local goldReward = player:GetEraAdjustedValue(mathCeil((104* iMod)*pietyLevelMod))
	...
	Event_JFD_RTP_Piety_Diaspora.Outcomes[3].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local pietyCost = player:GetEraAdjustedValue(mathCeil(12*iMod))
			if player:GetPiety() < pietyCost then return false end
			local pietyLevelMod = player:GetPietyLevelRewardMod()
			local cultureReward = player:GetEraAdjustedValue(mathCeil((34*iMod)*pietyLevelMod))
...
local Event_JFD_RTP_Piety_VoiceOfGod = {}
	...
	Event_JFD_RTP_Piety_VoiceOfGod.CanFunc = (
		function(player)			
			...
			if currentEraID < eraMedievalID or currentEraID >= eraModernID then return false end
			if (not player:HasStateReligion()) then return false end
			local faithCost = player:GetEraAdjustedValue(mathCeil(34*iMod))
			if player:GetFaith() < faithCost then return false end
			Event_JFD_RTP_Piety_VoiceOfGod.Weight = player:GetPietyLevelEventWeight()
			return true
		end
		)
	...
	Event_JFD_RTP_Piety_VoiceOfGod.Outcomes[1].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local pietyReward = player:GetEraAdjustedValue(mathCeil(8*iMod))
			local faithCost = player:GetEraAdjustedValue(mathCeil(34*iMod))
			if player:GetFaith() < faithCost then return false end
	...
	Event_JFD_RTP_Piety_VoiceOfGod.Outcomes[2].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local pietyCost = player:GetEraAdjustedValue(mathCeil(15*iMod))
			local pietyLevelMod = player:GetPietyLevelRewardMod()
			local faithReward = player:GetEraAdjustedValue(mathCeil((37*iMod)*pietyLevelMod))
			if player:GetPiety() < pietyCost then return false end
...
local Event_JFD_RTP_Piety_Miracles = {}
	...
	Event_JFD_RTP_Piety_Miracles.CanFunc = (
		function(player)	
			...
			if currentEraID < eraMedievalID or currentEraID >= eraModernID then return false end
			if (not player:HasStateReligion()) then return false end
			Event_JFD_RTP_Piety_Miracles.Weight = player:GetPietyLevelEventWeight()
			local cities = {}
	...
	Event_JFD_RTP_Piety_Miracles.Outcomes[1].CanFunc = (
		function(player)			
			...
			if player:GetPiety() < pietyCost then return false end
			local pietyLevelMod = player:GetPietyLevelRewardMod()
			local faithReward = player:GetEraAdjustedValue(mathCeil(29 * iMod * pietyLevelMod))
	...
	Event_JFD_RTP_Piety_Miracles.Outcomes[2].CanFunc = (
		function(player)			
			...
			if player:GetPiety() < pietyCost then return false end
			local pietyLevelMod = player:GetPietyLevelRewardMod()
			local goldReward = player:GetEraAdjustedValue(mathCeil((47*iMod)*pietyLevelMod))
	...
	Event_JFD_RTP_Piety_Miracles.Outcomes[3].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local faithCost = player:GetEraAdjustedValue(mathCeil(44*iMod))
			local pietyReward = player:GetEraAdjustedValue(mathCeil(12*iMod))
			if player:GetFaith() < faithCost then return false end
...
local Event_JFD_RTP_Piety_Peddler = {}
	...
	Event_JFD_RTP_Piety_Peddler.CanFunc = (
		function(player)	
			...
			Event_JFD_RTP_Piety_Peddler.Weight = player:GetPietyLevelEventWeight()
	...
	Event_JFD_RTP_Piety_Peddler.Outcomes[1].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local goldCost = player:GetEraAdjustedValue(mathCeil(200*iMod))
			if player:GetGold() < goldCost then return false end
...
local Event_JFD_RTP_Piety_HomoPriest = {}
	...
	Event_JFD_RTP_Piety_HomoPriest.Outcomes[1].CanFunc = (
		function(player)			
			...
			if player:GetPiety() < pietyCost then return false end
			local pietyLevelMod = player:GetPietyLevelRewardMod()
			local cultureReward = player:GetEraAdjustedValue(mathCeil((24*iMod)*pietyLevelMod))
	...
	Event_JFD_RTP_Piety_HomoPriest.Outcomes[2].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local cultureCost = player:GetEraAdjustedValue(mathCeil(22*iMod))
			local pietyReward = player:GetEraAdjustedValue(mathCeil(8*iMod))
			if player:GetJONSCulture() < cultureCost then return false end
	...
	Event_JFD_RTP_Piety_HomoPriest.Outcomes[3].CanFunc = (
		function(player)			
			...
			if player:GetPiety() < pietyCost then return false end
			local pietyLevelMod = player:GetPietyLevelRewardMod()
			local faithReward = player:GetEraAdjustedValue(mathCeil((36*iMod)*pietyLevelMod))
...
local Event_JFD_RTP_Piety_BenedictineMonk = {}
	...
	Event_JFD_RTP_Piety_BenedictineMonk.CanFunc = (
		function(player)			
			...
			local pietyCost = player:GetEraAdjustedValue(mathCeil(13*iMod))
			Event_JFD_RTP_Piety_BenedictineMonk.Weight = player:GetPietyLevelEventWeight()
			return true
		end
		)
	...
	Event_JFD_RTP_Piety_BenedictineMonk.Outcomes[2].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local pietyReward = player:GetEraAdjustedValue(mathCeil(15*iMod))
...
local Event_JFD_RTP_Piety_HolyOrder = {}
	...
	Event_JFD_RTP_Piety_HolyOrder.CanFunc = (
		function(player)
			...
			if currentEraID < eraMedievalID or currentEraID >= eraModernID then return false end
			if (not player:HasStateReligion()) then return false end
			Event_JFD_RTP_Piety_HolyOrder.Weight = player:GetPietyLevelEventWeight()
			if player:GetPietyLevel() < 1 then return false end
	...
	Event_JFD_RTP_Piety_HolyOrder.Outcomes[1].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local faithCost = player:GetEraAdjustedValue(mathCeil(50 * iMod))
			local goldCost 	= player:GetEraAdjustedValue(mathCeil(100 * iMod))
			if player:GetFaith() < faithCost then return false end 
			if player:GetGold()  < goldCost  then return false end 
	...
	Event_JFD_RTP_Piety_HolyOrder.Outcomes[2].CanFunc = (
		function(player)			
			...
			if player:GetPiety() < pietyCost then return false end
			local pietyLevelMod = player:GetPietyLevelRewardMod()
			local faithReward = player:GetEraAdjustedValue(mathCeil((65*iMod)*pietyLevelMod))
...
local Event_JFD_RTP_Piety_CallSecularization = {}
	...
	Event_JFD_RTP_Piety_CallSecularization.Outcomes[1].CanFunc = (
		function(player)			
			local pietyReward = player:GetPietyPercent(100, true)
	...
	Event_JFD_RTP_Piety_CallSecularization.Outcomes[2].CanFunc = (
		function(player)	
			local playerID = player:GetID()
			local pietyLevelMod = player:GetPietyLevelRewardMod()
			local cultureReward = player:GetEraAdjustedValue(mathCeil((70*iMod)*pietyLevelMod))
...
local Event_JFD_RTP_Piety_ClergyConcerned1 = {}
	...
	Event_JFD_RTP_Piety_ClergyConcerned1.Outcomes[1].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local pietyReward = player:GetEraAdjustedValue(mathCeil(10*iMod))
			local cultureCost = player:GetEraAdjustedValue(mathCeil(46*iMod))
			if player:GetJONSCulture() < cultureCost then return false end
	...
	Event_JFD_RTP_Piety_ClergyConcerned1.Outcomes[3].CanFunc = (
		function(player)	
			local playerID = player:GetID()
			local faithCost = player:GetEraAdjustedValue(mathCeil(70*iMod))
			local goldCost 	= player:GetEraAdjustedValue(mathCeil(120*iMod))
			if player:GetGold() < goldCost 	 then return false end
			if player:GetFaith() < faithCost then return false end
...
local Event_JFD_RTP_Piety_ClergyConcerned2 = {}
	...
	Event_JFD_RTP_Piety_ClergyConcerned2.CanFunc = (
		function(player)			
			...
			if player:GetPietyLevel() >= 3 then return false end
			return true
		end
		)
	...
	Event_JFD_RTP_Piety_ClergyConcerned2.Outcomes[1].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local pietyCost = player:GetEraAdjustedValue(mathCeil(20*iMod))
			local cultureReward = player:GetEraAdjustedValue(mathCeil(46*iMod))
			if player:GetPiety() < pietyCost then return false end
...
local Event_JFD_RTP_Piety_HellFog = {}
	...
	Event_JFD_RTP_Piety_HellFog.CanFunc = (
		function(player)			
			...
			local faithCost = player:GetEraAdjustedValue(mathCeil(50*iMod))
			if player:GetFaith() < faithCost then return false end
			local goldCost = player:GetEraAdjustedValue(mathCeil(65*iMod))
			if player:GetGold() < goldCost then return false end
...
local Event_JFD_RTP_Piety_HagiaSophia = {}
	...
	Event_JFD_RTP_Piety_HagiaSophia.CanFunc = (
		function(player)			
			...
			local goldCost = player:GetEraAdjustedValue(mathCeil(25*iMod))
			if player:GetGold() < goldCost then return false end
...
local Event_JFD_RTP_Piety_BookBurning = {}
	...
	Event_JFD_RTP_Piety_BookBurning.CanFunc = (
		function(player)			
			local playerID = player:GetID()
			if (not player:HasStateReligion()) then return false end
			local pietyCost = player:GetEraAdjustedValue(mathCeil(5*iMod))
			if player:GetPiety() < pietyCost then return false end
			local cultureCost = player:GetEraAdjustedValue(mathCeil(25*iMod))
			if player:GetJONSCulture() < cultureCost then return false end
			return true
		end
		)
	...
	Event_JFD_RTP_Piety_BookBurning.Outcomes[1].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local pietyReward = player:GetEraAdjustedValue(mathCeil(7*iMod))
			local cultureCost = player:GetEraAdjustedValue(mathCeil(25*iMod))
	...
	Event_JFD_RTP_Piety_BookBurning.Outcomes[2].CanFunc = (
		function(player)			
			local playerID = player:GetID()
			local pietyCost = player:GetEraAdjustedValue(mathCeil(5*iMod))
			local cultureReward = player:GetEraAdjustedValue(mathCeil(25*iMod))
...
local Event_JFD_RTP_Piety_Schism = {}
	...
	Event_JFD_RTP_Piety_Schism.CanFunc = (
		function(player)	
			...
			if player:GetCurrentEra() < eraRenaissanceID then return false end
			if (not player:HasStateReligion()) then return false end
			if player:GetPietyLevel() >= 4 then return false end
			local stateReligionID = player:GetStateReligion()
	...
	Event_JFD_RTP_Piety_Schism.Outcomes[1].DoFunc = (
		function(player) 
			...
			for belief in GameInfo.Beliefs("Follower > 0 AND RequiresFounding > 0") do
				beliefFollowers1[beliefFollower1Count] = belief.ID
				beliefFollower1Count = beliefFollower1Count + 1
			end
			for belief in GameInfo.Beliefs("Follower > 0 AND RequiresEnhancement > 0") do
...
local Event_JFD_RTP_Piety_Bodhisattva = {}
	...
	Event_JFD_RTP_Piety_Bodhisattva.Outcomes[2].CanFunc = (
		function(player)	
			local playerID = player:GetID()
			local faithCost = player:GetEraAdjustedValue(mathCeil(10*iMod))
	...
	Event_JFD_RTP_Piety_Bodhisattva.Outcomes[3].CanFunc = (
		function(player)	
			local playerID = player:GetID()
			local faithCost = player:GetEraAdjustedValue(mathCeil(25*iMod))
			local pietyReward = player:GetEraAdjustedValue(mathCeil(4*iMod))
