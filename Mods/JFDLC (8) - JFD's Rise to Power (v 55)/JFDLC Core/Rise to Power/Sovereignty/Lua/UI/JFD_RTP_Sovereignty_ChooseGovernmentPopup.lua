...
function RefreshList()
	...
	for government in GameInfo.JFD_Governments("IsDisabled = 0") do
		...
		if (reqBuilding~=nil and GameInfo.Buildings[reqBuilding]~=nil) and (not pPlayer:HasBuilding(GameInfoTypes[reqBuilding])) then
			itemInstance.Button:SetAlpha(0.5)
			itemInstance.Button:SetDisabled(true)
			itemInstance.Help:SetText(Locale.ConvertTextKey(governmentHelp) .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_JFD_CHOOSE_GOVERNMENT_REQUIRES_BUILDING", GameInfo.Buildings[reqBuilding].Description));
		end
