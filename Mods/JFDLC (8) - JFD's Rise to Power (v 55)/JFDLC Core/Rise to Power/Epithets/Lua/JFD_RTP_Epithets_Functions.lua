...
function JFD_RTP_Epithets_CityCaptured(oldOwnerID, capital, plotX, plotY, newOwnerID)
	...
	for row in GameInfo.JFD_Epithets("IsOnCityConquered = 1") do
		if player:HasEpithet(row.ID) then return end
		epithetID = row.ID
	...
	if city:IsHolyCityAnyReligion() then
		for row in GameInfo.JFD_Epithets("IsOnHolyCityCaptured = 1") do
			if player:HasEpithet(row.ID) then return end
			epithetID = row.ID
			epithetChance = (row.AcquisitionChance)*(row.NumHolyCitiesMod*player:GetNumHolyCities()+1)
		end
	end
	...
	if (epithetID and epithetChance > 0) then
		player:DoTestEpithetChance(epithetID, epithetChance)
	end
end
...
function JFD_RTP_Epithets_CityStateClaimed(playerID)
	...
	for row in GameInfo.JFD_Epithets("IsOnCityStateClaimed = 1") do
		if player:HasEpithet(row.ID) then return end
		local numAllies, numFriends = player:GetNumCityStateFriends()
		epithetID = row.ID
...
function JFD_RTP_Epithets_ReformPassed(playerID)
	...
	for row in GameInfo.JFD_Epithets("IsOnReformPassed = 1") do
		if player:HasEpithet(row.ID) then return end
		epithetID = row.ID
		epithetChance = row.AcquisitionChance*(row.NumReformMod*player:GetNumReforms(true))
	end
	if (epithetID and epithetChance > 0) then
		player:DoTestEpithetChance(epithetID, epithetChance)
	end
...
function JFD_RTP_Epithets_UnitCreated(playerID, unitID, unitTypeID)
	...
	for row in GameInfo.JFD_Epithets("IsOnUnitClassBorn = '" .. unitClass .. "'") do
		...
		if numSpecialistMod > 0 then
			...
			epithetChance = epithetChance*(numSpecialistMod*(player:GetTotalSpecialistCount(specialistID)+1))
		end
		local numResourceMod = row.NumResourceMod
		if numResourceMod > 0 then
			local resourceID = GameInfoTypes[row.ResourceMod]
...
function JFD_RTP_Epithets_ShowChooseEpithetPopup(epithetID)
	if activePlayer:HasEpithet(epithetID) then return end
	...
	local leaderName = activePlayer:GetDefaultName()
	local epithetDesc = activePlayer:GetEpithetDescription(epithetID)
	local epithetShortDesc = activePlayer:GetEpithetShortDescription(epithetID)
	...
	if activePlayer:HasAnyEpithet() then
		local currentEpithetID = activePlayer:GetEpithetType()
		local currentEpithetDesc = activePlayer:GetEpithetDescription(currentEpithetID)
		local currentEpithetShortDesc = activePlayer:GetEpithetShortDescription(currentEpithetID)
