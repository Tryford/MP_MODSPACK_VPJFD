...
table.insert(g_Interactions.BaseMenu, {
	OnShow =
		function (tInstance, iMinor, iActivePlayer, tInfo)
			...
			local sLabel = L("TXT_KEY_SUZERAINTRAIT_MINOR_CIV_SUK_NAQADA_BUY", sUnit, iCost, iDignitariesCost)
			if (pMinor:GetAlly() ~= iActivePlayer or not(pPlayer:GetCapitalCity()) or (pPlayer:GetGold() < iCost) or (pPlayer:GetNumResourceAvailable(resourceDignitariesID, false) < iDignitariesCost) or not bTech) then
	...
	OnClick =
		function (tInstance, iMinor, iActivePlayer, tInfo)
			...
			local bTech = (sTech == nil) or pPlayer:HasTech(GameInfoTypes[sTech])
			if (pMinor:GetAlly() ~= iActivePlayer or not(pPlayer:GetCapitalCity()) or (pPlayer:GetGold() < iCost) or (pPlayer:GetNumResourceAvailable(resourceDignitariesID, false) < iDignitariesCost) or not bTech) then return end
