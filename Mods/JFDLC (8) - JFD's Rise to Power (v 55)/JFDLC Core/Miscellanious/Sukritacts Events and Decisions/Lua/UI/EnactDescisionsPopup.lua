...
LuaEvents.JFD_OnEnactDecisionsPopup.Add(OnEnactDecisionsPopup)
...
function SortByEra(a, b)
	return Locale.Compare(a.Era, b.Era) < 0
end
...
g_SortOptions = {
	...
	{"TXT_KEY_DECISION_ERA_SORT", SortByEra},
	...
	if pPlayer:GetID() == Game.GetActivePlayer() then
		if tDecision.NotificationFunc ~= nil then
			...
			if pPlayer:IsHuman() then
				Events.AudioPlay2DSound("AS2D_JFD_DECISION_ENACTED")
				local StrMessage = Locale.ConvertTextKey("TXT_KEY_ENACT_ALERT", tDecision.Name)
				pPlayer:AddNotification(NotificationTypes.NOTIFICATION_JFD_DECISION, StrMessage, tDecision.Name, -1, -1)
			end
		end
	end
...
function RefreshList()
	...
	local numMagistrates = pPlayer:GetNumResourceAvailable(resourceMagistratesID, false)
	if numMagistrates > 0 then
		Controls.PanelSubtitle5:LocalizeAndSetText("[ICON_MAGISTRATES] [COLOR_POSITIVE_TEXT]{1_Num}[ENDCOLOR]", numMagistrates)
		Controls.PanelSubtitle5:LocalizeAndSetToolTip("TXT_KEY_DECISIONS_JFD_MAGISTRATES_TT", "[COLOR_POSITIVE_TEXT]" .. numMagistrates .. "[ENDCOLOR]")
	else
		Controls.PanelSubtitle5:LocalizeAndSetText("[ICON_MAGISTRATES] {1_Num}", numMagistrates)
		Controls.PanelSubtitle5:LocalizeAndSetToolTip("TXT_KEY_DECISIONS_JFD_MAGISTRATES_TT", numMagistrates)
	end
	Controls.PanelSubtitle5:SetHide(false)
	if resourceCaptainsID then
		local numCaptains = pPlayer:GetNumResourceAvailable(resourceCaptainsID, false)
		if numCaptains > 0 then
			Controls.PanelSubtitle1:LocalizeAndSetText("[ICON_JFD_CAPTAIN] [COLOR_POSITIVE_TEXT]{1_Num}[ENDCOLOR]", numCaptains)
			Controls.PanelSubtitle1:LocalizeAndSetToolTip("TXT_KEY_DECISIONS_JFD_CAPTAINS_TT", "[COLOR_POSITIVE_TEXT]" .. numCaptains .. "[ENDCOLOR]")
		else
			Controls.PanelSubtitle1:LocalizeAndSetText("[ICON_JFD_CAPTAIN] {1_Num}", numCaptains)
			Controls.PanelSubtitle1:LocalizeAndSetToolTip("TXT_KEY_DECISIONS_JFD_CAPTAINS_TT", numCaptains)
		end
		Controls.PanelSubtitle1:SetHide(false)
	end
	if resourceClericsID then
		local numClerics = pPlayer:GetNumResourceAvailable(resourceClericsID, false)
		if numClerics > 0 then
			Controls.PanelSubtitle2:LocalizeAndSetText("[ICON_JFD_CLERIC] [COLOR_POSITIVE_TEXT]{1_Num}[ENDCOLOR]", numClerics)
			Controls.PanelSubtitle2:LocalizeAndSetToolTip("TXT_KEY_DECISIONS_JFD_CLERICS_TT", "[COLOR_POSITIVE_TEXT]" .. numClerics .. "[ENDCOLOR]")
		else
			Controls.PanelSubtitle2:LocalizeAndSetText("[ICON_JFD_CLERIC] {1_Num}", numClerics)
			Controls.PanelSubtitle2:LocalizeAndSetToolTip("TXT_KEY_DECISIONS_JFD_CLERICS_TT", numClerics)
		end
		Controls.PanelSubtitle2:SetHide(false)
	end
	if resourceDignitariesID then
		local numDignitaries = pPlayer:GetNumResourceAvailable(resourceDignitariesID, false)
		if numDignitaries > 0 then
			Controls.PanelSubtitle3:LocalizeAndSetText("[ICON_JFD_DIGNITARY] [COLOR_POSITIVE_TEXT]{1_Num}[ENDCOLOR]", numDignitaries)
			Controls.PanelSubtitle3:LocalizeAndSetToolTip("TXT_KEY_DECISIONS_JFD_DIGNITARIES_TT", "[COLOR_POSITIVE_TEXT]" .. numDignitaries .. "[ENDCOLOR]")
		else
			Controls.PanelSubtitle3:LocalizeAndSetText("[ICON_JFD_DIGNITARY] {1_Num}", numDignitaries)
			Controls.PanelSubtitle3:LocalizeAndSetToolTip("TXT_KEY_DECISIONS_JFD_DIGNITARIES_TT", numDignitaries)
		end
		Controls.PanelSubtitle3:SetHide(false)
	end
	if resourceInventorsID then
		local numInventors = pPlayer:GetNumResourceAvailable(resourceInventorsID, false)
		if numInventors > 0 then
			Controls.PanelSubtitle4:LocalizeAndSetText("[ICON_JFD_INVENTOR] [COLOR_POSITIVE_TEXT]{1_Num}[ENDCOLOR]", numInventors)
			Controls.PanelSubtitle4:LocalizeAndSetToolTip("TXT_KEY_DECISIONS_JFD_INVENTORS_TT", "[COLOR_POSITIVE_TEXT]" .. numInventors .. "[ENDCOLOR]")
		else
			Controls.PanelSubtitle4:LocalizeAndSetText("[ICON_JFD_INVENTOR] {1_Num}", numInventors)
			Controls.PanelSubtitle4:LocalizeAndSetToolTip("TXT_KEY_DECISIONS_JFD_INVENTORS_TT", numInventors)
		end
		Controls.PanelSubtitle4:SetHide(false)
	end
	local tEnacted = {}
	local tEnactable = {}
	local tAvailable = {}
	...
	if pPlayer:IsHuman() then
		if iPlayer == iActivePlayer then
			...
			if (#tNewDecisions > 0) then
				...
				pPlayer:AddNotification(NotificationTypes.NOTIFICATION_JFD_DECISION, strTooltip, strTitle, -1, -1)
				LuaEvents.JFD_Tutorial_PushTutorialPopup(iPlayer, "IsOnDecisionAvailable", nil, nil, nil, nil)
			end
		end
