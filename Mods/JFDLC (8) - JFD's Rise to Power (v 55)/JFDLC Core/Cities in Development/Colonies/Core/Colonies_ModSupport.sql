...
WHERE Type = 'BUILDING_JFD_PENA_PALACE'
...
WHERE Type IN ('UNIT_JFD_COLONIST', 'UNIT_SETTLER', 'UNIT_WORKER', 'UNIT_JFD_WORKER', 'UNIT_WORKBOAT')
AND EXISTS (SELECT * FROM JFD_GlobalUserSettings WHERE Type = 'JFD_CID_COLONIES_BELIEF_ADDITIONS_MISSION_COLONIES' AND Value = 1);

CREATE TRIGGER JFDCID_Colonies_Unit_BeliefPurchases
AFTER INSERT ON Units 
WHEN NEW.Class IN ('UNITCLASS_WORKER', 'UNITCLASS_WORKBOAT', 'UNITCLASS_COLONIST') 
AND EXISTS (SELECT * FROM JFD_GlobalUserSettings WHERE Type = 'JFD_CID_COLONIES_BELIEF_ADDITIONS_MISSION_COLONIES' AND Value = 1)
BEGIN
	UPDATE Units
	SET FaithCost = 500, BeliefRequired = 'BELIEF_JFD_MISSION_COLONIES'
