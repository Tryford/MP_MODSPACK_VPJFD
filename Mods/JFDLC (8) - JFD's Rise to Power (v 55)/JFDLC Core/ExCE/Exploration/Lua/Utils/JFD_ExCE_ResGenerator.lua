...
include("JFD_ExCE_ResGeneratorUtils.lua");
--include("JFD_ExCEUtilities.lua");

local userSettingTribeAccurate = (Game.GetUserSetting("JFD_EXPLORATION_TRIBES_ACCURATE") == 1)
...
function OnMapResourceGenerator(New_ResourceType , ExecludesTable )
	...
	if New_Resource_Class == "RESOURCECLASS_LUXURY" or New_Resource_Class == "RESOURCECLASS_JFD_TRIBE" then	-- Luxury Resources			
	...
	if New_Resource_Class == "RESOURCECLASS_LUXURY" or New_Resource_Class == "RESOURCECLASS_JFD_TRIBE" then
	...
	--Now check for any rare resources
	for iRow in GameInfo.Resource_JFD_RarityMods() do
		if iRow.ResourceType == New_ResourceType then
			iQuantity = iQuantity / iRow.RarityMod
		end
	end

	-- Now ceil function to get integer quantity value:
	iQuantity = math.floor( iQuantity );
			...
			for x= 1,(x2-x1+1) do 
				for y= 1,(y2-y1+1) do
					...
					if is_Done then
						...
						if (not iPlot:IsNaturalWonder()) then
...
function OnMapImprovementGenerator(improvementType, rarityMod)

	--Get Improvement ID:
	local improvementFound = false
	local improvementID;
	for improvement in GameInfo.Improvements() do
		if improvement.Type == improvementType then 
			improvementID = improvement.ID
			improvementFound = true
		end
	end
	-- Error handling 
	if improvementFound == false then
		print("Improvement type (" .. improvementType .. ") was not found! check Improvements type in sql file.. script aborted");
		return;
	end
	-- Grid and Map area
	local mapX, mapY = Map.GetGridSize();
	local mapArea = mapX*mapY;

	------------------------ Resource Quantity ---------------------------------
	
	local quantity =  0.0008*mapArea + 2 ;
	local numCivs = Game.CountCivPlayersAlive() - PreGame.GetNumMinorCivs() ; 
	local numCivsModifier = 1 + (0.03*numCivs);
	
	-- Now multiply quantity with modifiers
	quantity = quantity * numCivsModifier
	quantity = (quantity / rarityMod)

	-- Now ceil function to get integer quantity value:
	quantity = math.floor( quantity );

	--Last Step - Max Limit: Maximum number of Landmarks allowed on any map = 50 
	if quantity > 50 then 
		quantity = 50;     
	end

	if quantity == 0 then	-- if 0 resources then exit the script no need to continue
		print("Quantity=0 , No landmarks were placed");
		return;
	end 

	print(improvementType .. " Quantity to distribute = " .. quantity);
	...
	K = quantity; --  #reigions = #Resources

	-- Get all allowed terrain types and feature types for the resource:
	GetAllowedPlotTypesForLandmark(improvementType);
	...
	if K>0 then
		for i = 1,K do
			is_Done = false;
			x1 = 1+ math.floor ( (mapX-1) * Boundaries_Table[K][i][1] );
			y1 = 1+ math.floor ( (mapY-1) * Boundaries_Table[K][i][2] );
			x2 = math.floor ( (mapX-1) * Boundaries_Table[K][i][3] );
			y2 = math.floor ( (mapY-1) * Boundaries_Table[K][i][4] );
			Rx = Shuffle_Function(x1,x2);   --Random x axis series
			Ry = Shuffle_Function(y1,y2);   --Random y axis series
			...
			for x= 1,(x2-x1+1) do 
				for y= 1,(y2-y1+1) do
					iPlot = Map.GetPlot(Rx[x], Ry[y]);
					--check terrains,feautures ,,, etc then set the resource:
					is_Done = iPlot:IsTerrainOcean()
					if is_Done then
						N = 1;
						-- Resource placed at plot
						iPlot:SetImprovementType(improvementID, N);
						print(N .. " " .. improvementType .. " Placed at X,Y = " .. Rx[x] .. "," .. Ry[y]);
						test_sum = test_sum + 1;
						test_q = test_q + N;
						break;
					end
	...
	print("----------------------------------------------------");
	print("Total " .. improvementType .. " Placed = " .. test_sum);
	print("Quantity " .. improvementType .. " Placed = " .. test_q);
	print("----------------------------------------------------");
end

function OnMapTribeGenerator(New_TribeType)
	--Get Tribe ID:
	local iTribeFound = false;
	local New_Tribe_ID;
	local New_Tribe_Class;
	local bTribeTribe
	for m_Tribe in GameInfo.JFD_Tribes("Type = '" .. New_TribeType .. "'") do
		New_Tribe_ID = m_Tribe.ID;
		iTribeFound = true;
	end
	-- Error handling 
	if iTribeFound == false then
		print("Tribe type (" .. New_TribeType .. ") was not found! check tribes type in xml file.. script aborted");
		return;
	end
	-- Grid and Map area
	local Xmax, Ymax = Map.GetGridSize();
	...
	--[[ Base Quantity:
		 ...
		 A) For Luxury Tribes:
			From statistics the formula for Luxury Tribes Quantity = 0.0008*iMapArea + 2
	
		 B) For Bonus Tribes:
			From statistics the formula for Bonus Tribes Quantity = 0.0046*iMapArea + 1

		 C) For Strategic Tribes:
			from statistics the formula for Strategic Tribes Quantity = 0.019*iMapArea + 9
	--]]

	--[[ Tribe Options:								------- Modifiers -------	
							OptionName			Value	Luxury	Strtg.	Bonus
							Sparse				1		0.75	0.75	0.75
							Standard			2		1		1		1
							Abundant			3		1.2		1.5		1.2
	...
	--[[ Nomber of Civs modifier:
		 ...
		 -For Luxury and Bonus Tribes:   modifier = 1 + 0.03*NomberOfCivs

		 -For Strategic Tribes:          modifier = 1 + 0.03*NomberOfCivs (when option="Strategic Balance")
		  (strategic resources are only affected by NomberOfCivs when strategic balance option is selected)
	--]]
	...
	--Now check for any rare resources
	--for iRow in GameInfo.Tribe_JFD_RarityMods() do
	--	if iRow.TribeType == New_TribeType then
	--		iQuantity = iQuantity / iRow.RarityMod
	--	end
	--end
	...
	--[[ Strategic Quantity Types:
		 If strategic resource then distribute the quantity according to Tribe_QuantityTypes in xml file.
		 iQuantityTypes: this table will be used to store all quantity types for a resource (example: for iron iQuantityTypes={2,6})
		 Strategic_Distribution: this table will be used to store randomly generated quantities from quantity types 
		                         (example: for iron Strategic_Distribution={2,6,6,2,2,2,6,2,6,6, ... etc))
	--]]
	...
	print(New_TribeType .. " Quantity to distribut = " .. iQuantity);

	--======================= Distributing Tribe on the map =======================
	...
	K = iQuantity; --  #reigions = #Tribes

	-- Get all allowed terrain types and feature types for the resource:
	--GetAllowedPlotTypesForTribe(New_TribeType, ExecludesTable);

	local tribe = GameInfo.JFD_Tribes[New_TribeType]
	local requiredFeature = tribe.RequiredFeature
	local requiredTerrain = tribe.RequiredTerrain
	local requiresHills = tribe.RequiresHills
	local requiresFlatlands = tribe.RequiresFlatlands
	local requiresMountain = tribe.RequiresMountain
	local requiresNotFeature = tribe.RequiresNoFeature
	-- Main loop
	local test_sum=0;
	local test_q=0;
	if K>0 then
		for i = 1,K do
			...
			for x= 1,(x2-x1+1) do 
				for y= 1,(y2-y1+1) do
					plot = Map.GetPlot(Rx[x], Ry[y]);
					--check terrains,feautures ,,, etc then set the resource:
					N = 1;
					if plot and (not plot:GetUnit()) and (not plot:IsCity()) and (not plot:IsTerrainOcean()) and (not plot:IsMountain()) and (not plot:IsWater()) then
						local placeTribe = true
						local featureID = plot:GetFeatureType()
						if (requiresNotFeature and featureID > -1) then
							placeTribe = false
						end
						if (requiredFeature and featureID ~= GameInfoTypes[requiredFeature]) then
							placeTribe = false
						end
						local terrainID = plot:GetTerrainType()
						if (requiredTerrain and featureID == -1 and terrainID ~= GameInfoTypes[requiredTerrain]) then
							placeTribe = false
						end
						if requiresHills and (not plot:IsPlotHill()) then
							placeTribe = false
						end
						if requiresFlatlands and plot:IsPlotHill() then
							placeTribe = false
						end
						if requiresMountain and (not plot:IsPlotMountain()) then
							placeTribe = false
						end
						for row in GameInfo.Improvements("JFD_IsTribe > 0") do
							if plot:IsWithinDistanceOfImprovement(row.ID, 3) then
								placeTribe = false
								--break
							end
						end
						if (placeTribe and plot:GetImprovementType() == -1) then
							local improvementType = GameInfo.JFD_Tribes[New_Tribe_ID].ArtDefineTag
							local improvementID = GameInfoTypes[improvementType]
							if (not JFD_Tribes[New_Tribe_ID]) then
								plot:SetImprovementType(improvementID)
								if plot:GetImprovementType() > -1 then
									JFD_Tribes[New_Tribe_ID] = {}
									JFD_Tribes[New_Tribe_ID].PlotX = plot:GetX()
									JFD_Tribes[New_Tribe_ID].PlotY = plot:GetY()
									LuaEvents.JFD_ExCE_Tribes_PlaceBanner(New_Tribe_ID)
									print(N .. " " .. New_TribeType .. " Placed at X,Y = " .. Rx[x] .. "," .. Ry[y]);
								end
							end
						end
					end
	...
	print("----------------------------------------------------");
	print("Total " .. New_TribeType .. " Placed = " .. test_sum);
	print("Quantity " .. New_TribeType .. " Placed = " .. test_q);
	print("----------------------------------------------------");
end
