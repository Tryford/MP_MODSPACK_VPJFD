...
function GetAllowedPlotTypes(iRes_Type, ExecludesTable)
	...
	-- Other Conditions:
	Only_Lakes = GameInfo.Resources[iRes_Type].Lakes;
	Only_Rivers = GameInfo.Resources[iRes_Type].Rivers;
	...
	print(iRes_Type, iFeature_Types.Jungle, ExecludesTable[i])
...
function GetAllowedPlotTypesForLandmark(landmarkType)

	local pTerrain;
	local pFeature;
	ResetAllowedPlotTypes();
	-- The allowed terrain and feature types are set from the xml file, so if you want to change allowed types change in xml not here.
	for pTerrain in GameInfo.Improvement_ValidTerrains() do
		if pTerrain.ImprovementType == landmarkType then
			if pTerrain.TerrainType == "TERRAIN_GRASS" then
				iTerrain_Types.Grass = 1;
			elseif pTerrain.TerrainType == "TERRAIN_PLAINS" then
				iTerrain_Types.Plains = 1;
	...
	for pFeature in GameInfo.Improvement_ValidFeatures() do
		if pFeature.ImprovementType == landmarkType then
			if pFeature.FeatureType == "FEATURE_OASIS" then
				iFeature_Types.Oasis = 1;
			elseif pFeature.FeatureType == "FEATURE_FLOOD_PLAINS" then
				iFeature_Types.Flood_Plains = 1;
...
function GetAllowedPlotTypesForTribe(New_TribeType, ExecludesTable)
	...
	-- The allowed terrain and feature types are set from the xml file, so if you want to change allowed types change in xml not here.
	for pTerrain in GameInfo.JFD_Tribe_TerrainBooleans("TribeType = '" .. New_TribeType .. "'") do
		if pTerrain.TerrainType == "TERRAIN_GRASS" then
			iTerrain_Types.Grass = 1;
		elseif pTerrain.TerrainType == "TERRAIN_PLAINS" then
			iTerrain_Types.Plains = 1;
	...
	for pFeature in GameInfo.JFD_Tribe_FeatureBooleans("TribeType = '" .. New_TribeType .. "'") do
		if pFeature.FeatureType == "FEATURE_OASIS" then
			iFeature_Types.Oasis = 1;
		elseif pFeature.FeatureType == "FEATURE_FLOOD_PLAINS" then
			iFeature_Types.Flood_Plains = 1;
	...
	-- Other Conditions:
	Only_Hills = GameInfo.JFD_Tribes[New_TribeType].Hills;
	Flat_Land = GameInfo.JFD_Tribes[New_TribeType].Flatlands;
	--- Terrain And Feature Execludes:
	if ExecludesTable ~= nil then
		if type(ExecludesTable) == "table" then
			Z = #ExecludesTable;
...
function isSuitable_Plot_Check( pPlot)
	...
	if pPlot:GetResourceType(-1) == -1 and pPlot:GetImprovementType(-1) == -1 and not pPlot:IsCity() and not pPlot:IsGoody() then
		if  (iTerrain +1) * iTerrain_Types.Grass == TerrainTypes.TERRAIN_GRASS +1 or
			...
			if TerrainExecludes[iTerrain] ~= 1 and FeatureExecludes[iFeature] ~= 1 then
				if bTribe then
					for loop, direction in ipairs(direction_types) do
						local adjPlot = Map.PlotDirection(pPlot:GetX(), pPlot:GetY(), direction) 
						 if adjPlot:GetImprovementType() ~= -1 then
							return false;
						end
					end
				end
				-- Only Lakes Condition
				if Only_Lakes and pPlot:IsLake() then
					return true;
				elseif Only_Lakes and not pPlot:IsLake() then
					return false;
				end
				...
				-- Can be Mountain Condition
				if Mountain_Valid and pPlot:IsMountain() then
					return true;
				elseif Mountain_Valid and not pPlot:IsMountain() then
					return false;
				end
