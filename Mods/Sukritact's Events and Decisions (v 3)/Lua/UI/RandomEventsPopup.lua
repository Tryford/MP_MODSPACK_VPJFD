...
if g_TSLSave_Enabled then
	...
	MapModData.EventsDecisions_RandomEvents = MapModData.EventsDecisions_RandomEvents or {}
	tableRoot = MapModData.EventsDecisions_RandomEvents
	tableName = "EventsDecisions_RandomEvents"

	include("EnD_TSLSerializerV3.lua");
	TableLoad(tableRoot, tableName)

	MY_MOD_NAME = "EventsDecisions_RandomEvents"; MY_MOD_SAVE = MapModData.EventsDecisions_RandomEvents;
	include("TSL_Sukritact_SaveUtils.lua");
	------------
else
	print("Using Sukritact's NewSaveUtils")
	include("Sukritact_SaveUtils.lua"); MY_MOD_NAME = "Sukritact_RandomEvents_Popup";
end
...
if g_TSLSave_Enabled then
	tableRoot.ScheduledEvents = {}; g_tScheduledEvents = tableRoot.ScheduledEvents;
else
	g_tScheduledEvents = {}
...
function ShowPopup(pPlayer, tEvent)
	...
	if pPlayer:IsHuman() then
		...
		if tEvent.EventImage then
			Controls.EventPicture:SetHide(false)
			Controls.EventPicture:SetTextureAndResize(tEvent.EventImage)
		else
			Controls.EventPicture:SetHide(true)
		end
