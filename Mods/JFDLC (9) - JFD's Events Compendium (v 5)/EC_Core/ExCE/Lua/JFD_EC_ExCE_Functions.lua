...
local eventChoiceVoyager1ID = GameInfoTypes["PLAYER_EVENT_JFD_EXPLORATION_VENTURER_CHOICE_1"]
...
function JFD_ExCE_Exploraton_EventChoice(playerID, eventChoiceID)
	if eventChoiceID == eventChoiceVoyager1ID then
...
function JFD_ExCE_Exploraton_EventChoiceStrings(playerID, cityID, eventChoice, tOverrideStrings)
	if eventChoice.Type == "PLAYER_EVENT_JFD_EXPLORATION_VENTURER_CHOICE_1" then
		local player = Players[playerID]
		local currentEra = player:GetCurrentEra()
		if currentEra == 0 then currentEra = 1 end
		local goldCost = 0 
		for row in GameInfo.EventChoice_MinimumStartYield("EventChoiceType = 'PLAYER_EVENT_JFD_EXPLORATION_VENTURER_CHOICE_1'") do
			goldCost = (row.Yield)*currentEra
			break
		end
		local unitID = GetStrongestMilitaryUnit(player, true, "UNITCOMBAT_RECON") or 0
		local help = Locale.ConvertTextKey("TXT_KEY_PLAYER_EVENT_JFD_EXPLORATION_VENTURER_CHOICE_1_HELP", goldCost, GameInfo.Units[unitID].Description)
