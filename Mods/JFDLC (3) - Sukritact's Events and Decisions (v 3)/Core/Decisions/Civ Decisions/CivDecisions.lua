...
local Decisions_SiamEmbassies = {}
	...
	Decisions_SiamEmbassies.Data2 = {10, 10, 20, 20, 20, 30, 30, 30}
	...
	function(pPlayer)
		...
		local iCost = math.ceil(#Decisions_SiamEmbassies.Data1 * 100 * iMod)
		...
		if (iEra < GameInfoTypes.ERA_RENAISSANCE) or (iEra > GameInfoTypes.ERA_INDUSTRIAL) then return true, false end
...
local Decisions_GermanConfederation = {}
	...
	function(pPlayer)
		...
		local iCostC = math.ceil(pPlayer:GetNextPolicyCost() * 0.6)
		local iCostG = math.ceil(800 * iMod)
		Decisions_GermanConfederation.Desc = Locale.ConvertTextKey("TXT_KEY_DECISIONS_GERMANCONFEDERATION_DESC", iCostG, iCostC)

		if (pPlayer:GetNumResourceAvailable(iMagistrate, false) < 1) then return true, false end
		if (pPlayer:GetJONSCulture() < iCostC) then return true, false end
		if (pPlayer:GetGold() < iCostG) then return true, false end
		...
		for pCity in pPlayer:Cities() do
			...
			if pOriginalOwner and pOriginalOwner:IsMinorCiv() then
				iNumCS = iNumCS + 1
			end	
		end
		if iNumCS < 4 then
		...
		local iEra = pPlayer:GetCurrentEra()
		if (iEra < GameInfoTypes.ERA_INDUSTRIAL) then return true, false end
	...
	function(pPlayer)
		...
		Decisions_GermanConfederation.Monitors[GameEvents.PlayerDoTurn](pPlayer:GetID())
		save(pPlayer, "Decisions_GermanConfederation", true)
	end
	)

	Decisions_GermanConfederation.Monitors = {}
	Decisions_GermanConfederation.Monitors[GameEvents.PlayerDoTurn] =  (	
	function(iPlayer)
		pPlayer = Players[iPlayer]
		if pPlayer:CountNumBuildings(GameInfoTypes.BUILDING_DECISIONS_GERMANCONFEDERATION) > 0 then
			...
			pCity:SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_GERMANCONFEDERATION2, iNumCS)
...
local Decisions_GermanGuildLaws = {}
	Decisions_GermanGuildLaws.Name = "TXT_KEY_DECISIONS_GERMANGUILDLAWS"
	Decisions_GermanGuildLaws.Desc = "TXT_KEY_DECISIONS_GERMANGUILDLAWS_DESC"
	HookDecisionCivilizationIcon(Decisions_GermanGuildLaws, "CIVILIZATION_GERMANY")
	Decisions_GermanGuildLaws.CanFunc = (
	function(pPlayer)
		if pPlayer:GetCivilizationType() ~= GameInfoTypes.CIVILIZATION_GERMANY then return false, false end
		if load(pPlayer, "Decisions_GermanGuildLaws") == true then
			Decisions_GermanGuildLaws.Desc = Locale.ConvertTextKey("TXT_KEY_DECISIONS_GERMANGUILDLAWS_ENACTED_DESC")
			return false, false, true
		end

		local iTurns = math.ceil(1 * iGAMod)
		Decisions_GermanGuildLaws.Desc = Locale.ConvertTextKey("TXT_KEY_DECISIONS_GERMANGUILDLAWS_DESC", iTurns)
		if (pPlayer:GetNumResourceAvailable(iMagistrate, false) < 1) then return true, false end
	...
	Decisions_GermanGuildLaws.DoFunc = (
	function(pPlayer)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -1)
		local iTurns = math.ceil(1 * iGAMod)
		for pCity in pPlayer:Cities() do
		...
		pPlayer:SetNumFreePolicies(1)
		pPlayer:SetNumFreePolicies(0)
		pPlayer:SetHasPolicy(GameInfoTypes.POLICY_DECISIONS_GERMANGUILDLAWS, true)		

		save(pPlayer, "Decisions_GermanGuildLaws", true)
	end
	)
	
Decisions_AddCivilisationSpecific(GameInfoTypes.CIVILIZATION_GERMANY, "Decisions_GermanGuildLaws", Decisions_GermanGuildLaws)
