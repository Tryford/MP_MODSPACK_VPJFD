...
function JFD_OnReformClicked(reformID)
	...
	local reformAnarchySentiment, reformAnarchyTurns, reformAnarchyCooldownTurns = JFD_GetReformAnarchy(Game.GetActivePlayer(), reformID)
...
function JFD_OnYes()
	...
	JFD_SetHasReform(Game.GetActivePlayer(), chosenReformID, true)
...
function JFD_RefreshYourGovernmentStats()
	...
	local headOfStateTT = JFD_GetLeaderTitle(activePlayerID)
	Controls.HeadOfState:LocalizeAndSetText(headOfState)
	Controls.HeadOfState:LocalizeAndSetToolTip(headOfStateTT)
	--Head of Government
	local isAbsolute = JFD_HasReform(activePlayerID, reformExecutiveAbsoluteID)
	local headOfGovt = activePlayer:GetName()
	local headOfGovtTitle = JFD_GetHeadOfGovernmentTitle(activePlayerID, (governmentID == governmentTheocracyID))
	local headOfGovtFull = ""
	local dominantPoliticID = JFD_GetDominantPoliticID(activePlayerID)
	if (dominantPoliticID and (not isAbsolute) and governmentID ~= governmentHolyRomeID) then
	...
	elseif governmentID == governmentHolyRomeID then
		IconHookup(5, 128, "JFD_SOVEREIGNTY_GOVERNMENT_ALPHA_ATLAS", Controls.HeadOfStateIcon)
		IconHookup(leader.PortraitIndex, 64, leader.IconAtlas, Controls.CivIconImg)
	else
	...
	local governmentDescriptionShort = JFD_GetGovernmentShortDescription(activePlayerID)
	if (governmentDescriptionShort and governmentID ~= governmentHolyRomeID) then
	...
	--Change Government
	local canChangeGovt, changeGovtTT = JFD_CanChangeGovernment(activePlayerID)
	Controls.ChangeGovernmentsButton:SetDisabled(not canChangeGovt)
	if changeGovtTT then
		Controls.ChangeGovernmentsButton:SetToolTipString(Locale.ConvertTextKey("TXT_KEY_JFD_CHANGE_GOVERNMENT") .. "[NEWLINE][NEWLINE]" .. changeGovtTT)
	else
		Controls.ChangeGovernmentsButton:LocalizeAndSetToolTip("TXT_KEY_JFD_CHANGE_GOVERNMENT")
	end
	--Culture
	...
	for privyCouncillor in GameInfo.JFD_PrivyCouncillors() do	
		local hasCouncillor = JFD_HasPrivyCouncillor(activePlayerID, privyCouncillor.ID)
		local numString = tostring(privyNum)
		local buttonName = "Privy"..numString .."Icon"
		local thisButton = Controls[buttonName]
		local titleDesc = JFD_GetPrivyCouncillorTitle(activePlayerID, privyCouncillor.ID)
		local privyTT = "[COLOR_JFD_SOVEREIGNTY]" .. titleDesc .. " - [COLOR_WARNING_TEXT]Vacant[ENDCOLOR][NEWLINE]"
		if hasCouncillor then
			local councillorUnit = JFD_GetPrivyCouncllor(activePlayerID, privyCouncillor.ID)
			...
			if JFD_RTP[privyCouncillor.ID] then
				councillorName = JFD_RTP[privyCouncillor.ID]
			end
	...
	--Free Reforms
	local numFreeReforms = JFD_GetNumFreeReforms(activePlayerID)
	...
	local reformAnarchySentiment, reformAnarchyTurns, reformAnarchyCooldownTurns, reformAnarchyTT = JFD_GetReformAnarchy(activePlayerID, true)
	...
	local reformCooldown, reformCooldownTT = JFD_GetReformCooldown(activePlayerID, true)
	Controls.Anarchy:SetText(reformAnarchyString) 
	Controls.Anarchy:LocalizeAndSetToolTip("TXT_KEY_JFD_GO_ANARCHY_TT", reformAnarchySentiment, (reformAnarchyTurns+defineDefaultAnarchyTurns)*100, reformAnarchyTurns, reformAnarchyCooldownTurns, reformAnarchyTT) 
	Controls.CurrentSovereigntyLabel:LocalizeAndSetText("TXT_KEY_JFD_CURRENT_SOVEREIGNTY_LABEL", sovereignty)
	Controls.CurrentSovereigntyLabel:LocalizeAndSetToolTip(JFD_GetSovereigntyTopPanelInfoTT(activePlayerID)) 
...
function JFD_RefreshYourGovernmentPolitics()
	...
	local government = GameInfo.JFD_Governments[governmentID]
	local isAbsolute = JFD_HasReform(activePlayerID, reformExecutiveAbsoluteID)
	local hasLegislature = JFD_HasLegislature(activePlayerID)
	...
	for row in GameInfo.JFD_Reforms("LegislatureHelp IS NOT NULL") do
		if JFD_HasReform(activePlayerID, row.ID) then
	...
	if hasLegislature then
		if governmentID == governmentHolyRomeID then
			...
			for row in GameInfo.JFD_Politics("PrereqGovernment = '" .. government.Type .. "' AND NOT Type = 'ELECTOR_JFD_ELECTOR'") do
				...
				if ((prereqReformID and JFD_HasReform(activePlayerID, prereqReformID)) or (not prereqReformID)) then
			...
			for _,politic in ipairs(politics) do
				...
				if (not politic.IsRevolutionaries) then
					...
					local politicDesc = Locale.ConvertTextKey(JFD_GetPoliticDescription(activePlayerID, politicID))
					local politicHighlight = "[COLOR_JFD_SOVEREIGNTY_FADING]"
					local politicPercent = politic.Percent
		...
		elseif governmentID == governmentTheocracyID then
			local politics = {};
			if JFD_HasReform(activePlayerID, reformTLegislatureAutocephalousID) then	
				local stateReligionID = activePlayer:GetStateReligion()
				for city in activePlayer:Cities() do
					local cityID = JFD_CompileCityID(city)
					local politicPercent = activePlayer:GetPoliticPercent(cityID)
					if (showDetailedFactions or politicPercent > 0) then
						table.insert(politics, {IsRevolutionaries = false, PoliticID = cityID, CityName = city:GetName(), Percent = politicPercent, CityHighestYield = JFD_GetHighestYield(activePlayerID, city)});
					elseif politicPercent > 0 then
						table.insert(politics, {IsRevolutionaries = false, PoliticID = cityID, CityName = city:GetName(), Percent = politicPercent, CityHighestYield = JFD_GetHighestYield(activePlayerID, city)});
					end
				end
				...
				for _,politic in ipairs(politics) do
					...
					if (not politic.IsRevolutionaries) then
						local politicID = politic.PoliticID
						local politicDesc = Locale.ConvertTextKey(JFD_GetPoliticDescription(activePlayerID, stateReligionID, true))
						local politicHighlight = "[COLOR_JFD_SOVEREIGNTY_FADING]"
						local politicPercent = politic.Percent
			...
			elseif JFD_HasReform(activePlayerID, reformTLegislatureEcumenicalID) then
				local stateReligionID = activePlayer:GetStateReligion()
				for otherPlayerID = 0, GameDefines.MAX_MINOR_CIVS - 1 do	
					local otherPlayer = Players[otherPlayerID]
					if otherPlayer:IsAlive() then
						...
						if ((showDetailedFactions or politicPercent > 0) and otherPlayer:GetStateReligion() == stateReligionID) then
							table.insert(politics, {IsRevolutionaries = false, PoliticID = otherPlayerID, PlayerAdj = otherPlayer:GetCivilizationAdjective(), Percent = politicPercent, CivHighestYield = JFD_GetHighestYield(otherPlayerID)});
						elseif politicPercent > 0 then
							table.insert(politics, {IsRevolutionaries = false, PoliticID = otherPlayerID, PlayerAdj = otherPlayer:GetCivilizationAdjective(), Percent = politicPercent, CivHighestYield = JFD_GetHighestYield(otherPlayerID)});
						end
					end
	...
	if activePlayer:HasGovernment() then
		...
		for row in GameInfo.JFD_Reforms("IsDisabled = 0 AND ReformBranchType = '" .. currentReformBranch.Type .. "' AND Alignment = 'REFORM_LEFT'") do
			...
			if isReformVisible then
				...
				local reformAlpha = 1
				local reformCost = JFD_GetReformCost(activePlayerID, reformID)
				...
				local reformHelp = JFD_GetReformHelpTT(activePlayerID, reformID)
				...
				local hasReform = JFD_HasReform(activePlayerID, reformID)
				if hasReform then
					...
					local canHaveReform, canHaveReformIfNotCooldown, disabledFontIcon = JFD_CanHaveReform(activePlayerID, reformID, true)
					...
					if showDetailedReforms then
						local reformFavoured = JFD_GetReformOpinionFavoured(activePlayerID, reformID)
						local reformOpposed  = JFD_GetReformOpinionOpposed(activePlayerID, reformID)
...
function JFD_RefreshYourReforms()
	...
	for reform in GameInfo.JFD_Reforms("IsDisabled = 0 AND HelpBonus IS NOT NULL AND HelpPenalty IS NOT NULL") do
		...
		if JFD_HasReform(activePlayerID, reform.ID) then
...
function JFD_RefreshWorldGovernments()
	...
	for playerID = 0, GameDefines.MAX_CIV_PLAYERS - 1 do	
		...
		if (player:IsAlive() and activeTeam:IsHasMet(player:GetTeam()) and (not(player:IsMinorCiv()))) then
			...
			if JFD_HasLegislature(playerID) then
				politicID = JFD_GetDominantPoliticID(playerID)
				...
				elseif (governmentID == governmentTheocracyID and politicID ~= politicRevolutionariesTID and JFD_HasLegislature(playerID)) then
					if JFD_HasReform(playerID, reformTLegislatureAutocephalousID) then
						politic = Locale.ConvertTextKey("[ICON_JFD_CLERGY]") .. " " .. Locale.ConvertTextKey(JFD_GetPoliticDescription(playerID, player:GetStateReligion(), true))
					...
					elseif JFD_HasReform(playerID, reformTLegislatureEcumenicalID) then
						politic = Locale.ConvertTextKey("[ICON_JFD_CLERGY]") .. " " .. player:GetCivilizationAdjective() .. " " .. Locale.ConvertTextKey(JFD_GetPoliticDescription(playerID, player:GetStateReligion(), true))
					...
					if politicID then
						local politicInfo = GameInfo.JFD_Politics[politicID]
						politic = Locale.ConvertTextKey(politicInfo.FontIcon) .. " " .. Locale.ConvertTextKey(JFD_GetPoliticDescription(playerID, politicID))
	...
	if(#worldGovernments > 0) then
		...
		for i,v in ipairs(worldGovernments) do
			...
			entry.GovernmentType:LocalizeAndSetToolTip(JFD_GetReformsListTT(v.PlayerID))
