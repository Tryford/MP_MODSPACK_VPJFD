...
include( "FLuaVector" );
include( "IconSupport" );
include( "JFD_RTP_SovereigntyUtils" );
...
local chooseGovernmentString = Locale.ConvertTextKey( "TXT_KEY_CHOOSE_JFD_GOVERNMENT" );
...
function OnEndTurnClicked()
	...
	--	end	
	elseif userSettingSovereignty and (JFD_CanAdoptGovernment(Game.GetActivePlayer())) then
...
function OnEndTurnDirty()
	...
	elseif (EndTurnBlockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_CHOOSE_IDEOLOGY) then
		...
		iFlashingState = FLASHING_END_TURN;
	elseif (userSettingSovereignty and JFD_CanAdoptGovernment(Game.GetActivePlayer())) then
		strEndTurnMessage = chooseGovernmentString;
		strButtonToolTip = Locale.ConvertTextKey("TXT_KEY_JFD_CAN_GOVERNMENT_NOTIFICATION_TEXT");
		buttonDisabled = false;
		iFlashingState = FLASHING_END_TURN;
	else
