...
include( "JFD_CID_ColoniesLoyaltyUtils.lua" );
...
local g_CurrencyChosenID = nil
function UpdateProsperity()
	local player = Players[activePlayerID];
	if userSettingProsperityCore then
		local prosperityScore = JFD_GetProsperityScore(activePlayerID)
		Controls.TotalProsperityValue:SetText(prosperityScore);
	else
		Controls.TotalProsperityValue:SetText("n/a");
	end
	if player:HasCurrency() then
		local currency = GameInfo.JFD_Currencies[player:GetCurrency()]
		local currencyDesc = player:GetCurrencyName()
		local currencyIcon = Locale.ConvertTextKey(currency.IconString)
		if (not currencyDesc) then
			currencyDesc = Locale.ConvertTextKey(currencyDesc)
		end
		Controls.YourCurrencyLabel:SetText(currencyIcon .. currencyDesc)
	else
		Controls.YourCurrencyLabel:LocalizeAndSetText("TXT_KEY_EO_TOTAL_JFD_CURRENCY_NONE")
	end

	if JFD_CanAdoptAnyCurrency(activePlayerID) then
		Controls.YourCurrencyLabel:SetDisabled(false)
		Controls.YourCurrencyLabel:LocalizeAndSetToolTip("TXT_KEY_EO_TOTAL_JFD_CURRENCY_NONE_TT")
	else
		Controls.YourCurrencyLabel:SetDisabled(true)
		Controls.YourCurrencyLabel:LocalizeAndSetToolTip("TXT_KEY_EO_TOTAL_JFD_CURRENCY_NONE_TT_DISABLED")
	end
end

-- JFD_OnCurrencySelected
function JFD_OnCurrencySelected(currencyID)
	local currency = GameInfo.JFD_Currencies[currencyID]
	Controls.CurrencyPulldown:GetButton():SetText(currency.IconString .. " " .. Locale.ConvertTextKey(currency.Description))
	g_CurrencyChosenID = currencyID
	Controls.CurrencyName:SetHide(false)
	Controls.CurrencyName:SetText(Locale.ConvertTextKey(currency.Description))
	...
	Controls.SelectButton:LocalizeAndSetToolTip("TXT_KEY_JFD_PROSPERITY_CHOOSE_CURRENCY_MENU_TOOLTIP", currency.IconString, currency.Description)
end

-- JFD_ShowConfirmCurrency
function JFD_ShowConfirmCurrency()
	local player = Players[activePlayerID]
	--if player:HasCurrency() then
	--	local pietyLevelID = JFD_GetPietyLevelID(playerID)
	--	local numAnarchyTurns = GameInfo.JFD_PietyLevels[pietyLevelID].AnarchyTurns
	--	Controls.LabelConfirm:LocalizeAndSetText("TXT_KEY_RO_CONFIRM_JFD_STATE_RELIGION_PIETY", Game.GetReligionName(g_StateReligionChosenID), numAnarchyTurns)
	--else
		Controls.LabelConfirm:LocalizeAndSetText("TXT_KEY_EO_CONFIRM_JFD_CURRENCY", GameInfo.JFD_Currencies[g_CurrencyChosenID].Description)
	--end
	Controls.Confirm:SetHide(false)
end
Controls.SelectButton:RegisterCallback(Mouse.eLClick, JFD_ShowConfirmCurrency)

-- JFD_ConfirmYes
function JFD_ConfirmYes()
	local player = Players[activePlayerID]
	player:SetCurrency(g_CurrencyChosenID)
	Controls.Confirm:SetHide(true)
	Controls.CurrencyChoicePopup:SetHide(true)
	UpdateProsperity()
end
...
-- JFD_ShowChooseCurrencyPopup
function JFD_ShowChooseCurrencyPopup()
	Controls.CurrencyChoicePopup:SetHide(false)
	Controls.CurrencyPulldown:ClearEntries()
	for currency in GameInfo.JFD_Currencies() do
		local currencyID = currency.ID
		if JFD_CanAdoptCurrency(activePlayerID, currencyID) then
			local entry = {}
			Controls.CurrencyPulldown:BuildEntry("InstanceOne", entry)
			entry.Button:SetVoid1(currencyID)
			entry.Button:SetText(currency.IconString .. " " .. Locale.ConvertTextKey(currency.Description))
		end
	end
	Controls.SelectButton:SetDisabled(true)
	Controls.CurrencyPulldown:GetButton():LocalizeAndSetText("TXT_KEY_JFD_PROSPERITY_CHOOSE_CURRENCY_MENU")
	Controls.CurrencyPulldown:CalculateInternals()
	Controls.CurrencyPulldown:RegisterSelectionCallback(JFD_OnCurrencySelected)
end
Controls.YourCurrencyLabel:RegisterCallback(Mouse.eLClick, JFD_ShowChooseCurrencyPopup)

-- JFD_CloseChooseCurrencyPopup
function JFD_CloseChooseCurrencyPopup()
	Controls.CurrencyChoicePopup:SetHide(true)
end
Controls.BackButton:RegisterCallback(Mouse.eLClick, JFD_CloseChooseCurrencyPopup)
-------------------------------------------------
-------------------------------------------------
function UpdateGPT()
