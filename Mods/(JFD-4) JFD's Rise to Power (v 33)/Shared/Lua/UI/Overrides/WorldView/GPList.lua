...
local m_DignitaryIM = InstanceManager:new( "GPInstance", "Root", Controls.DignitaryStack ); -- JFD RTP
...
local m_MagistrateIM = InstanceManager:new( "GPInstance", "Root", Controls.MagistrateStack ); -- JFD RTP
...
local m_ProphetIM = InstanceManager:new( "GPInstance", "Root", Controls.ProphetStack ); -- JFD RTP
...
function UpdateDisplay()
			...
			for pCity in pPlayer:Cities() do
				if (checkIfSpecialist(pPlayer, pSpecialistInfo, pCity)) then
					...
					elseif (pSpecialistInfo.Type == "SPECIALIST_CIVIL_SERVANT") then -- Gazebo CSD
						...
						bDiplomatFound = true;
					elseif (pSpecialistInfo.Type == "SPECIALIST_JFD_DIGNITARY") then -- JFD RTP
					...
					elseif (pSpecialistInfo.Type == "SPECIALIST_ENGINEER") then
						...
						bEngineerFound = true;
					elseif (pSpecialistInfo.Type == "SPECIALIST_JFD_MAGISTRATE") then -- JFD RTP
					...
					elseif (pSpecialistInfo.Type == "SPECIALIST_MERCHANT") then
						...
						bMerchantFound = true;
					elseif (pSpecialistInfo.Type == "SPECIALIST_JFD_MONK") then -- JFD RTP
	...
	Controls.DignitaryStack:SortChildren( SortFunction ); -- JFD RTP
	...
	Controls.MagistrateStack:SortChildren( SortFunction ); -- JFD RTP
	...
	Controls.ProphetStack:SortChildren( SortFunction ); -- JFD RTP
	Controls.WriterStack:SortChildren( SortFunction );

	Controls.ArtistHeader:SetHide( not bArtistFound );
	Controls.DignitaryHeader:SetHide( not bDignitaryFound ); -- JFD RTP
	...
	Controls.MagistrateHeader:SetHide( not bMagistrateFound ); -- JFD RTP
	...
	Controls.MagistrateStack:CalculateSize(); -- JFD RTP
	Controls.MagistrateStack:ReprocessAnchoring(); -- JFD RTP
	...
	Controls.ProphetStack:CalculateSize(); -- JFD RTP
	Controls.ProphetStack:ReprocessAnchoring(); -- JFD RTP
