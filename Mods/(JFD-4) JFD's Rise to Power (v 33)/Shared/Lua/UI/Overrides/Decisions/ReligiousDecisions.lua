...
local Decisions_HolyBook = {}
	...
	function(pPlayer)
		if (not pPlayer:HasStateReligion()) then return false, false end
		...
		iReward = (iReward * GameInfo.JFD_PietyLevels[JFD_GetPietyLevelID(pPlayer:GetID())].RewardModifier)
...
local Decisions_ReligionEvents_1 = {}
	...
	--=========================================================
		...
		--=========================================================
			...
			if bEnacted then
				...
				local iDelta1 = math.ceil((iCurrentEra * 5 + 45) * iCurrentEra * tReligionEvents_1[tBonuses[1]] * iUnity * iMod * 0.6)
				local iDelta2 = math.ceil((iCurrentEra * 5 + 45) * iCurrentEra * tReligionEvents_1[tBonuses[2]] * iUnity * iMod)
		...
		if (pPlayer:GetNumResourceAvailable(iMagistrate, false) >= 2) then bEnactable = true end
	...
	function(pPlayer)
		local iReligion = pPlayer:GetStateReligion()
		local sReligion = GameInfo.Religions[iReligion].Type
		local iUnity = GetReligiousUnity(pPlayer, iReligion)
		local iCurrentEra = pPlayer:GetCurrentEra() + 1
		...
		if religionTheme and pPlayer:IsHuman() then
