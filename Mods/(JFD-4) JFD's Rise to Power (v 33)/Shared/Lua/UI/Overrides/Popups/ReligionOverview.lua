...
Controls.TabButtonStateReligions:SetHide(not userSettingPietyCore)
...
function RefreshYourReligion()
	...
	Controls.StateReligionInfoStack:SetHide(not userSettingPietyCore)
	Controls.StateReligionStatus:SetDisabled(true)
	if (JFD_CanAdoptAnyStateReligion(playerID) and (not player:IsAnarchy())) then
	...
	elseif player:HasStateReligion() then
		...
		Controls.StateReligionStatus:SetToolTipString(stateReligionStatusTT)
		if player:IsAnarchy() then
		...
		elseif (player:HasStateReligion()) then 
			...
			if (player:HasGovernment() and player:GetGovernment() == governmentTheocracyID) then
				if pietyLevelID == pietyLevelDevoutID then
					pietyLevelHelp = Locale.ConvertTextKey("TXT_KEY_PIETY_LEVEL_JFD_THEOCRACY", 15) .. Locale.ConvertTextKey(pietyLevelHelp, JFD_GetStateReligionPressure(playerID))
				elseif pietyLevelID == pietyLevelVirtuousID then
					pietyLevelHelp = Locale.ConvertTextKey("TXT_KEY_PIETY_LEVEL_JFD_THEOCRACY", 5) .. Locale.ConvertTextKey(pietyLevelHelp, JFD_GetStateReligionPressure(playerID))
				else
					pietyLevelHelp = Locale.ConvertTextKey(pietyLevelHelp, JFD_GetStateReligionPressure(playerID))
				end
			else
				pietyLevelHelp = Locale.ConvertTextKey(pietyLevelHelp, JFD_GetStateReligionPressure(playerID))
			end
			...
			if (belief ~= nil) then
				...
				if showBelief then
					...
					if belief.Founder then
						...
						if belief.JFD_PietyDescription then
							entry.BeliefDescription:SetText("[ICON_BULLET]" .. Locale.ConvertTextKey(belief.JFD_PietyDescription))
						else
							entry.BeliefDescription:SetText("[ICON_BULLET]" .. Locale.ConvertTextKey(belief.Description) .. ".");
						end
					end
	...
	if(#stateReligions > 0) then
		...
		for i,v in ipairs(stateReligions) do
			...
			if (playerID ~= activePlayerID and (not player:HasSecularized())) then
				entry.StateReligionName:LocalizeAndSetToolTip(JFD_GetROStateReligionTT(v.CivilizationID))
			else
...
function JFD_ShowChooseStateReligionPopup()
	...
	if JFD_CanSecularize(playerID) then
	...
	for religion in GameInfo.Religions("ID <> '" .. currentStateReligionID .. "' AND ID > 0") do
		...
		if JFD_CanAdoptStateReligion(playerID, religionID) then
			...
			local religionFoundDate = JFD_TranslateDate(Game.GetFoundYear(religionID, religionFounderID))
