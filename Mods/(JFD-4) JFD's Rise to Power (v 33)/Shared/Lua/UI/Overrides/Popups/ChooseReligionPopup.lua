...
function OnPantheonBeliefClick()
	...
	if(ToggleBeliefContext("PantheonBelief")) then
		...
		for i,v in ipairs(Game.GetAvailablePantheonBeliefs()) do
			...
			if belief.Tooltip ~= nil then tooltip = belief.Tooltip; end
...
function RefreshReligions()
	...
	for i, religion in ipairs(religions) do
		...
		local pediaEntry = CivilopediaControl and (GameInfo.Religions[religion.ID].Description)
		if (pediaEntry and (not setDisabled)) then
			religionButton.Button:RegisterCallback(Mouse.eRClick, function() getPedia(pediaEntry) end)
		end
